...
const SCAN_INTERVAL = 5 * 60 * 1000; // 30 minute 
...
function getGPIOStatus(filePath) { 
  try { 
    ...
    const gpioMap = data.IOT_SENSORS?.GPIO;

    if (!gpioMap || typeof gpioMap !== 'object') {
        throw new Error('GPIO map not found');
    }

    return Object.fromEntries(
        Object.entries(gpioMap).map(([portId, gpio]) => [
            portId,
            Boolean(gpio.STATUS)
        ])
    );
...
========================= */ 
function sendGPIOFrame(id,status) { 
  if (id !== "1") return;
...
const server = net.createServer((socket) => { 
  ...
  const gpioStates  = getGPIOStatus(CONFIG_FILE);
  if (gpioStates) {
    for (const [portId, status] of Object.entries(gpioStates)) {
      if (status) sendGPIOFrame(portId, status);
    }
  }
