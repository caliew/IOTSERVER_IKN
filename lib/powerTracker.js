 ...
 * @param {number} thresholdMinutes - Minutes without packet before flagging offline (default: 30)
 * @param {Function} alertCallback - Callback function (siteName, alertObj) when offline event is detected
 */
lib.checkOfflineStatus = function(thresholdMinutes = 30, alertCallback = null) {
  const state = readState();
  const now = Date.now();
