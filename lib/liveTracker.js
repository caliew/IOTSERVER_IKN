...
/**
 * Updates single-line status bar for incoming telemetry.
 * Always updates in-place (\r) to keep terminal neat.
 */
...
function recordTelemetryEvent({
  ...
  _logs.append(fileName, jsonString, (err) => {
    if (err) {
      ...
      if (isAlert) {
        // Dedicated alert on new line ONLY for tracked sensors
        const alertMsg = eventRecord.alertEvaluation.message || 'Threshold Breached';
        const alertLine = `⚠️ [TRACKED ALERT] ${timeStr} | Site: ${siteName} | Sensor: ${nameStr} (${formattedMac}) | Temp: ${tempVal}°C | ${alertMsg}`;
        renderLiveStatusLine(alertLine, true);
      } else {
        // Updated tracked sensor live state line (in-place)
        const stateStr = `🎯 [TRACKED] ${timeStr} | Site: ${siteName} | Name: ${nameStr} | MAC: ${formattedMac} | Temp: ${tempVal}°C${humdVal !== null ? ' | Hum: ' + humdVal + '%' : ''}${battVal !== null ? ' | Batt: ' + battVal + '%' : ''} | Status: ${statusTag}`;
