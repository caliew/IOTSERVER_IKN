 ...
 * Supports MAC ID tracking, Site/Company qualified tracking (e.g. 'ikn_hospital/BA-92-D1-B4-B5-3A'), and auto-lookup of company config settings in .data/<company>/settings.json.
 */
  ...
  try {
    ...
    return parsed.TRACKED_SENSORS.map((entry) => {
      if (typeof entry === 'string') {
        ...
        return {
          site: null, // Matches any site/company
...
/**
 * Returns current list of tracked MAC IDs / rules from config/trackedSensors.json
 */
...
/**
 * Renders the periodic heartbeat / server alive bar
 */
 ...
 * Automatically looks up company config settings from .data/<siteName>/settings.json if sensorConfig is omitted.
 */
function recordTelemetryEvent({
  ...
  if (!isSensorTracked(normId, siteName)) return;

  const formattedMac = formatMac(macId);
  const fileName = `_TRACKING_${formattedMac}`;

  // If sensorConfig is not provided, resolve it automatically from .data/<siteName>/settings.json
