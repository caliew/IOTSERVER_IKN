...
function renderTrackedSensorRow(normMac, stateObj) {
  ...
  const { timestamp, siteName, sensorName, macId, tempVal, humdVal, battVal, statusTag } = stateObj;
  const stateStr = `🎯 [TRACKED] ${timestamp} | ${padEnd(siteName, 16)} | ${padEnd(sensorName, 24)} | MAC: ${macId} | Temp: ${padEnd(tempVal, 8)}${humdVal !== 'N/A' ? ' | Hum: ' + humdVal : ''}${battVal !== 'N/A' ? ' | Batt: ' + battVal : ''} | ${statusTag}`;
...
function recordTelemetryEvent({
  ...
  _logs.append(fileName, jsonString, (err) => {
    ...
    totalPacketsReceived++;
    const timeStr = new Date().toLocaleTimeString();
    const tempVal = parsedValues?.Temperature ?? parsedValues?.TEMP ?? parsedValues?.temp
      ?? (Array.isArray(parsedValues?.DATAS) ? parsedValues.DATAS[0] : null) ?? 'N/A';
    ...
    if (isAlert) {
      ...
      printScrollingAlert(`⚠️ [TRACKED ALERT] ${timeStr} | Site: ${siteName} | Sensor: ${nameStr} (${formattedMac}) | Temp: ${tempVal}°C | ${alertMsg}`);
    }
  });
}
