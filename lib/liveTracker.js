...
function formatHexBytesSpaced(hexStr) {
  if (!hexStr) return '';
  const clean = String(hexStr).toUpperCase().replace(/[^0-9A-F]/g, '');
  if (!clean) return '';
  const bytePairs = clean.match(/.{1,2}/g);
  return bytePairs ? bytePairs.join(' ') : clean;
}
...
function formatSensorReading(sensorType, unitSystem, parsedValues, evalResult) {
  ...
  const formattedHex = formatHexBytesSpaced(dataHex);
  const hexTag = formattedHex ? ` ${formattedHex}` : '';
...
function updateLiveStatus({ portId, siteName, macId, sensorName, tempVal }) {
  ...
  if (siteName && macId) {
    powerTracker.updateHeartbeat(siteName, macId, 'SENSOR');
  }
  if (siteName && portId) {
    powerTracker.updateHeartbeat(siteName, `PORT_${portId}`, 'GATEWAY');
  }
...
function renderTrackedSensorRow(normMac, stateObj) {
  ...
  const stateStr = `🎯 [TRACKED] ${timestamp} | ${padEnd(siteName, 14)} | ${padEnd(sensorName, 22)} | MAC: ${padEnd(macId, 17)} | ${padEnd(readingStr, 32)} | ${statusTag}`;
...
function recordTelemetryEvent({
  ...
  if (siteName && macId) {
    fileStores.recordTelemetry(siteName, macId, parsedValues || {}, evalResult?.isAlert || false);
  }
