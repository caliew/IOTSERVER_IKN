...
decoders.WhatsMateResponse = function (_Gateway, _WhatsAppGroup, _JSONObject) {
  console.log(`  >>[DECODERS.JS] LINE:1101 WHATSMATE RESPONSE..${_Gateway} [${_WhatsAppGroup}]`)
  _JSONObject?.error_message && console.log("  >>[DECODERS.JS] LINE:1102 ", _JSONObject?.error_message);
}
...
decoders.Telegrambot = function (targetChat, message) {
  ...
  console.log(`  >>[DECODERS.JS] [${getDateTimeStamp()}] DECODERS =>${String("TELEGRAMBOT").yellow} [${String(targetChat).red}]=${chatId} ..${String(message).blue}`);
  var jsonPayload = JSON.stringify({
    chat_id: chatId,
    text: message
  });
