...
decoders.decodeWISensorV1 = function (PortID, DataArr, callback) {
  ...
  if (Temp && Humd && modelType && modelID) {
    ...
    if (_MACID == 'BA7579512B0D') console.log(`.. ${getDateTimeStamp()} .. <${_MACID}> ${Temp}C ${Humd}%`)
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          ...
          // console.log('...WHATSMATE ACCOUNT 1 (TELEGRAM) ');
          ...
          console.log('[TelegramGateway] ERROR PARSING JSON:',err1.message)
        }
      });
    });   
  } catch (err) {
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          let object = JSON.parse(chunk);
          // console.log('...WHATSMATE ACCOUNT 2 (WHATSAPP) RESPONSE',object);
          // console.log(`...ID=${object?.id} RESULT=${object?.result}`);        
          _logs.append('_WHATSMATE_WHATSAPP1',`[WHATSMATE ACCOUNT 2] ${getDateTimeStamp()} ${whatsAppGroup} ..${object?.id}/${object?.result}`,()=>{});
        } catch (err1) {
          console.log('[WhatsAppGateway2] ERROR PARSING JSON:',err1.message)
        }
      });
    });   
  } catch (err) {
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          let object = JSON.parse(chunk);
          // console.log('...WHATSMATE ACCOUNT 3 (WHATSAPP) RESPONSE=',object);
