...
decoders.decodeWISensorV2 = function (PortID, Buffer, callback) {
  ...
  // -----------------------------------
  let _DATA = DataArr1.slice(1).join('2C');
  ...
  if (_CMND == 'A1') {
    ...
    _logs.append('_1011',`..DECODE WISENSOR V2 PORT=[${PortID}] COMMAND ID=<${_CMND}>..`,()=>{})
    return;
  }
  //  --------------
  ...
  //  ------------------------
  _logs.append('_1011',`..DECODE WISENSOR V2 PORT=[${PortID}] <${_MACID}>.${_TEMP}.>${_HUMD}?`,()=>{})
  //  ------------
  if (_MACID.length != 12) {
    callback(402,{message:'MAC ID INCOMPLETE'});
    _logs.append('_1011',`..DECIDE WISENSOR V2 PORT=[${PortID}] COMMAND ID=<${_CMND}>..`,()=>{})
    return;
  }
