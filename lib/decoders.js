...
decoders.decodeWISensorV2 = function (PortID, blnDEBUG, BUFFERDATA, callback) {
  ...
  blnDEBUG && console.log(`[${"DECODERS.JS".green}] PORT=<${String(PortID).red}> LINE:268 ..DECODEWISENSORV2.. DATA=<${truncateString(_DATA,20).green}>`);
  ...
  let _BATT = (String(_DATA.substr(38,2)).toUpperCase() == '1B') ? hexToSignedInt(_DATA.substr(40,02)) : -99;    // 1B
  ...
  // ----------------
  if (_TEMP < -50) {
    callback(409,{PORT:PortID,MACID:_MACID,TEMP:_TEMP,HUMD:_HUMD,NOTE:`WI-SENSOR V2 TEMP < -50`});
    return;
  }
  //  ------------
  if (_MACID.length != 12) {
  ...
  decoders.CheckAlert(sensorData,function(errorCode,message) {
    // -------------------------------
    _logs.append('_ALERTCX',`[DECODERS.JS] PORT.ID=[${PortID}] .${getDateTimeStamp()}. <${message?.name}> CHECK ALERT=<${errorCode}> READING=${message?.reading}`,()=>{{}});
    // ---------
    if (typeof fileName === "undefined" || hasNull(fileName)) {
...
decoders.decodeF8L10ST= function(PORTID,buffer,blnDEBUG,callback) {
  ...
  // const _PRE = PRE;
  // const _DATA = POST;
  blnDEBUG && console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).red}> LINE:654 ..DECODEF8L10ST.. PRE=<${String(_PRE).red}> <${String(_DATA.length).yellow}|${truncateString(_DATA,20).green}>`);
  ...
  if (_DATA.substring(0,2) == '80') {
    ...
    decoders.decodeWISensorV2(PORTID,blnDEBUG,_DATA,function (statusCode, payload){
      const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
       blnDEBUG && console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).red}> LINE:675 ..DECODEWISENSORV2.. STATUS CODE=<${String(statusCode).yellow}>`);
  ...
  switch (Number(_CMND)) {
    ...
    case 7:
      _MODE = 'HEART-BEAT'
      _FUNCTID = -1
      if (_DATA.indexOf('03') !== -1) {
        _MODE = 'RS485';
        _SENSORID = 1;
        _DATAHEX = _DATA.substring(_DATA.indexOf('03') + 4, _DATA.indexOf('03') + 4 + parseInt(_DATA.substring(_DATA.indexOf('03') + 2, _DATA.indexOf('03') + 4), 16)*2);
      } else {
        _DATAHEX = _DATA.substr(6,8);
      }
      // _DTUID = null;
  ...
  false && console.log('decodeF8L10ST ',PORTID,' PRE=',_PRE,'DTUID=',ObjSENSOR._DTUID,'DATA=',_DATA,ObjSENSOR._FUNCTID,ObjSENSOR._SENSORID,ObjSENSOR._NDATA,ObjSENSOR._DATAHEX);
  if (_CMND <= 8) {
...
//  -----------------
//  WHATSMATE GATEWAY 
//  [10]. TELEGRAM GATEWAY (TelegramGateway)  - WhatsMateAccount1 - TDK
//  [24]. WHATSAPP GATEWAY (WhatsAppGateway3) - WhatsMateAccount2 - BALAKONG TEA WAREHOUSE, IJN, SNOWCITY
//  [20]. WHATSAPP GATEWAY (WhatsAppGateway2) - WhatsMateAccount3 - SYSTEM INFO, MRE, SHINKO, 
//  [28]. WHATSAPP GATEWAY - TO REPLACE [24] :: 852 6351 0723 WhatsMateAccount4
//  [30]. WHATSAPP GATEWAY (WhatsAppGateway5) - WhatsMateAccount5 - EPSON, 
// --------------------
const MessengingGateway = {
  ...
  "WhatsMateAccount5" : {
    instanceId : "30", // TODO: Replace it with your gateway instance ID here 
    clientId : "affdemo77@gmail.com", // TODO: Replace it with your Forever Green client ID here
    clientSecret : "1344bf2871b141eab5bda0900ca15142",  // TODO: Replace it with your Forever Green client secret here
    gateway : 'WHATSAPP',
    remark : 'NEW EPSON'
  }
}
...
decoders.WhatsAppGateway4 = function(whatsAppGroup,message) {
  // --------------------------
  var jsonPayload = JSON.stringify({
    group_admin: "6597668621", // TODO: Specify the WhatsApp number of the group creator, including the country code
    group_name: whatsAppGroup,   // TODO:  Specify the name of the group
