...
decoders.decode485Sensor = function (PortID, DataArr, callback) {
  ...
  DEBUG_INTOUTPUT && console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .....[1].${hex_to_ascii(DataHEADER).green}|${hex_to_ascii(DataContent).substring(0,15).yellow}...`);
  DEBUG_INTOUTPUT && console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .....[2]..DATA=<${String(DataArr[0]).substring(0,30).green}..>`);
  // ---------------------------
  DEBUG_INTOUTPUT && _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR.. PORT=${PortID} .....[1].${hex_to_ascii(DataHEADER)}|${hex_to_ascii(DataContent)}..`,()=>{});
  DEBUG_INTOUTPUT && _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR.. PORT=${PortID} .....[2]..DATA=<${String(DataArr[0]).substring(0,30)}..>`,()=>{});
  //  ------------------------------
  ...
  if ( DataArr1.length > 0 && DataArr2.length > 0) {
    ...
    try
    {
      ...
      if (FUNCID !== 3)
      {
        // console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[1].${hex_to_ascii(DataHEADER).green}|${hex_to_ascii(DataContent).substring(0,15).yellow}...`);
        console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[2][401]..<${hex_to_ascii(DataHEADER)}|${hex_to_ascii(DataContent)}>.. `);
        ...
        callback(401,{DTUID,SENSORID,FUNCID,RCV_BYTES:[],NDATA:0,DATAS:[]});
        return;
      }
    }
    catch (err){
      console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[1].${hex_to_ascii(DataHEADER).green}|${hex_to_ascii(DataContent).substring(0,15).yellow}...`);
      console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[2][402]..<${DataHEADER}>.. `);
      _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR.. PORT=${PortID} .[1].${hex_to_ascii(DataHEADER)}|${hex_to_ascii(DataContent)}..`,()=>{});
      _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR..PORT=${PortID} [402]..<${DataHEADER}>.. `,()=>{});
      callback(402,{DTUID,SENSORID,FUNCID,RCV_BYTES:[],NDATA:0,DATAS:[]});
      return;
    }
    //  -------------------
    let sensorDataArr = []
    ...
    let RCV_BYTES = [];
    // ----------------
    DEBUG_INTOUTPUT && console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .....[3]..FUNCID=<${String(FUNCID).yellow}>|NDATA=<${String(NDATA).green}>..`);
    DEBUG_INTOUTPUT && _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR.. PORT=${PortID} .....[3]..FUNCID=<${FUNCID}>|NDATA=<${NDATA}>..`,()=>{});
    // ---------------------------
    for (i = 0; i < NDATA; i++) {
    ...
    _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
      ...
      if (!err || err) {
        ...
        _data.read("sensors",fileName,function(err,sensorObj) {
          ...
          //  ------------------------
          decoders.CheckAlert(sensorData,function(errorCode,message) {
            console.log('decode485Sensor',errorCode,message)

          });
          // -----------------------------
          DEBUG_ATCMND && console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[200].<${DataArr.length}>=<${String(DataArr[0]).substring(0,20).red}..>|<${String(DataArr[1]).substring(0,3)}>..FUNCID=[${FUNCID}] ..DTU.ID=[${String(DTUID).padStart(2,"0")}]..SNR.ID=[${String(SENSORID).padStart(2,"0")}].. `);
          DEBUG_ATCMND && _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR.. PORT=${PortID} .[200].<${DataArr.length}>=<${String(DataArr[0]).substring(0,20)}..>|<${String(DataArr[1]).substring(0,3)}>..FUNCID=[${FUNCID}] ..DTU.ID=[${String(DTUID).padStart(2,"0")}]..SNR.ID=[${String(SENSORID).padStart(2,"0")}].. `,()=>{});
          callback(200,sensorData)
          return;
          //  ----------------------------
        })
...
decoders.CheckAlert = function(sensorData,callback) {
  ...
  try {
    ...
    if (sensorIDLabel === "") {
      callback(403,'SENSOR ID LABEL INVALID');
      return;
    } 
