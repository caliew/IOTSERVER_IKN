...
decoders.decodeWISensorV2 = function (PortID, blnDEBUG, DATA, callback) {
  // ----------------
  let _DATA = String(DATA);
  let _PORTIDFile = '_' + PortID;
  false && blnDEBUG && console.log(`[${"DECODERS.JS".green}] PORT=<${String(PortID).red}> ..DECODEWISENSORV2.. DATA=<${String(_DATA).green}>`);
...
decoders.decodeF8L10ST= function(PORTID,buffer,blnDEBUG,callback) {
  ...
  blnDEBUG && console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).red}> ..DECODEF8L10ST.. PRE=<${String(_PRE).red}> <${String(_DATA.length).yellow}|${String(_DATA).green}>`);
  _logs.append(_PORTIDFile,`..${PORTID}..${getDateTimeStamp()} PORT=<${PORTID}> BYTE=<${_PRE}>..<${_DATA}>`,()=>{});
  // ------------------------------
