...
decoders.decodeF8L10ST= function(PORTID,buffer,blnDEBUG,callback) {
  // -------------------------
  const _dateTime = new Date();
  ...
  blnDEBUG && console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).yellow}> ..DECODEF8L10ST.. <${_DATA.length}|${_DATA}>`);
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          ...
          console.log('WHATS GROUP=',whatsAppGroup);
