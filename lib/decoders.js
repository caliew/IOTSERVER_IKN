...
decoders.WhatsMateResponse = function (_Gateway, _WhatsAppGroup, _JSONObject) {
  console.log(`  >>[DECODERS.JS] LINE:1095 WHATSMATE RESPONSE..${_Gateway} [${_WhatsAppGroup}]`)
  _JSONObject?.error_message && console.log("  >>[DECODERS.JS] LINE:1096 ", _JSONObject?.error_message);
  _JSONObject?.id && console.log(`  >>[DECODERS.JS] LINE:1097 ID=${_JSONObject?.id} RESULT=${_JSONObject?.result}`);
}
decoders.TelegramGateway10 = function (whatsAppGroup, message) {
  // --------------------------
  console.log(`  >>[DECODERS.JS] LINE:1101 [${getDateTimeStamp()}] DECODERS =>${String("TELEGRAMGATEWAY10").yellow} [${whatsAppGroup.red}] ..${message.blue}`)
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          ...
          _logs.append('_WHATSMATE_TELEGRAM12', `[WHATSMATE ACCOUNT 2] ${getDateTimeStamp()} ${whatsAppGroup} ..${object?.id}/${object?.result}`, () => { });
        } catch (err) {
...
decoders.Telegrambot = function (targetChat, message) {
  // --------------------------
  console.log(`  >>[DECODERS.JS] [${getDateTimeStamp()}] DECODERS =>${String("TELEGRAMBOT").yellow} [${String(targetChat).red}] ..${String(message).blue}`);
  var jsonPayload = JSON.stringify({
    chat_id: targetChat,
    text: message
  });
    ...
    var request = https.request(options, function (response) {
      ...
      response.on('end', function () {
        try {
          ...
          console.log(`  >>[DECODERS.JS] TELEGRAMBOT RESPONSE ok=${JSONObject?.ok} result=${JSONObject?.result?.message_id || JSONObject?.description}`);
          _logs.append('_TELEGRAM_BOT', `[TELEGRAM BOT] ${getDateTimeStamp()} ${targetChat} ..ok:${JSONObject?.ok}/${JSONObject?.description || JSONObject?.result?.message_id}`, () => { });
        } catch (err) {
...
decoders.WhatsAppGateway1 = function (whatsAppGroup, message) {
  // --------------------------
  console.log(`  >>[DECODERS.JS] LINE:1187 DECODER =>${String("WHATSAPPGATEWAY1").yellow} [${whatsAppGroup.red}] ..${message.blue}`)
  var jsonPayload = JSON.stringify({
    group_admin: "6597668621", // TODO: Specify the WhatsApp number of the group creator, including the country code
    group_name: whatsAppGroup,   // TODO:  Specify the name of the group
    message: message  // TODO: Specify the content of your message
...
decoders.WhatsAppGateway2 = function (whatsAppGroup, message) {
  // --------------------------
  console.log(`  >>[DECODERS.JS] LINE:1232 DECODERS =>${String("WHATSAPPGATEWAY2").yellow} [${whatsAppGroup.red}] ..${message.blue}`);
  var jsonPayload = JSON.stringify({
    group_admin: "6597668621", // TODO: Specify the WhatsApp number of the group creator, including the country code
    group_name: whatsAppGroup,   // TODO:  Specify the name of the group
    message: message  // TODO: Specify the content of your message
...
decoders.WhatsAppGateway30 = function (whatsAppGroup, message) {
  // --------------------------
  _DEBUG && console.log(`  >>[DECODERS.JS] LINE:1320 [${getDateTimeStamp()}] DECODERS =>${String("WHATSAPPGATEWAY30").yellow} [${whatsAppGroup.red}] ..${message.blue}`)
  var jsonPayload = JSON.stringify({
    group_admin: "6597668621", // TODO: Specify the WhatsApp number of the group creator, including the country code
    group_name: whatsAppGroup,   // TODO:  Specify the name of the group
    message: message  // TODO: Specify the content of your message
