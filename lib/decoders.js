...
decoders.decode485Sensor = function (PortID, DataArr, callback) {
  ...
  if (DataArr1.length > 0 && DataArr2.length > 0) {
    ...
    // -------------
    // GET THE FLOAT & INT (UNIVERSAL MODBUS 16-BIT & 32-BIT)
    // -------------
    for (let i = 0; i < RCV_BYTES.length; i++) {
      let _Int16 = hexToSignedInt(RCV_BYTES[i]);
      sensorDataIntArr.push(_Int16);
    }

    if (RCV_BYTES.length % 2 === 0) {
      for (let i = 0; i < RCV_BYTES.length / 2; i++) {
        const hexDirect = `${RCV_BYTES[2 * i]}${RCV_BYTES[2 * i + 1]}`;
        const hexSwapped = `${RCV_BYTES[2 * i + 1]}${RCV_BYTES[2 * i]}`;

        const floatDirect = parseFloat(`0x${hexDirect}`);
        const floatSwapped = parseFloat(`0x${hexSwapped}`);

        let bestFloat = floatSwapped;
        if (isNaN(bestFloat) || !isFinite(bestFloat) || Math.abs(bestFloat) < 1e-10 || Math.abs(bestFloat) > 1e10) {
          bestFloat = floatDirect;
        }

        if (typeof bestFloat === 'number' && !isNaN(bestFloat) && isFinite(bestFloat)) {
          sensorDataFloatArr.push(Number(bestFloat.toFixed(4)));
        } else {
          sensorDataFloatArr.push(hexToSignedInt(hexSwapped));
        }
      }
    } else {
      for (let i = 0; i < RCV_BYTES.length; i++) {
        sensorDataFloatArr.push(hexToSignedInt(RCV_BYTES[i]));
      }
    }
    // -------------
    var sensorData = {
...
decoders.decodeF8L10ST_APIMODE = function (PORTID, buffer, blnDEBUG, callback) {
  ...
  let _BATT = null;
  let _HB_DATAHEX = null;   // embedded RS485 bytes in HEART-BEAT (diagnostic only)
  let _HB_SENSORID = null;
  ...
  switch (Number(_CMND)) {
    ...
    case 6:
      ...
      break;
    case 7:
      // ─── HEART-BEAT mode (cmd=7) ─────────────────────────────────────────
      // The DTU piggybacks the last RS485 reading inside the heartbeat packet.
      // We MUST keep _MODE='HEART-BEAT' always — these bytes are NOT a fresh
      // Modbus poll response and must NOT be sent through alert evaluation.
      // Embedded bytes are extracted to _HB_DATAHEX for diagnostics only.
      // ─────────────────────────────────────────────────────────────────────
      _MODE = 'HEART-BEAT';
      _FUNCTID = -1;
      _BATT = parseInt(_DATA.substr(6, 2), 16);
      _DATAHEX = _DATA.substr(6, 8); // battery/status bytes

      let idx03 = -1;
      let _HB_DATAHEX = null;
      let _HB_SENSORID = null;
      ...
      if (idx03 !== -1) {
        // Embedded RS485 bytes found — store for diagnostics, do NOT evaluate as RS485
        const byteCount = parseInt(_DATA.substring(idx03 + 2, idx03 + 4), 16);
        _HB_DATAHEX = _DATA.substring(idx03 + 4, idx03 + 4 + byteCount * 2);
        _HB_SENSORID = 1;
      }
      blnDEBUG && console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).red}> LINE:683 ..DECODEF8L10ST.. [HEART-BEAT] BATT=${String(_BATT).yellow} HB_DATA=${String(_HB_DATAHEX ?? 'none').red}`);
      // _DTUID = null;
      // _SENSORID = null;
      break;
    case 8:
  ...
  const ObjSENSOR = {
    ...
    ...(_BATT != null ? { _BATT } : {}),
    ...(_HB_DATAHEX != null ? { _HB_DATAHEX, _HB_SENSORID } : {})
  }
...
decoders.decodeF8L10ST_ATMODE = function (PORTID, buffer, blnDEBUG, callback) {
  ...
  switch (Number(_CMND)) {
    ...
    case 7:
      // ─── HEART-BEAT mode (cmd=7) ─────────────────────────────────────────
      // Same as APIMODE: embedded RS485 bytes inside heartbeat are diagnostic
      // only. _MODE stays 'HEART-BEAT' — never sent to alert evaluation.
      // ─────────────────────────────────────────────────────────────────────
      _MODE = 'HEART-BEAT'
      _FUNCTID = -1
      _DATAHEX = _DATA.substr(6, 8); // battery/status bytes
      ...
      {
        let _hbIdx03 = -1;
        let _HB_DATAHEX_AT = null;
        let _HB_SENSORID_AT = null;
        for (let pos = 8; pos <= _DATA.length - 6; pos += 2) {
          if (_DATA.substring(pos, pos + 2) === '03') {
            const candidateByteCount = parseInt(_DATA.substring(pos + 2, pos + 4), 16);
            if (candidateByteCount > 0 && candidateByteCount <= 64 && pos + 4 + candidateByteCount * 2 <= _DATA.length) {
        ...
        if (_hbIdx03 !== -1) {
          const byteCount = parseInt(_DATA.substring(_hbIdx03 + 2, _hbIdx03 + 4), 16);
          _HB_DATAHEX_AT = _DATA.substring(_hbIdx03 + 4, _hbIdx03 + 4 + byteCount * 2);
          _HB_SENSORID_AT = 1;
        }
        if (_HB_DATAHEX_AT) { _HB_DATAHEX = _HB_DATAHEX_AT; _HB_SENSORID = _HB_SENSORID_AT; }
      }
      // _DTUID = null;
      // _SENSORID = null;
      break;
  ...
  const ObjSENSOR = {
    _DATE, _TIME, PORTID, _DTUID, _SENSORID, _FUNCTID, _NDATA, _DATA, _MODE, _PORT, _TYPE, _DATAHEX,
    ...(_BATT != null ? { _BATT } : {}),
    ...(_HB_DATAHEX != null ? { _HB_DATAHEX, _HB_SENSORID } : {}),
    _READING1, _READING2
  }
