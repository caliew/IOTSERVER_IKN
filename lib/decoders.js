      ...
      if (!err || err) {
        ...
        _data.read("sensors",fileName,function(err,sensorObj) {
          ...
          if (sensorData.SENSORTYPE === null) {
              ...
              callback(4037,`FILENAME=${fileName}  ... SENSOR.TYPE=${sensorData.SENSORTYPE}`);
            return;
          }
          // --------
          _logs.append(fileName, logString, function (err) {
...
decoders.decodeF8L10ST= function(PortID,buffer,callback) {
  ...
  let _DTUID = parseInt(_PRE.substr(8,2),16);
  ...
  // -------------------------------
  console.log(`[DECODEF8L10ST]..CHECK...DTU=<${_DTUID}> DATA=${_DATA}... CMND=${_CMND} ..MODE=${_MODE}  FUNCT=${_FUNCTID}`)
  if (_CMND <= 8) {
      ...
      if (sensors.length > 0 && sensors[0].type=='WISENSOR') {
        ...
        if (_TEMP < 0) {
          ...
          if ( !sensorIDLabel.includes('BA-75-79-51-2B-0D')  && !sensorIDLabel.includes('B0-F6-BF-85-9E-13')) {
            ...
            callback(406,{"statusCode":406,name,sensorIDLabel,reading,limitMIN,limitMAX});
            return;
          }
        }
