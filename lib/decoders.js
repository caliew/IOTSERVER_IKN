...
decoders.decode485Sensor = function (PortID, DataArr, callback) {
  ...
  if ( DataArr1.length > 0 && DataArr2.length > 0) {
    ...
    // --------
    let Keys = Object.keys(decoders.sensors);
    if ( decoders.sensors.hasOwnProperty(_KEY)) delete decoders.sensors[_KEY];
...
//  WHATSMATE ACCOUNT 2 - BALAKONG TEA WAREHOUSE,SHINKO
  ...
  AlertGroup.find({}).exec((error,alertgroups) => {
    ...
    alertgroups.forEach( group => {
      ...
      if ( nIndex > -1) {
        ...
        console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}..ALERT GROUP <${group.name}|${str}> ..`);
        var jsonPayload = JSON.stringify(_AlertObject);
        // -----
