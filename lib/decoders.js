...
decoders.decodeWISensorV1 = function (PortID, DataArr, callback) {
  ...
  if (Temp && Humd && modelType && modelID) {
    ...
    let _MACID = modelID.replace(/-/g,'').toUpperCase();
    ...
    decoders.CheckAlert(sensorData,function(errorCode,message) {
      if (errorCode == 200) {
        ...
        if (_MACID == 'BA7579512B0D') console.log(`.. ${getDateTimeStamp()} .. ERROR CODE <${errorCode}>`)
...
decoders.decodeWISensorV2 = function (PortID, Buffer, callback) {
  ...
  // ------------
  if (_TEMP < -50) {
    callback(409,{modelID:_MACID,message:`FAULTY DATA TEMP < -50 MACID<${_MACID}>`});
    return;
  }
  //  ------------
  if (_MACID.length != 12) {
  ...
  decoders.CheckAlert(sensorData,function(errorCode,message) {
    // -------------------------------
    _logs.append('_ALERTCX',`[DECODERS.JS] PORT.ID=[${PortID}] <${hours}:${minutes}> <${message?.name}> CHECK ALERT=<${errorCode}> READING=${message?.reading}`,()=>{{}});
    // ---------
    if (typeof fileName === "undefined" || hasNull(fileName)) {
...
decoders.decode485Sensor = function (PortID, DataArr, callback) {
  ...
  if ( DataArr1.length > 0 && DataArr2.length > 0) {
    ...
    try
    {
      ...
      if (FUNCID !== 3)
      {
        // console.log(`[${'DECODERS.JS'.green}] <${String(PortID).yellow}> ..${'DECODE485SENSOR'.red}.. [${FUNCID}][401] .<${hex_to_ascii(DataHEADER)}|${hex_to_ascii(DataContent)}>.. `);
    ...
    if (CHECKOBJS.length > 0) {
      ...
      console.log(`[${'DECODERS.JS'.green}] <${String(PortID).yellow}> ..${'DECODE485SENSOR'.red}.. decoders.GET_ATCMND=${String(CHECKOBJS.length).red} ..FILENAME=${String(fileName).green} `);
    ...
    _data.read("sensors",fileName,function(err,sensorObj) {
      ...
      decoders.CheckAlert(sensorData,function(errorCode,message) {
        // ------------------
        console.log(`[${'DECODERS.JS'.green}] <${getDateTimeStamp()}> <${String(PortID).green}> ${'DECODE485SENSOR'.blue} CheckAlert=[${String(errorCode).red}] <${String(fileName).toUpperCase().green}>`);
        if (errorCode !== 200) {
...
decoders.decodeF8L10ST= function(PORTID,buffer,callback) {
  ...
  // console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).yellow}> ..DECODEF8L10ST.. <${_DATA}>`);
  _logs.append('_DECODEF8L10ST',`..${PORTID}..${_DATE} ${_TIME} PORT=<${PORTID}> BYTE=<${_PRE}>..<${_DATA}>`,()=>{});
  // ------------------------------
  ...
  if (_DATA.substring(0,2) == '80') {
    ...
    // -------------
    // console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).yellow}> ..DECODE WISENSOR V2..`);
    decoders.decodeWISensorV2(PORTID,_DATA,function (statusCode, payload){
      ...
      // console.log(`[${"DECODERS.JS".green}] PORT=<${String(PORTID).yellow}> ..DECODE WISENSOR V2.. STATUS CODE=<${statusCode}>`);
      _logs.append('_DECODEF8L10ST',`..${PORTID}..${_DATE}/${_TIME} PORT=<${PORTID}> AP MODE .. BYTE=<${_PRE}>..<${_DATA}>`,()=>{});
      _logs.append('_DECODEF8L10ST',`..${PORTID}..${_DATE}/${_TIME} PORT=<${PORTID}> WISENSOR=<${modelType}|${modelID}> <${Temperature}/${Humidity}/${BATT}/${INTERVAL}>`,()=>{});
      // --------
      if (statusCode == 200) {
  ...
  // -------------------------------
  // _logs.append('_DECODEF8L10ST',`..${PORTID}..${_DATE} ${_TIME} PORT=<${PORTID}> DTU.ID=<${_DTUID}> <${_DATA.substr(0,2)}> BYTE=<${_PRE}>..<${_DATA}>`,()=>{});
  // ----------
  false && _data.read('teawarehouse','settings',function(err,settingData) {
    if (settingData.hasOwnProperty(_DTUID) && PORTID == 1009) {
      ...
      if (ObjSensor.MODE === 'RS485') {
        ...
        if (_TEMP > _limits.Temperature.max)  _FLAG[0] = 1;
        if (_TEMP < _limits.Temperature.min)  _FLAG[1] = 1;
        if (_RH > _limits.Humidity.max)       _FLAG[2] = 1;
        if (_RH < _limits.Humidity.min)       _FLAG[3] = 1;
        if (_CO2 > _limits.CO2.max)           _FLAG[4] = 1;
        if (_CO2 < _limits.CO2.min)           _FLAG[5] = 1;
        // ----------
  ...
  if (_CMND <= 8) {
    callback(407,{ _DATE, _TIME, PORTID, _DTUID, _SENSORID, _FUNCTID, _NDATA, _DATA, _MODE, _PORT, _TYPE, _DATAHEX, _READING1, _READING2 });
  } else {
...
decoders.decodeF8L10ST_ATMODE= function(PORTID,buffer,callback) {
  ...
  switch (Number(_CMND)) {
    ...
    case 7:
      ...
      console.log('..decodeF8L10ST_ATMODE.. HEART-BEAT..',PORTID,Number(_CMND))
      break;
    case 8:
      _MODE = 'POWER-OFF'
      break;
  ...
  // -------------------------------
  if (Number(_CMND) > 0 ) {
    console.log('..decodeF8L10ST_ATMODE.. HEART-BEAT..',PORTID,Number(_CMND),_DTUID);
...
decoders.CheckAlert = function(sensorData,callback) {
  ...
  try {
    ...
    // let _MACID = sensorData?.modelID?.replace(/-/g,'').toUpperCase();
    // -----------------------
    if (sensorIDLabel === "") {
    ...
    Sensor.find(query).exec( (error,sensors) => {
      ...
      if (sensors?.length > 0 && sensors[0]?.type==='WISENSOR') {
        ...
        // COLD ROOM PROBE ADJUSTMENT : B0-BC-82-C4-C4-41
        // INTERMITTEN FLAG :BA-B8-04-3C-BA-6A :BA-18-63-D6-C8-D0
        let SENSOR_COLDROOM = ['BA-75-79-51-2B-0D','B0-F6-BF-85-9E-13','BA-B8-04-3C-BA-6A' ];
        let SENSOR_ADJ = ['B0-BC-82-C4-C4-41'];
        let SENSOR_SNOWCITY = ['B0-36-98-DE-39-01','B0-64-C8-17-0D-9A','B0-1E-E8-DE-54-78','B0-03-CD-1D-23-02','B0-B4-1A-17-9E-33','B0-7E-29-E2-71-A1' ];
        ...
        if (_TEMP < 0.05) {
          ...
          if ( !SENSOR_COLDROOM.includes(sensorIDLabel)  && !SENSOR_SNOWCITY.includes(sensorIDLabel)) {
            // ---------
            _logs.append('_ALERTCX',`[DECODERS.JS] ${sensorIDLabel} ${hourNow}:${minuteNow} <${name}> ...TEMP < 0.1 AND SENSOR NOT IN WHITE LISTS...`,()=>{{}});
        ...
        if (_TEMP >= 0.05 && (SENSOR_COLDROOM.includes(sensorIDLabel))) {
          type = 'TEMPERATURE'
          // if (_MACID == 'BA7579512B0D') console.log(`.. ${getDateTimeStamp()} .<${sensorIDLabel}>.TEMP=${_TEMP}.. <${limitTEMPMIN}|${limitTEMPMAX}>C <${limitHUMDMAX}|${limitHUMDMIN}>%`)
        ...
        let errorMessage = { errorCode:409, message:'ERROR IN DECODERS.CHECKALERT'}
        callback(407,{"statusCode":409,errorMessage});
      }
    })
  } catch (err) {
    console.log(`..CHECK ALERT.. CALLBACK 400`);
    console.log(err);
...
//  -----------------
//  WHATSMATE GATEWAY 
//  [10]. TELEGRAM GATEWAY - WhatsMateAccount1 - TDK
//  [24]. WHATSAPP GATEWAY - WhatsMateAccount2 - BALAKONG TEA WAREHOUSE, IJN, SNOWCITY
//  [28]. WHATSAPP GATEWAY - TO REPLACE [24] :: 852 6351 0723
//  [20]. WHATSAPP GATEWAY - WhatsMateAccount3 - SYSTEM INFO, MRE, SHINKO, 
// --------------------
const MessengingGateway = {
  ...
  "WhatsMateAccount2" : {
    instanceId : "28",      // TODO: Replace it with your gateway instance ID here
    instanceId_OLD : "24",  // TODO: Replace it with your gateway instance ID here
  ...
  "WhatsMateAccount4" : {
    ...
    clientSecret : "6a1c9eeb79674e7ba477572b326458b2",  // TODO: Replace it with your Forever Green client secret here
    gateway : 'WHATSAPP',
    remark : 'TO REPLACE CLIENT ID 24 - WhatsMateAccount2'
  }
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          ...
          // console.log(`...ID=${object?.id} RESULT=${object?.result}`);
          _logs.append('_WHATSMATE_TELEGRAM',`[WHATSMATE ACCOUNT 1] ${getDateTimeStamp()} ${whatsAppGroup} ..${object?.id}/${object?.result}`,()=>{});
        } catch (err1) {
          console.log('[GATEWAY1] ERROR PARSING JSON:',err1.message)
        }
      });
    });   
  } catch (err) {
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          let object = JSON.parse(chunk);
          console.log('...GATEWAY1 (WHATSAPP) RESPONSE',object);
          console.log(`...ID=${object?.id} RESULT=${object?.result}`);        
          _logs.append('_WHATSMATE_WHATSAPP1',`[WHATSMATE ACCOUNT 2] ${getDateTimeStamp()} ${whatsAppGroup} ..${object?.id}/${object?.result}`,()=>{});
        } catch (err1) {
          console.log('[GATEWAY2] ERROR PARSING JSON:',err1.message)
        }
      });
    });   
  } catch (err) {
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          let object = JSON.parse(chunk);
          _logs.append('_WHATSMATE_WHATSAPP2',`[WHATSMATE ACCOUNT 3] ${getDateTimeStamp()} ${whatsAppGroup} ..${object?.id}/${object?.result}`,()=>{});
        } catch (err1) {
          console.log('[GATEWAY2] ERROR PARSING JSON:',err1.message);
        }
      });
...
decoders.WhatsAppGateway3 = function(whatsAppGroup,message) {
  // --------------------------
  var jsonPayload = JSON.stringify({
    group_admin: "6597668621", // TODO: Specify the WhatsApp number of the group creator, including the country code
    group_name: whatsAppGroup,   // TODO:  Specify the name of the group
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        try {
          let object = JSON.parse(chunk);
          _logs.append('_WHATSMATE_WHATSAPP2',`[WHATSMATE ACCOUNT 4] ${getDateTimeStamp()} ${whatsAppGroup} ..${object?.id}/${object?.result}`,()=>{});
        } catch (err1) {
          console.log('[GATEWAY3] ERROR PARSING JSON:',err1.message);
        }
      });
    });   
  } catch (err) {
