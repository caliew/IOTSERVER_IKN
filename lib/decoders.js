      ...
      if (sensors.length > 0 && sensors[0].type!=='WISENSOR'){
        ...
        switch (sensorData.SENSORTYPE) {
          case 'AIRFLOW-VEL':
            ...
            console.log(`[${"DECODERS.JS".green}] <${sensorData.SENSORTYPE.yellow}> [${sensorData.DTUID.red}-${String(sensorData.SENSORID).red}] <${String(sensors.length).yellow}> NAME=<${String(name).green}> LABEL=<${String(label).red}>  VALUE=<${String(value).green}> ALERT=${String(AlertFlag).toUpperCase().yellow} ..<${String(sensorData.RCV_BYTES).green}>..`);
            break;
          case 'AIR-PRES':
            label = 'AIR PRESSURE';
          ...
          case 'WATER-TEMP':
            ...
            console.log(`[${"DECODERS.JS".green}] <${sensorData.SENSORTYPE.yellow}> [${sensorData.DTUID.red}-${String(sensorData.SENSORID).red}] <${String(sensors.length).yellow}> NAME=<${String(name).green}> LABEL=<${String(label).red}>  VALUE=<${String(value).green} C> ALERT=${String(AlertFlag).toUpperCase().yellow} ..<${String(sensorData.RCV_BYTES).green}>..`);
            break;
          case 'AIRFLOW-TEMP':
            label = 'AIR TEMPERATURE';
            let _TEMP = (Number(sensorData.DATAS[1])/10).toFixed(2);
            let _RH = (Number(sensorData.DATAS[0])/10).toFixed(2);
            console.log(`[${"DECODERS.JS".green}] <${sensorData.SENSORTYPE.yellow}> [${sensorData.DTUID.red}-${String(sensorData.SENSORID).red}] <${String(sensors.length).yellow}> NAME=<${String(name).green}> LABEL=<${String(label).red}>  VALUE=<${String(_RH).green} %/${String(_TEMP).green} C> ALERT=${String(AlertFlag).toUpperCase().yellow} ..<${String(sensorData.RCV_BYTES).green}>..`);
            break;
