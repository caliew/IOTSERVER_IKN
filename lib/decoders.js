...
const TelegramChatMap = {
  ...
  "MRE IOT Monitoring System": "-4234611374"
};

decoders.Telegrambot = function (targetChat, message) {
  // Resolve group name to Telegram chat_id if mapped, otherwise use targetChat
  var chatId = TelegramChatMap[targetChat] || targetChat;

  // --------------------------
  console.log(`  >>[DECODERS.JS] [${getDateTimeStamp()}] DECODERS =>${String("TELEGRAMBOT").yellow} [${String(targetChat).red} -> ${String(chatId).green}] ..${String(message).blue}`);
  var jsonPayload = JSON.stringify({
    chat_id: chatId,
    text: message
  });
