...
decoders.decodeWISensorV2 = function (PortID, Buffer, callback) {
  ...
  // -----------------------------------
  let _DATA = DataArr.length == 1 ? Buffer : DataArr1.slice(1).join('2C');
  let _HEADER = _DATA.substr(0, 2).toString("UTF-8").toUpperCase();
  let _CMND = _DATA.substr(2, 2).toString("UTF-8").toUpperCase();
  let _LEN = _DATA.substr(4, 4).toString("UTF-8").toUpperCase();
  ...
  // --------------
  if (_CMND == 'A1') {
    callback(402,{message:'COMMAND A1'});
    _logs.append('_1011',`..${hours}:${minutes}:${seconds} WI-SENSOR V2 [${PortID}] <${_CMND}>..WRONG COMMAND.. MAC ID=${_MACID}`,()=>{})
    return;
  }
  //  --------------
  ...
  // -------------
  (PortID == 1012) && _logs.append('_1012',`[${PortID}] BUFFER .0D.<${DataArr.length}> ..${_DATA}..<${_MACID}>=${_TEMP}C..${_HUMD}%`,()=>{})
  if (_TEMP < -50) {
    callback(404,{message:'FAULTY DATA'});
    return;
  }
  //  ------------------------
  if (PortID == 1008) {
    _logs.append('_1011',`..${hours}:${minutes}:${seconds} WI-SENSOR V2 [${PortID}] <${DataArr.length}|${_MACID}> ${_TEMP}C ${_HUMD}%`,()=>{})
  }
  //  ------------
  if (_MACID.length != 12) {
    callback(402,{message:'MAC ID INCOMPLETE'});
    _logs.append('_1011',`..${hours}:${minutes}:${seconds} WI-SENSOR V2 [${PortID}] <${_CMND}>..INCOMPLETE MAC ID.. MAC ID=<${_MACID}>..`,()=>{})
    return;
  }
  // ---------
  var sensorData = {
  ...
  decoders.CheckAlert(sensorData,function(errorCode,message) {
    ...
    _data.update("sensors", fileName, sensorData, function (err) {
      ...
      // ----------------------------
      var logString = JSON.stringify(sensorData);
      if ( flagSCREENOutput_WISENSOR2 && String(PortID) === '1008' ) console.log(`[${"DECODERS.JS".blue}] <${getDateTimeStamp()}> <${String(PortID).green}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      if ( flagSCREENOutput_WISENSOR2 && String(PortID) === '1009' ) console.log(`[${"DECODERS.JS".blue}] <${getDateTimeStamp()}> <${String(PortID).yellow}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      if ( flagSCREENOutput_WISENSOR2 && String(PortID) === '1010' ) console.log(`[${"DECODERS.JS".blue}] <${getDateTimeStamp()}> <${String(PortID).red}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      // --------------
      // Stored in Logs
      // --------------
      _logs.append(fileName, logString, function (err) {
...
decoders.decodeF8L10ST= function(PortID,buffer,callback) {
  ...
  // ---------------------------------------------
  if (_DTUID == 1 && _DATA.substring(0,2) == '80') {
    (PortID == 1012) && _logs.append("_1012",`PORT=<${PortID}> DTU.ID=<${_DTUID}> BYTE=<${_DATA.substring(0,2)}>..<${_DATA}>`,()=>{});
    // ------
    decoders.decodeWISensorV2(PortID,_DATA,function (statusCode, payload){
      ...
      if (statusCode == 200) {
        _logs.append("_1012",`${statusCode}..${modelID}.${Temperature}.${Humidity}.`,()=>{}) 
      } else {
        _logs.append("_1012",`${statusCode}..${payload.message}.`,()=>{}) 
      }
    })
  }
...
//  WHATSMATE GATEWAY 
//  WHATSMATE ACCOUNT 1 - TDK, IKN
//  WHATSMATE ACCOUNT 2 - BELAKONG TEA WAREHOUSE,SHINKO
...
decoders.sendWhatsAppMessage = function(sensor,str,_AlertObject,minutes) {
  ...
  AlertGroup.find({}).exec((error,alertgroups) => {
    ...
    alertgroups.forEach( group => {
      ...
      // -----------------
      if ( nIndex > -1) {
        console.log(`[${"DECODERS.JS".green}] <${getDateTimeStamp()}> ..ALERT .[${group.name}].[${label.red}]. ${str.yellow}`)
        var jsonPayload = JSON.stringify(_AlertObject);
        // -----
        if (group.name === 'TDK Alerts Group') _logs.append('_TDKALERTS',jsonPayload,()=>{});
        if (group.name !== 'TDK Alerts Group') decoders.WhatMateMessageGateway1(group.name,str);
        // -----
        var alertData = {
          "datetime": new Date(),
          "message": str
...
//  WHATS MATE GATEWAY
//  ------------------
decoders.WhatMateMessageGateway1 = function(whatsAppGroup,message) {
  // --------------------------
  console.log(`[${"DECODERS.JS".yellow}] GATEWAY 1 <${whatsAppGroup.green}> ..<${message.yellow}>..`);
  ...
  var whatAppOptions = {
    ...
    port: 80,
    path: "/v3/whatsapp/group/text/message/" + WhatsMateAccount1.instanceId,
    method: "POST",
    headers: {
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        ...
        console.log(`[${'DECODERS.JS'.yellow}] ...${getDateTimeStamp()}...WHATSMATE GATEWAY 1..STATUS CODE=${String(response.statusCode).red}
              RESULT<${String(result).toUpperCase().yellow}>..ID<${String(id).green}>`);
         _logs.append('_WHATSAPP',`[DECODERS.JS] WA GATEWAY 1 : <${getDateTimeStamp()}> STATUS CODE=${response.statusCode} RESULT<${String(result).toUpperCase()}>..ID<${String(id).toUpperCase()}>`,()=>{{}});
      });
    });   
  } catch (err) {
...
decoders.WhatMateMessageGateway2 = function(whatsAppGroup,message) {
  // --------------------------
  console.log(`[${"DECODERS.JS".yellow}] GATEWAY 2 <${whatsAppGroup.green}> ..<${message.yellow}>..`);
  var jsonPayload = JSON.stringify({
    group_admin: "6597668621", // TODO: Specify the WhatsApp number of the group creator, including the country code
    group_name: whatsAppGroup,   // TODO:  Specify the name of the group
    message: message  // TODO: Specify the content of your message
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        let object = JSON.parse(chunk);
        const { id, result } = object;
        console.log(`[${'DECODERS.JS'.yellow}] ...${getDateTimeStamp()}...WHATSMATE GATEWAY 2..STATUS CODE=${String(response.statusCode).red}
              RESULT<${String(result).toUpperCase().yellow}>..ID<${String(id).green}>`);
         _logs.append('_WHATSAPP',`[DECODERS.JS] WA GATEWAY 2 : <${getDateTimeStamp()}> STATUS CODE=${response.statusCode} RESULT<${String(result).toUpperCase()}>..ID<${String(id).toUpperCase()}>`,()=>{{}});
      });
    });   
  } catch (err) {
    console.log(`[${'DECODERS.JS'.yellow}] .. SENDWHATS APP ERROR <${err}>`)
