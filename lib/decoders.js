...
const flagPRINT_WISENSOR1_CONSOLE = false;
const flagPRINT_WISENSOR2_CONSOLE = false;
const flagPRINT_485SENSOR_CONSOLE = true;
...
decoders.decodeWISensorV1 = function (PortID, DataArr, callback) {
  ...
  if (Temp && Humd && modelType && modelID) {
    ...
    _data.update("sensors", fileName, sensorData, function (err) {
      ...
      if ( flagPRINT_WISENSOR1_CONSOLE && String(PortID) === '1008' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).green}> ${'WI-SENSOR1'.red} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      if ( flagPRINT_WISENSOR1_CONSOLE && String(PortID) === '1009' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).yellow}> ${'WI-SENSOR1'.red} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      if ( flagPRINT_WISENSOR1_CONSOLE && String(PortID) === '1010' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).red}> ${'WI-SENSOR1'.red} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      // --------------
      // Stored in Logs
      // --------------
...
decoders.decodeWISensorV2 = function (PortID, Buffer, callback) {
  ...
  _data.update("sensors", fileName, sensorData, function (err) {
    ...
    if (err) { 
      _data.create("sensors", fileName, sensorData, function (err) {
        if (!err) {
          ...
          console.log(sensorData);
          _logs.append('_ERROR',`[DECODERS.JS] ..decodeWISensorV2.. NEW SENSOR CREATED =${String(fileName)} `,()=>{});
        } else {
    ...
    let _now = new Date();
    let _TIMESTAMP = `${_now.getDate()}/${_now.getMonth()+1}/${_now.getFullYear()} ${_now.getHours()}:${_now.getMinutes()}`
    if ( flagPRINT_WISENSOR2_CONSOLE && String(PortID) === '1008' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).green}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
    if ( flagPRINT_WISENSOR2_CONSOLE && String(PortID) === '1009' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).yellow}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
    if ( flagPRINT_WISENSOR2_CONSOLE && String(PortID) === '1010' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).red}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
    // --------------
    // Stored in Logs
    // --------------
    _logs.append(fileName, logString, function (err) {
...
decoders.decode485Sensor = function (PortID, DataArr, callback) {
  ...
  // -----------
  console.log(`[${'DECODERS.JS'.green}] ${'DECODE485SENSOR'.blue} PORT=${String(PortID).green} ${DataArr}`);
  flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <1> ENTER`,()=>{});
  // -------------------
  ...
  // ------------------------------
  flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <2> GET DATA`,()=>{});
  // -------------------------------
  ...
  if ( DataArr1.length > 0 && DataArr2.length > 0) {
    // ---------------------------------------
    flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <3> VERIFY DATA <${DataArr1.length}|${DataArr2.length}>`,()=>{});
    ...
    try
    {
      ...
      if (FUNCID !== 3)
      {
        flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] DECODE485SENSOR [403] PORT=<${String(PortID)}> ${DataArr}`,()=>{});
    ...
    // ----------------------
    flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <4> SENSOR DATA OBJECT`,()=>{});
    // ---------------------------
    ...
    if(typeof fileName === "undefined" || hasNull(fileName)){
      ...
      flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] DECODE485SENSOR [403] PORT=${PortID} ..<${DataArr.length}>=<${DataArr[0]}>|<${DataArr[1]}>..FUNCID=[${FUNCID}] ..DTU.ID=[${String(DTUID).padStart(2,"0")}]..SNR.ID=[${String(SENSORID).padStart(2,"0")}].. `,()=>{});
      callback(403,{DTUID,SENSORID,FUNCID,RCV_BYTES:[],NDATA:0,DATAS:[]});
      return;
    }
    // -----------------------------------------------------
    ...
    // -----------------------------------------------------
    flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <5> FILENAME=${fileName}`,()=>{});
    // -------------
    _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
      // --------
      flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <6> DATA READ FROM RAWDATA <${String(err).toUpperCase()}>`,()=>{});
      // --------
      if (!err) {
        // -------------------
        flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <7> CMD_SENT <${CMD_SENT}>`,()=>{});
        // -------------------
        if (typeof CMD_SENT === 'object') {
          // ----------------
          flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] .. <8> CHECKOBJS=<${CHECKOBJS}>`,()=>{});          
          // ----------------
          ...
          flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] .. <9> APPEND TO THE TEST FILE =<${_KEY}>`,()=>{});          
          // ----------------------
          ...
          _data.update('rawData','_485SENSORS',decoders.sensors,function(err) {
            // -----
            flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <7> DATA UPDATE _485SENSORS ${err}`,()=>{});
            // -----
          } );
          // ------
        }
        // ---------------------------------------
        flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <8> DATA READ SENSORS ${fileName}`,()=>{});
        // ---------------------------------------
        _data.read("sensors",fileName,function(err,sensorObj) {
          ...
          _data.update("sensors", fileName, sensorData, function (err) {
            // -------------------
            flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <9> UPDATE READ SENSORS ${fileName}`,()=>{});
            // ------------------            
            if (err && fileName) { 
              // -------------
              ...
              _data.create("sensors", fileName, sensorData, function (err) {
                // --------
                flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <9> DATA CREATE SENSORS ${fileName}`,()=>{});
          ...
          let _now = new Date();
          let _TIMESTAMP = `${_now.getDate()}/${_now.getMonth()+1}/${_now.getFullYear()} ${_now.getHours()}:${_now.getMinutes()}`
          if ( flagPRINT_485SENSOR_CONSOLE && String(PortID) === '1008' ) console.log(`[${'DECODERS.JS'.green}] <${_TIMESTAMP}> <${String(PortID).green}> ${'DECODE485SENSOR'.blue} APPEND LOG <${String(fileName).toUpperCase().green}>`);
          if ( flagPRINT_485SENSOR_CONSOLE && String(PortID) === '1009' ) console.log(`[${'DECODERS.JS'.green}] <${_TIMESTAMP}> <${String(PortID).yellow}> ${'DECODE485SENSOR'.blue} APPEND LOG <${String(fileName).toUpperCase().green}>`);
          if ( flagPRINT_485SENSOR_CONSOLE && String(PortID) === '1010' ) console.log(`[${'DECODERS.JS'.green}] <${_TIMESTAMP}> <${String(PortID).red}> ${'DECODE485SENSOR'.blue} APPEND LOG <${String(fileName).toUpperCase().green}>`);
          // --------------
          // Stored in Logs
          // --------------
          _logs.append(fileName, logString, function (err) {
            // ---------
            flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <10> LOGS FILE APPEND=${fileName}`,()=>{});
            // ----------
            if (!err) {
              debug("Logging to file succeeded");
            } else {
          ...
          decoders.CheckAlert(sensorData);
          // -----------------------------
          flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..PORT=${String(PortID)} <11> CALLBACK`,()=>{});
          flagPRINTERRORLOG && _logs.append('_ERROR',`[DECODERS.JS] ..decode485Sensor[200]..PORT[${PortID}] ....DTU.ID=[${String(DTUID).padStart(3,"0")}]..SNR.ID=[${String(SENSORID).padStart(3,"0")}].. <${DataArr.length}>=<${String(DataArr[0]).substring(0,15)}..>|<${String(DataArr[1]).substring(0,2)}>..FUNCID=[${FUNCID}]`,()=>{});
          callback(200,sensorData)
          return;
          //  ----------------------------
        })
...
decoders.CheckAlert = function(sensorData,callback) {
  ...
  try {
    ...
    Sensor.find(query).exec( (error,sensors) => {
      // ------------
      if (sensors && sensors.length >0 && sensors[0].type=='WISENSOR') {
        // -----------------------------
