...
decoders.decodeWISensorV1 = function (PortID, DataArr, callback) {
  ...
  if (Temp && Humd && modelType && modelID) {
    ...
    _data.update("sensors", fileName, sensorData, function (err) {
      ...
      if ( String(PortID) === '1008' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).green}> ${'WI-SENSOR1'.red} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      if ( String(PortID) === '1009' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).yellow}> ${'WI-SENSOR1'.red} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      if ( String(PortID) === '1010' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).red}> ${'WI-SENSOR1'.red} APPEND LOG <${String(fileName).toUpperCase().green}>`);
      // --------------
      // Stored in Logs
      // --------------
...
decoders.decodeWISensorV2 = function (PortID, Buffer, callback) {
  ...
  _data.update("sensors", fileName, sensorData, function (err) {
    ...
    let _now = new Date();
    let _TIMESTAMP = `${_now.getDate()}/${_now.getMonth()+1}/${_now.getFullYear()} ${_now.getHours()}:${_now.getMinutes()}`
    if ( String(PortID) === '1008' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).green}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
    if ( String(PortID) === '1009' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).yellow}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
    if ( String(PortID) === '1010' ) console.log(`[${"DECODERS.JS".blue}] <${_TIMESTAMP}> <${String(PortID).red}> ${'WI-SENSOR2'.yellow} APPEND LOG <${String(fileName).toUpperCase().green}>`);
    // --------------
    // Stored in Logs
    // --------------
    _logs.append(fileName, logString, function (err) {
    ...
    _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
      ...
      if (!err) {
        ...
        _data.read("sensors",fileName,function(err,sensorObj) {
          ...
          let _now = new Date();
          let _TIMESTAMP = `${_now.getDate()}/${_now.getMonth()+1}/${_now.getFullYear()} ${_now.getHours()}:${_now.getMinutes()}`
          if ( String(PortID) === '1008' ) console.log(`[${'DECODERS.JS'.green}] <${_TIMESTAMP}> <${String(PortID).green}> ${'DECODE485SENSOR'.blue} APPEND LOG <${String(fileName).toUpperCase().green}>`);
          if ( String(PortID) === '1009' ) console.log(`[${'DECODERS.JS'.green}] <${_TIMESTAMP}> <${String(PortID).yellow}> ${'DECODE485SENSOR'.blue} APPEND LOG <${String(fileName).toUpperCase().green}>`);
          if ( String(PortID) === '1010' ) console.log(`[${'DECODERS.JS'.green}] <${_TIMESTAMP}> <${String(PortID).red}> ${'DECODE485SENSOR'.blue} APPEND LOG <${String(fileName).toUpperCase().green}>`);
          // --------------
          // Stored in Logs
          // --------------
          _logs.append(fileName, logString, function (err) {
...
decoders.decodeF8L10ST= function(PortID,buffer,callback) {
  ...
  let _HEADER = buffer.substr(0,2);
  let _LENGTH = parseInt(buffer.substr(2,2),16);
  let _CMND = buffer.substr(4,2);
  let _DATA = buffer.substring(6,(_LENGTH-1)*2).toUpperCase();
  let _DATALENGTH = parseInt(_DATA.substr(4,2),16);
  // let _CMND = _DATA.substr(4,2)
  let _IOPORT = -1;
  let _IOTYPE = -1;
  let _DATAHEX = _DATA.substr(6,_DATALENGTH*2)
  ...
  let _READING = Number(parseFloat(`0x${_DATAHEX}`)).toFixed(3);
  ...
  // ----------
  if (_CMND > 2) {
    _IOPORT = _DATA.substr(6,2);
    _IOTYPE = _DATA.substr(8,2)
    // ----------
    switch(_IOTYPE) {
      case '01':
        _TYPE = 'ANALOG INPUT'
  ...
  // -------------------------------
  if (_CMND > 0 ) {
    callback(400,{ _DATE, _TIME, PortID, _DTUID, _SENSORID, _FUNCTID,  _DATA, _MODE, _PORT, _TYPE, _DATAHEX, _DATAINT, _READING });
  } else {
    callback(403, null);
  }
  // ----------
...
decoders.decodeF8L10ST_ATMODE= function(PortID,buffer,callback) {
  ...
  let _DATAINT = parseInt(_DATAHEX,16);
  let _READING = `${Number(_DATAINT * 3.3 * 1000 / (4095.0 * 150.0) / 20.0 * 100.0 / 20.0 ).toFixed(2)} A`;
  // FA 06 04 0B 01 01 DB F2
  ...
  // -------------------------------
  if (Number(_CMND) > 0 ) {
    callback(400,{ _DATE, _TIME, _DTUID, _DATA, _MODE, _PORT, _TYPE, _DATAHEX, _DATAINT, _READING });
  } else {
    callback(403, null);
  }
  // ----------
