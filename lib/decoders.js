    ...
    if (settingData.hasOwnProperty(_DTUID)) {
      ...
      if (ObjSensor.MODE === 'RS485') {
        ...
        if (_TEMP > _limits.Temperature.max)  {
          _FLAG[0] = 1;
          // decoders.WhatMateMessage(WhatsAppGroup,`${_NAME} 温度=${_TEMP}C > ${_limits.Temperature.max}C`);
        }
        if (_TEMP < _limits.Temperature.min)  {
          _FLAG[1] = 1;
          // decoders.WhatMateMessage(WhatsAppGroup,`${_NAME} 温度=${_TEMP}C < ${_limits.Temperature.min}C`);
        }
        if (_RH > _limits.Humidity.max)       {
          _FLAG[2] = 1;
          // decoders.WhatMateMessage(WhatsAppGroup,`${_NAME} 湿度=${_RH}% > ${_limits.Humidity.max}%`);
        }
        if (_RH < _limits.Humidity.min)       {
          _FLAG[3] = 1;
          // decoders.WhatMateMessage(WhatsAppGroup,`${_NAME} 湿度=${_RH}% < ${_limits.Humidity.min}%`);
        }
        if (_CO2 > _limits.CO2.max)           {
          _FLAG[4] = 1;
          // decoders.WhatMateMessage(WhatsAppGroup,`${_NAME} 二氧化碳=${_CO2} > ${_limits.CO2.max}`);
        }
        if (_CO2 < _limits.CO2.min)           {
          _FLAG[5] = 1;
          // decoders.WhatMateMessage(WhatsAppGroup,`${_NAME} 二氧化碳=${_CO2} < ${_limits.CO2.min}`);
        }
        // ----------
      ...
      if (sensors.length > 0 && sensors[0].type==='WISENSOR') {
        ...
        if (_TEMP > limitTEMPMAX) {
          try {
            ...
            newAlert.save();
            // -------------
            let _AlertObject = { TIMESTAMP:new Date(), TYPE:type, sensorType, sign, DTU:dtuId, SENSORID:sensorId, NAME:name, VALUE:_TEMP, LIMITS: limitTEMPMAX }
            var jsonPayload = JSON.stringify(_AlertObject);
            _logs.append('_TDKALERTS',jsonPayload,()=>{});
            // -------------
            decoders.sendWhatsApp(sensors[0],`${sensors[0].name} TEMPERATURE=${_TEMP}C > LIMIT ${limitTEMPMAX}C`,alertPoints);
        ...
        if (_TEMP < limitTEMPMIN) {
          try {
            ...
            newAlert.save();
            // -------------
            let _AlertObject = { TIMESTAMP:new Date(), TYPE:type, sensorType, sign, DTU:dtuId, SENSORID:sensorId, NAME:name, VALUE:_TEMP, LIMITS: limitTEMPMIN }
            var jsonPayload = JSON.stringify(_AlertObject);
            _logs.append('_TDKALERTS',jsonPayload,()=>{});
            // -------------
            decoders.sendWhatsApp(sensors[0],`${sensors[0].name} TEMPERATURE=${_TEMP}C < LIMIT ${limitTEMPMIN}C`,alertPoints)
            callback(200,newAlert);
            return;
            // -------------
          } catch (err) {
        ...
        if (_HUMD > 0 ) {
          ...
          if (_HUMD > limitHUMDMAX) {
            try {
              ...
              newAlert.save();
              // -------------
              let _AlertObject = { TIMESTAMP:new Date(), TYPE:type, sensorType, sign, DTU:dtuId, SENSORID:sensorId, NAME:name, VALUE:_HUMD, LIMITS: limitHUMDMAX }
              var jsonPayload = JSON.stringify(_AlertObject);
              _logs.append('_TDKALERTS',jsonPayload,()=>{});
              // -------------
              decoders.sendWhatsApp(sensors[0],`${sensors[0].name} HUMIDITY=${_HUMD}% > LIMIT ${limitHUMDMAX}%`,alertPoints);
              callback(200,newAlert)
              return;
              // -------------
            } catch (err) {
          ...
          if (_HUMD < limitHUMDMIN) {
            try {
              ...
              newAlert.save();
              // -------------
              let _AlertObject = { TIMESTAMP:new Date(), TYPE:type, sensorType, sign, DTU:dtuId, SENSORID:sensorId, NAME:name, VALUE:_HUMD, LIMITS: limitHUMDMIN }
              var jsonPayload = JSON.stringify(_AlertObject);
              _logs.append('_TDKALERTS',jsonPayload,()=>{});
              // -------------
              decoders.sendWhatsApp(sensors[0],`${sensors[0].name} HUMIDITY=${_HUMD}% < LIMIT ${limitHUMDMIN}%`,alertPoints);
              callback(200,newAlert)
              return;
              // -------------
            } catch (err) {
      ...
      if (sensors.length > 0 && sensors[0].type!=='WISENSOR'){
        ...
        switch (sensorData.SENSORTYPE) {
          case 'AIRFLOW-VEL':
            ...
            // console.log(`[${"DECODERS.JS".green}] FOUND SENSOR IN DATABASE...<${sensorIDLabel.green}>... [${sensorData.DTUID.red}-${String(sensorData.SENSORID).red}]...<${String(sensors.length).yellow}>`)
            console.log(`[${"DECODERS.JS".green}] <${sensorData.SENSORTYPE.yellow}> [${sensorData.DTUID.red}-${String(sensorData.SENSORID).red}] <${String(sensors.length).yellow}> NAME=<${String(name).green}> LABEL=<${String(label).red}>  VALUE=<${String(value).green}> ALERT=${String(AlertFlag).toUpperCase().yellow} ..<${String(sensorData.RCV_BYTES).green}>.. ${String(sensorData.DATAS).yellow}`);
            // console.log(`[${"DECODERS.JS".green}] TYPE=<[${String(sensorData.SENSORTYPE).yellow}]> NAME=<${String(name).green}> LABEL=<${String(label).red}> VALUE=<${String(value).green}>...${String(AlertFlag).yellow}...`)
            break;
          case 'AIR-PRES':
            label = 'AIR PRESSURE';
          ...
          case 'WATER-TEMP':
            ...
            console.log(`[${"DECODERS.JS".green}] <${sensorData.SENSORTYPE.yellow}> [${sensorData.DTUID.red}-${String(sensorData.SENSORID).red}] <${String(sensors.length).yellow}> NAME=<${String(name).green}> LABEL=<${String(label).red}>  VALUE=<${String(value).green} C> ALERT=${String(AlertFlag).toUpperCase().yellow} ..<${String(sensorData.RCV_BYTES).green}>.. ${String(sensorData.DATAS).yellow}`);
            break;
          case 'AIRFLOW-TEMP':
            ...
            // console.log(sensorData);
            // Obj["temperature"]=`${(Number(_dataArr[1])/10).toFixed(2)} C`;
            console.log(`[${"DECODERS.JS".green}] <${sensorData.SENSORTYPE.yellow}> [${sensorData.DTUID.red}-${String(sensorData.SENSORID).red}] <${String(sensors.length).yellow}> NAME=<${String(name).green}> LABEL=<${String(label).red}>  VALUE=<${String(_RH).green} %/${String(_TEMP).green} C> ALERT=${String(AlertFlag).toUpperCase().yellow} ..<${String(sensorData.RCV_BYTES).green}>.. ${String(sensorData.DATAS).yellow}`);
            break;
        ...
        if (AlertFlag) {
          ...
          _logs.append('_TDKALERTS',jsonPayload,()=>{});
          // -------------
          decoders.sendWhatsApp(sensors[0],`${sensors[0].name} ${label}=0.0`,alertPoints)
          callback(200,newAlert);
        }
        // -------------
        return;
...
decoders.sendWhatsApp = function(sensor,str,alertPoints) {
  ...
  Alert.find(query).sort({date: -1}).exec((error,alerts) => {
    ...
    console.log(`[${"DECODERS.JS".green}] ....<${String(alerts.length).red}>.... SENDWHATSAPP...${String(sensor.dtuId).yellow}-${String(sensor.sensorId).yellow}..`)
    // --------------------
    if (alerts.length > 1) {
      // --------
...
decoders.sendWhatsAppMessage = function(sensor,str,minutes) {
  ...
  console.log(`[${"DECODERS.JS".green}] SENDING WHAT'S APP MESSAGES...[${label.red}]. <${str.yellow}>`)
