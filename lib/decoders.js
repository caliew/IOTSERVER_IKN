...
decoders.decode485Sensor = function (PortID, DataArr, callback) {
  ...
  // DEBUG_CONSOLE && console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[1].${hex_to_ascii(DataHEADER).green}|${hex_to_ascii(DataContent).substring(0,15).yellow}...`);
  ...
  if ( DataArr1.length > 0 && DataArr2.length > 0) {
    ...
    // ---------------------------------------
    // DEBUG_CONSOLE && console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[2]..DATA=<${DataArr1.length}|${DataArr2.length}>..`);
    // _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR.. PORT=${PortID} .[2]..DATA=<${DataArr1.length}|${DataArr2.length}>..`,()=>{});
    //  ------------------------------
    try
    {
    ...
    let RCV_BYTES = [];
    // ----------------
    // console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[3]..NDATA=<${NDATA}>..`);
    // _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR..PORT=${PortID} .[3]..NDATA=<${NDATA}>..`,()=>{});
    // ---------------------------
    for (i = 0; i < NDATA; i++) {
    ...
    _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
      ...
      if (!err) {
        ...
        _data.read("sensors",fileName,function(err,sensorObj) {
          ...
          decoders.CheckAlert(sensorData);
          // -----------------------------
          console.log(`[${'DECODERS.JS'.green}] ..${'DECODE485SENSOR'.red}.. PORT=${String(PortID).yellow} .[200].<${DataArr.length}>=<${String(DataArr[0]).substring(0,20).red}..>|<${String(DataArr[1]).substring(0,3)}>..FUNCID=[${FUNCID}] ..DTU.ID=[${String(DTUID).padStart(2,"0")}]..SNR.ID=[${String(SENSORID).padStart(2,"0")}].. `);
          _logs.append('_ERROR',`[DECODERS.JS] ..DECODE485SENSOR.. PORT=${PortID} .[200].<${DataArr.length}>=<${String(DataArr[0]).substring(0,20)}..>|<${String(DataArr[1]).substring(0,3)}>..FUNCID=[${FUNCID}] ..DTU.ID=[${String(DTUID).padStart(2,"0")}]..SNR.ID=[${String(SENSORID).padStart(2,"0")}].. `,()=>{});
          callback(200,sensorData)
          return;
          //  ----------------------------
        })
