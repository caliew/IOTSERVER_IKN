    ...
    Sensor.find(query).exec( (error,sensors) => {
      ...
      if (sensors.length > 0 && sensors[0].type==='WISENSOR') {
        ...
        if (_TEMP > limitTEMPMAX) {
          try {
            ...
            // -------------
            decoders.sendWhatsApp(sensors[0],`${sensors[0].name} TEMPERATURE=${_TEMP}C > LIMIT ${limitTEMPMAX}C`,alertPoints,newAlert);
        ...
        if (_TEMP < limitTEMPMIN) {
          try {
            ...
            _logs.append('_TDKALERTS',jsonPayload,()=>{});
            // -------------
            decoders.sendWhatsApp(sensors[0],`${sensors[0].name} TEMPERATURE=${_TEMP}C < LIMIT ${limitTEMPMIN}C`,alertPoints,newAlert)
            callback(200,newAlert);
            return;
            // -------------
          } catch (err) {
        ...
        if (_HUMD > 0 ) {
          ...
          if (_HUMD > limitHUMDMAX) {
            try {
              ...
              _logs.append('_TDKALERTS',jsonPayload,()=>{});
              // -------------
              decoders.sendWhatsApp(sensors[0],`${sensors[0].name} HUMIDITY=${_HUMD}% > LIMIT ${limitHUMDMAX}%`,alertPoints,newAlert);
              callback(200,newAlert)
              return;
              // -------------
            } catch (err) {
          ...
          if (_HUMD < limitHUMDMIN) {
            try {
              ...
              _logs.append('_TDKALERTS',jsonPayload,()=>{});
              // -------------
              decoders.sendWhatsApp(sensors[0],`${sensors[0].name} HUMIDITY=${_HUMD}% < LIMIT ${limitHUMDMIN}%`,alertPoints,newAlert);
              callback(200,newAlert)
              return;
              // -------------
            } catch (err) {
      ...
      if (sensors.length > 0 && sensors[0].type!=='WISENSOR'){
        ...
        if (AlertFlag) {
          ...
          _logs.append('_TDKALERTS',jsonPayload,()=>{});
          // -------------
          decoders.sendWhatsApp(sensors[0],`${sensors[0].name} ${label}=0.0`,alertPoints,newAlert)
          callback(200,newAlert);
        }
        // -------------
        return;
...
decoders.sendWhatsAppMessage = function(sensor,str,sensorObj,minutes) {
  // -----------
  ...
  AlertGroup.find({}).exec((error,alertgroups) => {
    ...
    alertgroups.forEach( group => {
      ...
      // -----------------
      if ( nIndex > -1) {
        if (group.name === 'TDK Alerts Group') {
          var jsonPayload = JSON.stringify(sensorObj);
          _logs.append('_TDK',jsonPayload,()=>{});
        }
