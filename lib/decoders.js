  ...
  AlertGroup.find({}).exec((error,alertgroups) => {
    ...
    alertgroups.forEach( group => {
      ...
      if ( nIndex > -1) {
        ...
        _logs.append('_WHATSAPP',`[DECODERS.JS] <${getDateTimeStamp()}> ..ALERT .[${group.name}].[${label}]. ${str}`,()=>{{}});
        var jsonPayload = JSON.stringify(_AlertObject);
        // -----
...
decoders.WhatMateMessageGateway1 = function(whatsAppGroup,message) {
  // --------------------------
  console.log(`[${"DECODERS.JS".yellow}] GATEWAY1 <${whatsAppGroup.green}> ..<${message.yellow}>..`);
  ...
  try {
    ...
    request.on('response', function (response) {
      ...
      response.on('data', function (chunk) {
        ...
        console.log(`[${'DECODERS.JS'.yellow}] ...${getDateTimeStamp()}...STATUS CODE=${response.statusCode}
         ..${message}.. RESPONSE RECEIVED FROM WhatsMate WA GATEWAY: 
         RESULT<${String(result).toUpperCase().yellow}>..ID<${String(id).green}>`);
         _logs.append('_WHATSAPP',`[DECODERS.JS] WA GATEWAY : <${getDateTimeStamp()}> STATUS CODE=${response.statusCode} RESULT<${String(result).toUpperCase()}>..ID<${String(id).toUpperCase()}>`,()=>{{}});
      });
    });   
  } catch (err) {
