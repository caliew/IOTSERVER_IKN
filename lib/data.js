  ...
  if (!_WhiteSpace && data) {
    try {
      if (!fs.existsSync(lib.baseDir+dir)){
        fs.mkdirSync(lib.baseDir+dir, { recursive: true });
      }
    } catch(e){}
    // ------
...
lib.append = async function(dir,file,str,callback){
  // ----------------------------------------
  if (dir === null || file === null ) {
    callback("FILE OR DIRECTORY NOT DEFINED");
    return;
  ...
  if (!_WhiteSpace) {
    ...
    fs.readFile(_filePathName, 'utf8', function(err,data){
      ...
      if(!err && data){
        ...
        try{
          // --------------
          parsedData = helpers.parseJsonToObject(data,_filePathName,'data.js');
