...
server.init = function() {
    ...
    app.get('/shinko',(req,res)=>{
        console.log(`.. <${'SENSORS.JS'.magenta}> ..${req.originalUrl.toUpperCase().yellow} [${req.method.green}]`)
        _logs.read('_SHINKO',30,null,null,false,function(err,sensorData) {
            // -----------------------------
            let ObjData = {};
            ObjData['sensorData'] = sensorData;
            // console.log(sensorData)
    ...
    app.post('/create-pdf', (req, res) => {
        // -------
        console.log(`.. <${'EXPRESSSERVER.JS'.green}> ..${req.originalUrl.toUpperCase().yellow} [${req.method.green}]`)
        let sensorsArr = [];
    ...
    app.get('/pddata',(req,res)=>{
        console.log(`.. <${'SENSORS.JS'.magenta}> ..${req.originalUrl.toUpperCase().yellow} [${req.method.green}]`);
        res.sendFile(`${__dirname}/logs/pddata.log`);
    })
    ...
    // -----------------------
    // app.listen(PORT, () => console.log(`[${'EXPRESSSERVER.JS'.green}]... REST API Server INIT On PORT ${String(config.RESTAPIPort).cyan}`));    
    app.listen(5000, () => console.log(`[${'EXPRESSSERVER.JS'.green}]... REST API Server INIT On PORT ${String(config.RESTAPIPort).cyan}`));    
    // --------
}
