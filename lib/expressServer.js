...
server.init = function () {
  ...
  // POST /api/fileStores/sync - Synchronize device stats with incidents
  app.post('/api/fileStores/sync', (req, res) => {
    try {
      const site = req.body?.site || req.body?.siteName || req.query.site || req.query.siteName;
      const year = req.body?.year || req.query.year;
      const result = fileStores.syncDeviceStats(site, year);
      return res.status(200).json(result);
    } catch (err) {
      console.error('Error in POST /api/fileStores/sync:', err);
      return res.status(500).json({ error: 'Internal Server Error' });
    }
  });

  // GET /api/fileStores/sync
  app.get('/api/fileStores/sync', (req, res) => {
    try {
      const site = req.query.site || req.query.siteName;
      ...
      console.error('Error in GET /api/fileStores/sync:', err);
      return res.status(500).json({ error: 'Internal Server Error' });
    }
  });
