     ...
     if(!err && userIds && userIds.length > 0){
       ...
       userIds.forEach(function(userId, index){
         _data.read('users',userId,function(err,userData){
           if(!err && userData){
             console.log(`${userData.firstName} ${userData.lastName} ${userData.phone} ${userData.hashedPassword}`)
             cli.verticalSpace(0);
           }
         });
       });
     ...
     if(!err && checkIds && checkIds.length > 0){
       ...
       checkIds.forEach(function(checkId){
         _data.read('gateways',checkId,function(err,checkData){
           if(!err && checkData){
             if (ActiveGateways.indexOf(checkData.GATEWAYID) > -1) checkData.ACTIVE = 'ON';
             console.log(checkData.PORT,checkData.MATCHKEY,checkData.TIMESTAMP,checkData.GATEWAYID);
           }
         });
       });
     }
     ...
     _logs.decompress(logFileName,function(err,strData){
       if(!err && strData){
         ...
         arr.forEach(function(jsonString){
           var logObject = helpers.parseJsonToObject(jsonString,'','cli.js');
