...
lib.read = function(file, nLINES, date0, date1, debugMODE, callback) {
  ...
  fs.readFile(_filePathName, 'utf8', (err, data) => {
    ...
    false && logDebugInfo(file, nLINES, startDate, endDate, lastNLines, debugMODE);
  ...
  function hasDateRange(_startDate, _endDate) {
  ...
  function processLogData(data, file, nLINES, startDate, endDate, debugMODE) {
    if (!data) return [];
