...
lib.list = function(includeCompressedLogs,callback){
  ...
  if (!_WhiteSpace) {
    ...
    callback(`[${DATA.JS.yellow}] ..${"LIB.LIST".red}.. WHITESPACE IN IN FILEPATHNAME ${_filePathName}`);
    return;
  }
};
...
lib.read = function(file,nLINES,date0,date1,debugMODE,callback){
  ...
  let _filePathName = lib.baseDir + file + '.log'
  let _WhiteSpace = hasWhiteSpace(_filePathName);
  debugMODE && console.log(`[${"LOGS.JS".green}].. ${file.toString().yellow}....<${_WhiteSpace}>`);
  // ----------------
  if (!_WhiteSpace) {
    fs.readFile(lib.baseDir+'/'+file+'.log', 'utf8', function(err,data){
      ...
      if (data) {
        ...
        //  -----------------------------
        let totalCount = nLINES > -1 ? nLINES : 9999;
        // ---------------
        let _filtered = datas.filter(item => {
          let objItem = helpers.parseJsonToObject(item);
          if (objItem) {
        ...
        if (date0 === null && date1 === null) {
          for (var i=0; i <= totalCount; i++){
            ...
            let parsedData
            try {
              if (datas[datas.length-i-1])  parsedData = helpers.parseJsonToObject(datas[datas.length-i-1]);          
              // ----------------------------------------
...
lib.readLastRecord = function(file,callback){
  ...
  if (!_WhiteSpace) {
    fs.readFile(lib.baseDir+'/'+file+'.log', 'utf8', function(err,data) {
      ...
      if (data) {
        var datas = data.split(/\r?\n/);
        let parsedData = helpers.parseJsonToObject(datas[datas.length-2]);
    ...
    callback(`[${DATA.JS.yellow}] ..${"LIB.READLASTRECORD".red}.. WHITESPACE IN IN FILEPATHNAME ${_filePathName}`);
    return;
  }
}
