...
handlers.fileStoresRollups = function(data, callback) {
  if (data.method === 'get') {
    ...
    const site = qs.site || qs.siteName;
    const macId = qs.macId || qs.mac;
    const year = qs.year ? parseInt(qs.year, 10) : new Date().getFullYear();
...
handlers.fileStoresSync = function(data, callback) {
  if (data.method === 'post' || data.method === 'get') {
    const payload = data.payload || {};
    const qs = data.queryStringObject || {};
    const site = payload.site || payload.siteName || qs.site || qs.siteName;
    const year = payload.year || qs.year;
...
handlers.incidents = function(data, callback) {
  if (data.method === 'get') {
    ...
    const site = qs.site || qs.siteName;
    const year = qs.year ? parseInt(qs.year, 10) : new Date().getFullYear();
    const isChecked = qs.isChecked;
    const isVerified = qs.isVerified;
    const macId = qs.macId;
    const alertType = qs.alertType || qs.type;
    ...
    const payload = data.payload || {};
    const qs = data.queryStringObject || {};
    const site = payload.site || payload.siteName || qs.site;
    const year = payload.year || qs.year || new Date().getFullYear();
    const incidentId = payload.incidentId || qs.incidentId;
    const month = payload.month || qs.month;
