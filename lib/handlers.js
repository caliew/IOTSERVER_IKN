...
handlers.fileStoresRollups = function(data, callback) {
  if (data.method === 'get') {
    const site = data.queryStringObject.site || data.queryStringObject.siteName;
    const macId = data.queryStringObject.macId || data.queryStringObject.mac;
    const year = data.queryStringObject.year || new Date().getFullYear();

    if (!site || !macId) {
      return callback(400, { Error: 'Missing site and macId parameter' });
    }

    const rollup = fileStores.getRollup(site, macId, year);
    if (rollup) {
      callback(200, rollup);
    } else {
      callback(404, { Error: 'Rollup data not found for given site/macId/year' });
    }
  } else {
    callback(405);
  }
...
handlers.incidents = function(data, callback) {
  if (data.method === 'get') {
    const site = data.queryStringObject.site || data.queryStringObject.siteName;
    const year = data.queryStringObject.year || new Date().getFullYear();
    const isChecked = data.queryStringObject.isChecked;
    const isVerified = data.queryStringObject.isVerified;
    const macId = data.queryStringObject.macId;
    const alertType = data.queryStringObject.alertType || data.queryStringObject.type;
    const month = data.queryStringObject.month;
    const day = data.queryStringObject.day;
    const week = data.queryStringObject.week;

    if (!site) {
      return callback(400, { Error: 'Missing required query parameter: site' });
    }
    ...
    callback(200, incidents);
  } else if (data.method === 'post' || data.method === 'put') {
    const payload = data.payload || {};
    const site = payload.site || payload.siteName || (data.queryStringObject ? data.queryStringObject.site : null);
    const year = payload.year || (data.queryStringObject ? data.queryStringObject.year : null) || new Date().getFullYear();
    const incidentId = payload.incidentId || (data.queryStringObject ? data.queryStringObject.incidentId : null);
    const month = payload.month || (data.queryStringObject ? data.queryStringObject.month : null);

    if (!site || !incidentId) {
      return callback(400, { Error: 'Missing required fields: site and incidentId' });
    }

    if (month && !payload.month) payload.month = month;

    const updated = fileStores.updateIncidentVerification(site, year, incidentId, payload);
    if (updated) {
      callback(200, { Success: true, message: 'Incident verification updated' });
    } else {
      callback(404, { Error: 'Incident ID not found or update failed' });
    }
  } else {
    callback(405);
  }
...
handlers.powerTrackerStatus = function(data, callback) {
  if (data.method === 'get') {
    const site = data.queryStringObject ? (data.queryStringObject.site || data.queryStringObject.siteName) : null;
    const summary = powerTracker.getSummary(site);
    callback(200, summary);
  } else {
    callback(405);
  }
};
