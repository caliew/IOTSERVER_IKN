/*
 * Library for storing and aggregating pre-computed sensor data & incident records
 * Structure:
 *   .data/fileStores/<siteName>/<macId>/<year>.json  (Weekly, Monthly, Yearly rollups)
 *   .data/fileStores/<siteName>/incidents_<year>_<month>.json (Incident verification logs)
 */

const fs = require('fs');
const path = require('path');

const lib = {};

// Base directory for fileStores
lib.baseDir = path.join(__dirname, '/../.data/fileStores/');

// Helper: Ensure directory exists
function ensureDir(dirPath) {
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }
}

// Helper: Get ISO Week string (e.g. "W37")
function getISOWeekString(date) {
  const d = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()));
  const dayNum = d.getUTCDay() || 7;
  d.setUTCDate(d.getUTCDate() + 4 - dayNum);
  const yearStart = new Date(Date.UTC(d.getUTCFullYear(), 0, 1));
  const weekNo = Math.ceil((((d - yearStart) / 86400000) + 1) / 7);
  return `W${String(weekNo).padStart(2, '0')}`;
}

// Known site name alias mappings (e.g. FE site keys -> disk directory names)
const SITE_ALIASES = {
  IKNOPSROOM: 'IKN_OPROOM',
  IKNOPS_ROOM: 'IKN_OPROOM',
  IKN_OPSROOM: 'IKN_OPROOM',
  IKNHOSPITAL: 'IKN_HOSPITAL',
  IKNPATHOLOGY: 'IKN_PATHOLOGY',
  IKN_PATHOLOGY: 'IKN_PATHOLOGY',
  PATHOLOGY: 'IKN_PATHOLOGY',
  NEGMWGT: 'NIPPONGLASS',
  TDKJOHOR: 'TDK_JOHOR',
};

function resolveSiteName(name) {
  if (!name) return name;
  const key = String(name).toUpperCase().replace(/[^A-Z0-9]/g, '');
  return SITE_ALIASES[key] || SITE_ALIASES[name] || name;
}

// Helper: Get sanitized site name
function sanitizeName(str) {
  const resolved = resolveSiteName(str);
  return String(resolved || 'default').replace(/[^a-zA-Z0-9_-]/g, '_');
}

// Helper: Find directory case-insensitively and flexibly matching delimiters (:-_)
// Also supports prefix matching for DTU IDs (e.g. '3553' matches '3553-1', '3553-2')
function findMatchingSubdir(parentDir, targetName) {
  if (!targetName || !fs.existsSync(parentDir)) return null;

  const resolvedTarget = resolveSiteName(targetName);
  const sanitized = sanitizeName(resolvedTarget);
  const normalize = (str) => String(str || '').toLowerCase().replace(/[:_\s]/g, '-');
  const targetNorm = normalize(resolvedTarget);

  try {
    const entries = fs.readdirSync(parentDir);
    // 1. Direct match with exact sanitized name
    for (const entry of entries) {
      if (entry === sanitized) return entry;
    }
    // 2. Case and delimiter insensitive match
    for (const entry of entries) {
      if (normalize(entry) === targetNorm) return entry;
    }
    // 3. Match ignoring all hyphens/delimiters (e.g. B0BC82C4C441 vs b0-bc-82-c4-c4-41)
    const targetClean = targetNorm.replace(/-/g, '');
    for (const entry of entries) {
      if (normalize(entry).replace(/-/g, '') === targetClean) return entry;
    }
    // 4. Prefix match: '3553' matches '3553-1', '3553-2' etc (DTU ID with sensor sub-index)
    for (const entry of entries) {
      const entryNorm = normalize(entry);
      if (entryNorm === targetNorm + '-1' || entryNorm.startsWith(targetNorm + '-')) return entry;
    }
  } catch (err) {
    console.error(`[FILESTORES.JS] Error searching directory ${parentDir}:`, err.message);
  }
  return null;
}

// Helper: Find ALL matching subdirs for a DTU prefix (e.g. '3553' -> ['3553-1','3553-2'])
function findAllMatchingSubdirs(parentDir, targetName) {
  if (!targetName || !fs.existsSync(parentDir)) return [];

  const resolvedTarget = resolveSiteName(targetName);
  const normalize = (str) => String(str || '').toLowerCase().replace(/[:_\s]/g, '-');
  const targetNorm = normalize(resolvedTarget);
  const targetClean = targetNorm.replace(/-/g, '');

  try {
    const entries = fs.readdirSync(parentDir);
    const matches = [];
    for (const entry of entries) {
      const entryNorm = normalize(entry);
      const entryClean = entryNorm.replace(/-/g, '');
      if (
        entryNorm === targetNorm ||                          // exact
        entryClean === targetClean ||                        // delimiter-insensitive
        entryNorm === targetNorm + '-1' ||                   // DTU-1 exact
        entryNorm.startsWith(targetNorm + '-')              // DTU-N prefix
      ) {
        matches.push(entry);
      }
    }
    return matches;
  } catch (err) {
    console.error(`[FILESTORES.JS] Error scanning directory ${parentDir}:`, err.message);
  }
  return [];
}

// Helper: Safe JSON read
function readJsonFile(filePath, defaultVal = null) {
  try {
    if (!fs.existsSync(filePath)) return defaultVal;
    const content = fs.readFileSync(filePath, 'utf8');
    return JSON.parse(content);
  } catch (err) {
    console.error(`[FILESTORES.JS] Error reading ${filePath}:`, err.message);
    return defaultVal;
  }
}

// Helper: Safe JSON write
function writeJsonFile(filePath, data) {
  try {
    const dir = path.dirname(filePath);
    ensureDir(dir);
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2), 'utf8');
    return true;
  } catch (err) {
    console.error(`[FILESTORES.JS] Error writing ${filePath}:`, err.message);
    return false;
  }
}

// Helper: Update Min/Max/Average metrics object
function updateMetricBucket(bucket, value) {
  if (value === undefined || value === null || isNaN(value)) return;
  const numVal = Number(value);

  if (bucket.count === 0 || bucket.min === undefined) {
    bucket.min = numVal;
    bucket.max = numVal;
    bucket.sum = numVal;
    bucket.count = 1;
    bucket.avg = numVal;
  } else {
    bucket.min = Math.min(bucket.min, numVal);
    bucket.max = Math.max(bucket.max, numVal);
    bucket.sum += numVal;
    bucket.count += 1;
    bucket.avg = Number((bucket.sum / bucket.count).toFixed(2));
  }
}

/**
 * Record a telemetry payload for a sensor into Weekly, Monthly, and Yearly rollups.
 * @param {string} rawSiteName - Site name (e.g. "ikn_hospital", "AEROSOFT")
 * @param {string} rawMacId - Sensor MAC / Device ID (e.g. "B0-BC-82-C4-C4-41")
 * @param {Object} readings - Parsed telemetry values or eval results
 * @param {boolean} isAlert - Whether this reading triggered an alert
 */
lib.recordTelemetry = function(rawSiteName, rawMacId, readings = {}, isAlert = false) {
  if (!rawSiteName || !rawMacId) return;

  const siteName = sanitizeName(rawSiteName);
  const macId = sanitizeName(rawMacId);
  const now = new Date();
  const year = now.getFullYear();
  const monthStr = String(now.getMonth() + 1).padStart(2, '0');
  const dayStr = String(now.getDate()).padStart(2, '0');
  const hourStr = String(now.getHours()).padStart(2, '0');
  const dayKey = `${monthStr}-${dayStr}`;
  const weekStr = getISOWeekString(now);

  const filePath = path.join(lib.baseDir, siteName, macId, `${year}.json`);

  let store = readJsonFile(filePath, {
    site: siteName,
    macId: macId,
    year: year,
    lastUpdated: now.toISOString(),
    yearly: { sampleCount: 0, limitBreaches: 0, metrics: {} },
    monthly: {},
    weekly: {},
    daily: {}
  });

  if (!store.daily) store.daily = {};

  store.lastUpdated = now.toISOString();
  store.yearly.sampleCount += 1;
  if (isAlert) store.yearly.limitBreaches += 1;

  // Ensure month bucket
  if (!store.monthly[monthStr]) {
    store.monthly[monthStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
  }
  store.monthly[monthStr].sampleCount += 1;
  if (isAlert) store.monthly[monthStr].limitBreaches += 1;

  // Ensure week bucket
  if (!store.weekly[weekStr]) {
    store.weekly[weekStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
  }
  store.weekly[weekStr].sampleCount += 1;
  if (isAlert) store.weekly[weekStr].limitBreaches += 1;

  // Ensure daily & hourly bucket
  if (!store.daily[dayKey]) {
    store.daily[dayKey] = { date: `${year}-${monthStr}-${dayStr}`, sampleCount: 0, limitBreaches: 0, metrics: {}, hourly: {} };
  }
  if (!store.daily[dayKey].hourly) store.daily[dayKey].hourly = {};

  store.daily[dayKey].sampleCount += 1;
  if (isAlert) store.daily[dayKey].limitBreaches += 1;

  if (!store.daily[dayKey].hourly[hourStr]) {
    store.daily[dayKey].hourly[hourStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
  }
  store.daily[dayKey].hourly[hourStr].sampleCount += 1;
  if (isAlert) store.daily[dayKey].hourly[hourStr].limitBreaches += 1;

  // Extract numerical telemetry values
  const valuesToTrack = {
    temperature: readings.Temperature ?? readings.temperature ?? readings.TEMP ?? null,
    humidity: readings.Humidity ?? readings.humidity ?? readings.RH ?? null,
    current: readings.current ?? readings.CURRENT ?? null,
    pressure: readings.pressure ?? readings.PRESSURE ?? null,
    dew: readings.dew ?? readings.DEW ?? null,
    waterLevel: readings.waterlevel ?? readings.waterLevel ?? readings.WATERLEVEL ?? null
  };

  // Update metrics for Year, Month, Week, Day, Hour
  for (const [key, val] of Object.entries(valuesToTrack)) {
    if (val !== null && val !== undefined && !isNaN(val)) {
      // Yearly
      if (!store.yearly.metrics[key]) store.yearly.metrics[key] = { min: 0, max: 0, sum: 0, count: 0, avg: 0 };
      updateMetricBucket(store.yearly.metrics[key], val);

      // Monthly
      if (!store.monthly[monthStr].metrics[key]) store.monthly[monthStr].metrics[key] = { min: 0, max: 0, sum: 0, count: 0, avg: 0 };
      updateMetricBucket(store.monthly[monthStr].metrics[key], val);

      // Weekly
      if (!store.weekly[weekStr].metrics[key]) store.weekly[weekStr].metrics[key] = { min: 0, max: 0, sum: 0, count: 0, avg: 0 };
      updateMetricBucket(store.weekly[weekStr].metrics[key], val);

      // Daily
      if (!store.daily[dayKey].metrics[key]) store.daily[dayKey].metrics[key] = { min: 0, max: 0, sum: 0, count: 0, avg: 0 };
      updateMetricBucket(store.daily[dayKey].metrics[key], val);

      // Hourly
      if (!store.daily[dayKey].hourly[hourStr].metrics[key]) store.daily[dayKey].hourly[hourStr].metrics[key] = { min: 0, max: 0, sum: 0, count: 0, avg: 0 };
      updateMetricBucket(store.daily[dayKey].hourly[hourStr].metrics[key], val);
    }
  }

  writeJsonFile(filePath, store);
};

/**
 * Record an alarm incident into the site's monthly incident log (incidents_YYYY_MM.json).
 * Initial state includes isChecked = false, isVerified = false.
 */
lib.recordIncident = function(rawSiteName, incidentData = {}) {
  if (!rawSiteName) return null;

  const siteName = sanitizeName(rawSiteName);
  const now = incidentData.timestamp ? new Date(incidentData.timestamp) : new Date();
  const year = now.getFullYear();
  const monthStr = String(now.getMonth() + 1).padStart(2, '0');
  const dayStr = String(now.getDate()).padStart(2, '0');
  const weekStr = getISOWeekString(now);

  const filePath = path.join(lib.baseDir, siteName, `incidents_${year}_${monthStr}.json`);

  let incidents = readJsonFile(filePath, []);
  if (!Array.isArray(incidents)) incidents = [];

  const incidentId = `INC-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
  const newIncident = {
    incidentId: incidentId,
    siteName: siteName,
    macId: incidentData.macId ? sanitizeName(incidentData.macId) : 'UNKNOWN',
    sensorName: incidentData.sensorName || incidentData.sensorConfig?.NAME || 'UNKNOWN',
    alertType: incidentData.alertType || incidentData.TYPE || 'LIMIT_BREACH',
    message: incidentData.message || incidentData.MESSAGE || '',
    timestamp: now.toISOString(),
    year: year,
    month: monthStr,
    day: dayStr,
    week: weekStr,
    isChecked: false,
    isVerified: false,
    checkedBy: null,
    checkedAt: null,
    notes: ''
  };

  incidents.unshift(newIncident); // Add newest first
  writeJsonFile(filePath, incidents);

  // Increment limitBreaches in device stats file
  if (newIncident.macId && newIncident.macId !== 'UNKNOWN') {
    lib.incrementDeviceLimitBreach(siteName, newIncident.macId, now);
  }

  return newIncident;
};

/**
 * Increment limitBreaches count in device rollup file (<year>.json) for a live incident.
 */
lib.incrementDeviceLimitBreach = function(rawSiteName, rawMacId, timestamp = new Date()) {
  if (!rawSiteName || !rawMacId) return;

  const siteDirName = findMatchingSubdir(lib.baseDir, rawSiteName) || sanitizeName(rawSiteName);
  const siteDir = path.join(lib.baseDir, siteDirName);

  if (!fs.existsSync(siteDir)) return;

  const macDirs = findAllMatchingSubdirs(siteDir, rawMacId);
  const targetDirs = macDirs.length > 0 ? macDirs : [sanitizeName(rawMacId)];

  const d = new Date(timestamp);
  const yr = d.getFullYear();
  const monthStr = String(d.getMonth() + 1).padStart(2, '0');
  const dayStr = String(d.getDate()).padStart(2, '0');
  const dayKey = `${monthStr}-${dayStr}`;
  const weekStr = getISOWeekString(d);
  const hourStr = String(d.getHours()).padStart(2, '0');

  for (const macDirName of targetDirs) {
    const yearFilePath = path.join(siteDir, macDirName, `${yr}.json`);
    let store = readJsonFile(yearFilePath, null);
    if (!store) {
      ensureDir(path.join(siteDir, macDirName));
      store = {
        site: siteDirName,
        macId: macDirName,
        year: yr,
        lastUpdated: d.toISOString(),
        yearly: { sampleCount: 0, limitBreaches: 0, metrics: {} },
        monthly: {},
        weekly: {},
        daily: {}
      };
    }

    store.lastUpdated = d.toISOString();
    store.yearly = store.yearly || { sampleCount: 0, limitBreaches: 0, metrics: {} };
    store.yearly.limitBreaches = (store.yearly.limitBreaches || 0) + 1;

    store.monthly = store.monthly || {};
    if (!store.monthly[monthStr]) store.monthly[monthStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
    store.monthly[monthStr].limitBreaches = (store.monthly[monthStr].limitBreaches || 0) + 1;

    store.weekly = store.weekly || {};
    if (!store.weekly[weekStr]) store.weekly[weekStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
    store.weekly[weekStr].limitBreaches = (store.weekly[weekStr].limitBreaches || 0) + 1;

    store.daily = store.daily || {};
    if (!store.daily[dayKey]) {
      store.daily[dayKey] = { date: `${yr}-${monthStr}-${dayStr}`, sampleCount: 0, limitBreaches: 0, metrics: {}, hourly: {} };
    }
    if (!store.daily[dayKey].hourly) store.daily[dayKey].hourly = {};
    store.daily[dayKey].limitBreaches = (store.daily[dayKey].limitBreaches || 0) + 1;

    if (!store.daily[dayKey].hourly[hourStr]) {
      store.daily[dayKey].hourly[hourStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
    }
    store.daily[dayKey].hourly[hourStr].limitBreaches = (store.daily[dayKey].hourly[hourStr].limitBreaches || 0) + 1;

    writeJsonFile(yearFilePath, store);
  }
};

/**
 * Synchronize device stats files (<siteName>/<macId>/<year>.json) limitBreaches counters
 * with incident records from incidents_<year>_<month>.json files.
 * @param {string} [rawSiteName] - Optional site name or 'all' to filter by site
 * @param {number|string} [targetYear] - Optional target year to filter by year
 * @returns {Object} Summary of sync operation
 */
lib.syncDeviceStats = function(rawSiteName = null, targetYear = null) {
  const summary = {
    success: true,
    sitesProcessed: [],
    totalDevicesUpdated: 0,
    totalIncidentsSynced: 0,
    details: []
  };

  if (!fs.existsSync(lib.baseDir)) {
    summary.success = false;
    summary.error = 'Base directory does not exist';
    return summary;
  }

  // Determine site directories to process
  let siteFolders = [];
  if (rawSiteName && rawSiteName !== 'all') {
    const matched = findMatchingSubdir(lib.baseDir, rawSiteName) || sanitizeName(rawSiteName);
    if (fs.existsSync(path.join(lib.baseDir, matched))) {
      siteFolders = [matched];
    }
  } else {
    try {
      siteFolders = fs.readdirSync(lib.baseDir).filter(f => fs.statSync(path.join(lib.baseDir, f)).isDirectory());
    } catch (err) {
      console.error('[FILESTORES.JS] Error reading base directory:', err.message);
    }
  }

  for (const siteFolder of siteFolders) {
    const siteDir = path.join(lib.baseDir, siteFolder);
    if (!fs.existsSync(siteDir)) continue;

    summary.sitesProcessed.push(siteFolder);

    // 1. Read all incident files in siteDir
    let incidentFiles = [];
    try {
      const files = fs.readdirSync(siteDir);
      incidentFiles = files.filter(f => (f.startsWith('incidents_') || f === 'incidents.json') && f.endsWith('.json'));
    } catch (err) {
      console.error(`[FILESTORES.JS] Error listing ${siteDir}:`, err.message);
      continue;
    }

    let allIncidents = [];
    for (const incFileName of incidentFiles) {
      const incFilePath = path.join(siteDir, incFileName);
      const fileData = readJsonFile(incFilePath, []);
      if (Array.isArray(fileData)) {
        allIncidents = allIncidents.concat(fileData);
      }
    }

    if (targetYear) {
      const yrNum = parseInt(targetYear, 10);
      allIncidents = allIncidents.filter(inc => {
        const incYr = inc.year || (inc.timestamp ? new Date(inc.timestamp).getFullYear() : null);
        return incYr === yrNum;
      });
    }

    // 2. Map incidents to device subdirectories and years
    // key: `${macDirName}:${year}` -> array of incident objects
    const incidentsByDeviceAndYear = {};

    for (const inc of allIncidents) {
      if (!inc.macId) continue;
      const d = inc.timestamp ? new Date(inc.timestamp) : new Date();
      const yr = inc.year || d.getFullYear();
      const monthStr = inc.month ? String(inc.month).padStart(2, '0') : String(d.getMonth() + 1).padStart(2, '0');
      const dayStr = inc.day ? String(inc.day).padStart(2, '0') : String(d.getDate()).padStart(2, '0');
      const weekStr = inc.week || getISOWeekString(d);
      const hourStr = String(d.getHours()).padStart(2, '0');
      const dayKey = `${monthStr}-${dayStr}`;

      // Resolve matching subdirs
      let macDirs = findAllMatchingSubdirs(siteDir, inc.macId);
      if (macDirs.length === 0) {
        macDirs = [sanitizeName(inc.macId)];
      }

      for (const macDirName of macDirs) {
        const key = `${macDirName}:${yr}`;
        if (!incidentsByDeviceAndYear[key]) {
          incidentsByDeviceAndYear[key] = [];
        }
        incidentsByDeviceAndYear[key].push({
          ...inc,
          year: yr,
          monthStr,
          dayStr,
          weekStr,
          hourStr,
          dayKey
        });
      }
    }

    // 3. Find all device directories (subfolders in siteDir) to update
    const subdirs = fs.readdirSync(siteDir).filter(f => fs.statSync(path.join(siteDir, f)).isDirectory());
    
    const allDeviceDirs = new Set(subdirs);
    for (const key of Object.keys(incidentsByDeviceAndYear)) {
      const [macDirName] = key.split(':');
      allDeviceDirs.add(macDirName);
    }

    for (const macDirName of allDeviceDirs) {
      const deviceDir = path.join(siteDir, macDirName);
      ensureDir(deviceDir);

      // Find all target years for this device directory
      const targetYearsSet = new Set();
      if (targetYear) {
        targetYearsSet.add(parseInt(targetYear, 10));
      } else {
        try {
          const files = fs.readdirSync(deviceDir);
          files.forEach(f => {
            if (/^\d{4}\.json$/.test(f)) {
              targetYearsSet.add(parseInt(f.replace('.json', ''), 10));
            }
          });
        } catch (err) {}

        for (const key of Object.keys(incidentsByDeviceAndYear)) {
          const [mDir, yrStr] = key.split(':');
          if (mDir === macDirName) {
            targetYearsSet.add(parseInt(yrStr, 10));
          }
        }
      }

      for (const yr of targetYearsSet) {
        const key = `${macDirName}:${yr}`;
        const deviceIncidents = incidentsByDeviceAndYear[key] || [];

        const yearFilePath = path.join(deviceDir, `${yr}.json`);
        let store = readJsonFile(yearFilePath, {
          site: siteFolder,
          macId: macDirName,
          year: yr,
          lastUpdated: new Date().toISOString(),
          yearly: { sampleCount: 0, limitBreaches: 0, metrics: {} },
          monthly: {},
          weekly: {},
          daily: {}
        });

        if (!store.yearly) store.yearly = { sampleCount: 0, limitBreaches: 0, metrics: {} };
        if (!store.monthly) store.monthly = {};
        if (!store.weekly) store.weekly = {};
        if (!store.daily) store.daily = {};

        // RESET limitBreaches for this year's store
        store.yearly.limitBreaches = 0;
        for (const m of Object.keys(store.monthly)) {
          if (store.monthly[m]) store.monthly[m].limitBreaches = 0;
        }
        for (const w of Object.keys(store.weekly)) {
          if (store.weekly[w]) store.weekly[w].limitBreaches = 0;
        }
        for (const dKey of Object.keys(store.daily)) {
          if (store.daily[dKey]) {
            store.daily[dKey].limitBreaches = 0;
            if (store.daily[dKey].hourly) {
              for (const hKey of Object.keys(store.daily[dKey].hourly)) {
                if (store.daily[dKey].hourly[hKey]) {
                  store.daily[dKey].hourly[hKey].limitBreaches = 0;
                }
              }
            }
          }
        }

        // APPLY deviceIncidents
        for (const inc of deviceIncidents) {
          const { monthStr, weekStr, dayStr, dayKey, hourStr } = inc;

          store.yearly.limitBreaches += 1;

          if (!store.monthly[monthStr]) {
            store.monthly[monthStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
          }
          store.monthly[monthStr].limitBreaches += 1;

          if (!store.weekly[weekStr]) {
            store.weekly[weekStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
          }
          store.weekly[weekStr].limitBreaches += 1;

          if (!store.daily[dayKey]) {
            store.daily[dayKey] = {
              date: `${yr}-${monthStr}-${dayStr}`,
              sampleCount: 0,
              limitBreaches: 0,
              metrics: {},
              hourly: {}
            };
          }
          if (!store.daily[dayKey].hourly) store.daily[dayKey].hourly = {};

          store.daily[dayKey].limitBreaches += 1;

          if (!store.daily[dayKey].hourly[hourStr]) {
            store.daily[dayKey].hourly[hourStr] = { sampleCount: 0, limitBreaches: 0, metrics: {} };
          }
          store.daily[dayKey].hourly[hourStr].limitBreaches += 1;
        }

        store.lastUpdated = new Date().toISOString();
        writeJsonFile(yearFilePath, store);

        summary.totalDevicesUpdated += 1;
        summary.totalIncidentsSynced += deviceIncidents.length;
        summary.details.push({
          site: siteFolder,
          macId: macDirName,
          year: yr,
          limitBreaches: store.yearly.limitBreaches,
          incidentsCount: deviceIncidents.length
        });
      }
    }
  }

  return summary;
};

/**
 * Update incident verification state from FE.
 */
lib.updateIncidentVerification = function(rawSiteName, year, incidentId, payload = {}) {
  if (!rawSiteName || !incidentId) return false;

  const siteDirName = findMatchingSubdir(lib.baseDir, rawSiteName) || sanitizeName(rawSiteName);
  const targetYear = year || new Date().getFullYear();
  const siteDir = path.join(lib.baseDir, siteDirName);

  if (!fs.existsSync(siteDir)) return false;

  let monthStr = payload.month ? String(payload.month).padStart(2, '0') : null;

  // Determine target files to search
  let candidateFiles = [];
  if (monthStr) {
    candidateFiles.push(path.join(siteDir, `incidents_${targetYear}_${monthStr}.json`));
  } else {
    // Scan all matching files in siteDir
    try {
      const files = fs.readdirSync(siteDir);
      files.forEach(f => {
        if ((f.startsWith(`incidents_${targetYear}_`) || f === `incidents_${targetYear}.json`) && f.endsWith('.json')) {
          candidateFiles.push(path.join(siteDir, f));
        }
      });
    } catch (err) {
      console.error(`[FILESTORES.JS] Error scanning ${siteDir}:`, err.message);
    }
  }

  for (const filePath of candidateFiles) {
    let incidents = readJsonFile(filePath, []);
    if (!Array.isArray(incidents)) continue;

    const idx = incidents.findIndex(inc => inc.incidentId === incidentId);
    if (idx !== -1) {
      incidents[idx].isChecked = payload.isChecked !== undefined ? Boolean(payload.isChecked) : true;
      incidents[idx].isVerified = payload.isVerified !== undefined ? Boolean(payload.isVerified) : true;
      incidents[idx].checkedBy = payload.checkedBy || 'operator';
      incidents[idx].checkedAt = new Date().toISOString();
      if (payload.notes !== undefined) incidents[idx].notes = String(payload.notes);

      return writeJsonFile(filePath, incidents);
    }
  }

  return false;
};

/**
 * Fetch telemetry rollup summary for a sensor.
 * Supports:
 *  - WiSensor MAC IDs (exact or case/delimiter flexible)
 *  - DTU IDs: '3553' will find and aggregate '3553-1', '3553-2' etc.
 */
lib.getRollup = function(rawSiteName, rawMacId, year = new Date().getFullYear()) {
  if (!rawSiteName || !rawMacId) return null;

  const siteDirName = findMatchingSubdir(lib.baseDir, rawSiteName) || sanitizeName(rawSiteName);
  const siteDir = path.join(lib.baseDir, siteDirName);

  if (!fs.existsSync(siteDir)) return null;

  const targetYear = year ? parseInt(year, 10) : new Date().getFullYear();

  // Find all matching sensor subdirectories (handles '3553' -> ['3553-1','3553-2'] AND exact MAC IDs)
  const macDirNames = findAllMatchingSubdirs(siteDir, rawMacId);

  if (macDirNames.length === 0) return null;

  // If only one match, return it directly
  if (macDirNames.length === 1) {
    const sensorDir = path.join(siteDir, macDirNames[0]);
    let filePath = path.join(sensorDir, `${targetYear}.json`);
    if (!fs.existsSync(filePath)) {
      try {
        const files = fs.readdirSync(sensorDir).filter(f => f.endsWith('.json')).sort().reverse();
        if (files.length > 0) filePath = path.join(sensorDir, files[0]);
      } catch (err) {}
    }
    return readJsonFile(filePath, null);
  }

  // Multiple matches: aggregate/merge rollup data across all sensor sub-folders
  const rollups = [];
  for (const macDirName of macDirNames) {
    const sensorDir = path.join(siteDir, macDirName);
    if (!fs.existsSync(sensorDir)) continue;
    let filePath = path.join(sensorDir, `${targetYear}.json`);
    if (!fs.existsSync(filePath)) {
      try {
        const files = fs.readdirSync(sensorDir).filter(f => f.endsWith('.json')).sort().reverse();
        if (files.length > 0) filePath = path.join(sensorDir, files[0]);
      } catch (err) {}
    }
    const data = readJsonFile(filePath, null);
    if (data) rollups.push({ macDirName, data });
  }

  if (rollups.length === 0) return null;

  // Return merged result: use first as base, add sub-sensors array
  const base = rollups[0].data;
  if (rollups.length === 1) return base;

  return {
    site: base.site,
    macId: String(rawMacId),
    year: targetYear,
    lastUpdated: rollups.reduce((latest, r) => r.data.lastUpdated > latest ? r.data.lastUpdated : latest, ''),
    yearly: base.yearly,
    monthly: base.monthly,
    weekly: base.weekly,
    daily: base.daily,
    subSensors: rollups.map(r => ({ macId: r.macDirName, ...r.data })),
  };
};

lib.getTelemetryRollup = lib.getRollup;

/**
 * Fetch incident list for a site with optional filters.
 */
lib.getIncidents = function(rawSiteName, year = new Date().getFullYear(), filters = {}) {
  if (!rawSiteName) return [];
  const siteDirName = findMatchingSubdir(lib.baseDir, rawSiteName) || sanitizeName(rawSiteName);
  const siteDir = path.join(lib.baseDir, siteDirName);

  if (!fs.existsSync(siteDir)) return [];

  let incidents = [];
  let monthStr = filters.month ? String(filters.month).padStart(2, '0') : null;

  if (monthStr) {
    const filePath = path.join(siteDir, `incidents_${year}_${monthStr}.json`);
    const monthData = readJsonFile(filePath, []);
    if (Array.isArray(monthData)) incidents = monthData;
  } else {
    // Collect all monthly files for the target year (and legacy year file if present)
    try {
      const files = fs.readdirSync(siteDir);
      files.forEach(f => {
        if ((f.startsWith(`incidents_${year}_`) || f === `incidents_${year}.json`) && f.endsWith('.json')) {
          const fileData = readJsonFile(path.join(siteDir, f), []);
          if (Array.isArray(fileData)) {
            incidents = incidents.concat(fileData);
          }
        }
      });
    } catch (err) {
      console.error(`[FILESTORES.JS] Error reading incident files in ${siteDir}:`, err.message);
    }
  }

  // Sort newest first
  incidents.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));

  return incidents.filter(inc => {
    if (filters.macId && sanitizeName(filters.macId) !== inc.macId) return false;
    if (filters.alertType && String(filters.alertType).toUpperCase() !== String(inc.alertType).toUpperCase()) return false;
    if (filters.isChecked !== undefined && Boolean(filters.isChecked) !== inc.isChecked) return false;
    if (filters.isVerified !== undefined && Boolean(filters.isVerified) !== inc.isVerified) return false;
    if (filters.day && String(filters.day).padStart(2, '0') !== inc.day) return false;
    if (filters.week && String(filters.week).toUpperCase() !== inc.week) return false;
    return true;
  });
};

module.exports = lib;


