  ...
  switch (type) {
    ...
    case 'AC CURRENT': {
      ...
      if (readingCurrent === undefined && hex) {
        if (readings.useCtRatio || sensorSetting.CTRATIO !== undefined) {
          // AP Mode / F8L10ST DTU CT Ratio decoding
          const ctRatio = sensorSetting.CTRATIO ?? 1;
          if (ctRatio !== 1) {
            readingCurrent = (Math.abs(helpers.hexToDecimal(hex)) * ctRatio) / 5.0;
          } else {
            readingCurrent = Math.abs(helpers.HEXTOINT(hex) / 100.0);
          }
          ...
          readingCurrent = Number(helpers.hexToSignedInt(hex) / 100);
        }
      }
    ...
    case 'AIR PRESSURE': {
      ...
      let readingPressure = readings.pressure;
      if (readingPressure === undefined && hex) {
        if (readings.useCtRatio || hex.length === 8) {
          // AP Mode decoding
          const hex1 = hex.slice(0, 4);
          const hex2 = hex.slice(4, 8);
          const newHEX = hex2 + hex1;
          if (hex.length === 8) {
            readingPressure = helpers.parseFloat(`0x${newHEX}`);
          } else {
            readingPressure = parseInt(hex, 16) * 1000;
          }
          readingPressure += offsetPress;
        } else {
          // Standard WiSensor hex decoding
          readingPressure = helpers.parseFloat(`0x${hex.slice(4, 8)}${hex.slice(0, 4)}`);
        }
        readingPressure = unitSystem === 'BAR' ? (readingPressure * 1.0e-5).toFixed(2) : readingPressure;
      }
      const calcPressMax = unitSystem === 'BAR' ? (pressMax * 1.0e-5).toFixed(2) : pressMax;
