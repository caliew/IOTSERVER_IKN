...
function evaluateSensorReadings({ type, sensorSetting, readings }) {
  ...
  switch (type) {
    ...
    case 'AIR PRESSURE': {
      ...
      if (readingPressure === undefined && hex) {
        const cleanHex = String(hex).trim();
        if (readings.useCtRatio || cleanHex.length === 8) {
          // AP Mode decoding
          ...
          if (cleanHex.length === 8) {
            ...
            readingPressure = parseInt(cleanHex, 16) * 1000;
          }
          readingPressure += offsetPress;
        } else {
          // Standard WiSensor hex decoding (middle-endian byte swap)
          readingPressure = helpers.parseFloat(`0x${cleanHex.slice(4, 8)}${cleanHex.slice(0, 4)}`);
        }
        // Match old code: if unitSystem == 'BAR', keep raw float; else scale Pa to BAR (1.0e-5)
        readingPressure = unitSystem === 'BAR' ? readingPressure : (readingPressure * 1.0e-5).toFixed(2);
      }
