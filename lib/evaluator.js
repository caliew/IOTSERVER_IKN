    ...
    case 'AIR PRESSURE': {
      ...
      if (readingPressure === undefined && hex) {
        ...
        if (cleanHex.length === 8) {
          // Check if valid 32-bit IEEE float (WiSensor / AP Mode float)
          const hex1 = cleanHex.slice(0, 4);
          const hex2 = cleanHex.slice(4, 8);
          const swappedHex = hex2 + hex1;
          const candidateFloat = helpers.parseFloat(`0x${swappedHex}`);

          if (
            typeof candidateFloat === 'number' &&
            !isNaN(candidateFloat) &&
            isFinite(candidateFloat) &&
            Math.abs(candidateFloat) >= 0.001 &&
            Math.abs(candidateFloat) <= 100000
          ) {
            parsedVal = unitSystem === 'BAR' ? candidateFloat : candidateFloat * 1.0e-5;
          } else {
            // Not a valid 32-bit float (e.g. 2-byte 16-bit integer '0418' + 2-byte trailing CRC '1CC4')
            const intVal = helpers.hexToSignedInt(hex1);
            parsedVal = intVal > 100 ? intVal / 100.0 : intVal / 10.0;
          }
        } else if (cleanHex.length >= 4) {
          // 4-character hex (2 bytes like '0079')
          const hex4 = cleanHex.slice(0, 4);
          const intVal = helpers.hexToSignedInt(hex4);
          parsedVal = intVal > 100 ? intVal / 100.0 : intVal / 10.0;
        }
