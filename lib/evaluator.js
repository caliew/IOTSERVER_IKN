...
function evaluateSensorReadings({ type, sensorSetting, readings }) {
  ...
  const offsetRh = sensorSetting.OFFSET_RH ? Number(sensorSetting.OFFSET_RH) : (sensorSetting._OFFSET_RH ? Number(sensorSetting._OFFSET_RH) : 0);
  ...
  switch (type) {
    ...
    case 'DIFF PRESS': {
      ...
      if (readingPressure === undefined && hex) {
        ...
        if (cleanHex.length <= 4) {
          ...
          readingPressure = helpers.calculateDiffPressure(cleanHex, offsetPress);
    ...
    case 'AIR PRESSURE': {
      ...
      if (readingPressure === undefined && hex) {
        ...
        if (cleanHex.length === 8) {
          ...
          const hex2 = cleanHex.slice(4, 8);
          const swappedHex = hex2 + hex1;
          const floatVal = helpers.parseFloat(`0x${swappedHex}`);
          if (typeof floatVal === 'number' && !isNaN(floatVal) && isFinite(floatVal)) {
            pascals = floatVal;
          }
        ...
        if (pascals !== null) {
          const pressInBar = pascals / 100000.0;
          const rawVal = unitSystem === 'BAR' ? pressInBar : pascals;
          readingPressure = Number((rawVal + offsetPress).toFixed(2));
        } else {
          return { isAlert: false, isFalseSignal: true, alertObj: null, message: '' };
        }
      }
    ...
    case 'TEMP & RH': {
      ...
      if ((readingTemp === undefined || readingRh === undefined) && hex) {
        ...
        readingTemp = Number(readingTemp);
        readingRh = Number(readingRh);
      }

      // Apply offsets uniformly across all WiSensor data sources
      if (readingTemp !== undefined && readingTemp !== null) readingTemp = Number(readingTemp) + offsetTemp;
      if (readingRh !== undefined && readingRh !== null) readingRh = Number(readingRh) + offsetRh;
