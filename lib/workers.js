 ...
 */
 ...
 workers.TIMERLOOP1 = function(nInterval){
  ...
  LoopInterval = setInterval(function() {
    ...
    // -------------------
    console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'TIMERLOOP1'.yellow}> ... SOCKET CONNECTED <${_server.socketArr.length}> ...TIMER <${nInterval}>`);
    // ---------------------------------------
    ...
    if (_server.socketArr){
      ...
      _data.read("ATCOMMANDS",'_ATCOMMANDS_TIMER1',function(err,ATCMNDSObj) {
        ...
        if(err) {
         ...
         // -----------
         console.log(`[${'WORKERS.JS'.green}]  .. AT COMMANDS READ FROM <${'_ATCOMMANDS_TIMER1'.yellow}> .. ACTIVE AT-COMMAND <${ACTIVE_CMND.length}/${ATCMNDSObj.length}>..`);
        // --------------
        ...
        _data.read("rawData","_485SENSORS",function(err,CMD_485RCV) {
          ...
          _data.read("rawData",'ATCMND_SENT1',function(err,CMD_SENT) {
            ...
            _data.create("rawData", 'ATCMND_SENT1', ACTIVE_CMND, function (err) {
              ...
              // -----------
              console.log(`[${'WORKERS.JS'.green}]  .. ATCMND_SENT1 COMPLETE..`);
              // -----------
            });
          });
        })
