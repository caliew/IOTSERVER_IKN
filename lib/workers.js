...
const { stringify } = require('querystring');
 ...
 const nTIMER2 = 15;    // TDKSENSORSGROUP
 ...
 workers.TDKSENSORSGROUP = function(nInterval,JSONFilename) {
   ...
   let LoopInterval = setInterval(function() {
     ...
     // ------------------------
     // TO RESET THE 
     // DECODER ATCMNDArr AND SET COUNTER_485=0
     // ------------------------
     decoder.RESET_ATCMND();
     // -----------------------
     let _GATEWAYIDArr = [];
     if (_server.socketArr){
       ...
       _data.read("ATCOMMANDS",JSONFilename,function(err,ATCMNDSObj) {
          ...
          // -----------
          _logs.append('_ERROR',`[WORKERS.JS] ${time} <${JSONFilename}> GATEWAY=<${_GATEWAYTEXT}> TIMER<${nInterval}>.. SOCKETS=<${_server.socketArr.length}>`,()=>{});
          ...
          _logs.append('_ERROR',`[WORKERS.JS] ${time} <${JSONFilename}> SOCKETS=<${String(_ATSOCKETS)}|${String(_DIFFSOCKETS)}> .AT-COMMAND=<${ACTIVE_CMND.length}|${ATCMNDSObj.length}>`,()=>{});
         // --------------
              ...
              _data.create("rawData", 'ATCMND_SENT2', ACTIVE_CMND, function (err) {
                ...
                TDKSENSORSGROUP = setInterval(function() {
                  ...
                  if (item && item.FLAG) {
                    ...
                    // ---------- 
                    // REGISTER THE ATMCND INTO DECODER.ATCMNDArr
                    decoder.ADD_ATCMND(item)
                    // ----------
                  }
                  // ---------
