 ...
 */
 ...
 workers.TDKSENSORSGROUP = function(nInterval,JSONFilename) {
   // --------------
   let today = new Date();
   let date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
   ...
   let LoopInterval = setInterval(function() {
     ...
     // ------------------------
     // TO RESET THE 
     // DECODERS ATCMNDArr AND SET COUNTER_485=0
     // ------------------------
     decoders.RESET_ATCMND();
     // -----------------------
     let _GATEWAYIDArr = [];
     if (_server.socketArr){
       ...
       // AT COMMANDS TIMER2
       // -------------------
       _data.read("ATCOMMANDS",JSONFilename,function(err,ATCMNDSObj) {
        // -----------  
        if(err) {
         ...
         _data.read("rawData","_485SENSORS",function(err,CMD_485RCV) {
           ...
           _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
             ...
             // ---------------
              ...
              // ---------
              _data.create("rawData", 'ATCMND_SENT2', {...ACTIVE_CMND, time}, function (err) {
                let nSTEP = 1;
                if (ACTIVE_CMND.length == 0) return;
                // -------------------
                // LOOP TO SEND AT COMMAND 
                ...
                TDKSENSORSGROUP = setInterval(function() {
                  ...
                  if (item && item.FLAG) {
                    ...
                    // ---------- 
                    // REGISTER THE ATMCND INTO DECODERS ATCMNDArr
                    decoders.ADD_ATCMND(item)
                    // ----------
                  }
                  // ---------
                  ...
                  if (TDKSENSORSGROUP_COUNTER > ACTIVE_CMND.length -1) {
                    ...
                    // _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> AT-COMMAND SENT=<${TDKSENSORSGROUP_COUNTER}>`,()=>{});
                    // ----------------------------
                  }
                  // --------------
                },1000 * nSTEP);
  ...
  let LoopInterval = setInterval(function() {
    ...
    if (minutes%nRPORTTIMER == 0) {
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        bln_DEBUG_SERVERSTATUSUPDATEDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER2=[${nRPORTTIMER.toString().yellow}] <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length.toString().red}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..`);
        let nameAlerts = alertsData?.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
            map[key] = [];
