 ...
 */
 ...
 const TIMERLOOP1_ERRORLOG = false;
 const TIMERLOOP1_CONSOLE  = false;
 ...
 workers.TIMERLOOP1 = function(nInterval){
  ...
  LoopInterval = setInterval(function() {
    ...
    // -------------------
    TIMERLOOP1_ERRORLOG && console.log(`[${'WORKERS.JS'.green}]  .. ..`);
    ...
    if (_server.socketArr){
      ...
      _data.read("ATCOMMANDS",'_ATCOMMANDS_TIMER1',function(err,ATCMNDSObj) {
        ...
        // -----------
        TIMERLOOP1_ERRORLOG && console.log(`[${'WORKERS.JS'.green}]  .. ..`);
        TIMERLOOP1_ERRORLOG && console.log(`[${'WORKERS.JS'.green}]  .. AT COMMANDS READ FROM <${'_ATCOMMANDS_TIMER1'.yellow}> .. ACTIVE AT-COMMAND <${ACTIVE_CMND.length}/${ATCMNDSObj.length}>..`);
        // --------------
                ...
                if (item.FLAG) {
                  ...
                  switch (item.TYPE.toUpperCase()) {
                    ...
                    case "485":
                      ...
                      break;
                    case "DTU-STATE":
                      BYTES = `AT+TST=${item.DTUID},DTU-STATE=${item.DTUID}\r\n`;
                      StrATCMND = `AT+TST=${item.DTUID},DTU-STATE=${item.DTUID}`;
                      break;
                    case "SYSTEM":
 ...
 workers.TDKSENSORSGROUP = function(nInterval){
   ...
   LoopInterval = setInterval(function() {
     ...
     console.log(`[${'WORKERS.JS'.red}] ${time.red} <${'TDKSENSORSGROUP'.green}> AT COMMAND LOOPING SOCKET CONNECTED <${_server.socketArr.length}> TIMER <${nInterval}>`);
     // ---------
     // TO RESET THE JSON OBJECT
     // decoder.RESET_ATCMND();
     // --------------------
     if (_server.socketArr){
       ...
       _server.socketArr.forEach((socket) => {
        ...
        console.log(`[${'WORKERS.JS'.red}] ${time.red} <${'TDKSENSORSGROUP'.green}> GATEWAY=<${_GATEWAYTEXT}> ..`);
       ...
       _data.read("ATCOMMANDS",'_ATCOMMANDS_TIMER2',function(err,ATCMNDSObj) {
         ...
         _data.read("rawData","_485SENSORS",function(err,CMD_485RCV) {
           ...
           // 1. CHECK ON NUMBER OF RETURN 485 SENSORS RESPONSE IN LAST REQUESTS
           // ------------------------------------------------------------------
           DEBUG_CONSOLE  && console.log(`[${'WORKERS.JS'.red}]  .. [1] DATA READ _485SENSORS.JSON <${String(err).toUpperCase().yellow}>`);
           ...
           _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
             // ---------
             DEBUG_CONSOLE  && console.log(`[${'WORKERS.JS'.red}]  .. [2] DATA READ RAWDATA\ATCMND_SENT2.JSON <${String(err).toUpperCase().yellow}>`);
             ...
             // --------                          
              ...
              _data.create("rawData", 'ATCMND_SENT2', ACTIVE_CMND, function (err) {
                // ---------
                if (!err) {
                  DEBUG_CONSOLE  && console.log(`[${'WORKERS.JS'.red}]  .. [3] FILE <${'ATCMND_SENT2'.green}> CREATED <ATCMND_SENT2>.`);
                  DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> .. RAWDATA\\ATCMND_SENT2.JSON FILE CREATED`,()=>{});
                } else {
                  DEBUG_CONSOLE  && console.log(`[${'WORKERS.JS'.red}]  .. [3] FILE <${'ATCMND_SENT2'.green}> COULD BE NOT CREATED`);
                ...
                TDKSENSORSGROUP = setInterval(function() {
                  ...
                  if (item && item.FLAG) {
                    ...
                    // ------------------
                    ATCMND_CONSOLE  && console.log(`[${'WORKERS.JS'.red}] ATCMND >> ${StrATCMND}..`)
                    ATCMND_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] <TDKSENSORSGROUP> ATCMND >> ${StrATCMND} `,()=>{});
                    // ------------------
                    _server.socketArr.forEach((socket,index) => {
                      // ------------------------
                      // FILTER THE GATEWAY WHERE AT-COMMAND IS TO BE SENT
                ...
                // -----------
                DEBUG_CONSOLE  && console.log(`[${'WORKERS.JS'.red}]  .. [4] ATCMND_SENT2 COMPLETE..`);
                DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  [4] ATCMND_SENT2 COMPLETE ..`,()=>{});                
                // ------------
              });
            });
         })
 ...
 workers.STATSANALYSIS = function(nInterval) {
    ...
    LoopInterval = setInterval(function() {
      ...
      Sensor.find({}).sort({date: -1}).exec( (error,sensors) => {
        // ------------------
        let nCOUNTER = 0;
        console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> ... SENSORS IN DATABASE <${sensors.length}> `);
        // -----------------------
