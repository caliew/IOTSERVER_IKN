 ...
 workers.SERVER_STATUS_UPDATE = function(nInterval) {
   ...
   LoopInterval = setInterval(function() {
     ...
     if (today.getMinutes()%30 == 0) {
      ...
      let _day0 = new Date();
      let _day1 = new Date();
      _day0.setHours(today.getHours(),today.getMinutes()-29,0);
      _day1.setHours(today.getHours(),today.getMinutes(),0)
      // ------------------
      // TEAWAREHOUSEALERTS
      // ------------------
      console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} <SERVER_STATUS_UPDATE> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day0.getMinutes()).yellow}.. `);
      _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        console.log(`../TEAWAREHOUSE/ALERTS...DATA FOUND=<${alertsData.length}>...`);
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        console.log(`../_TDK/ALERTS...DATA FOUND=<${alertsData.length}>...`);
        let nameAlerts = alertsData.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
            map[key] = [];
        ...
        if (strMessage2.length > 10) {
          console.log(ObjKEYS,WhatsAppGroup1,strMessage2.length)
