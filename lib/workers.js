     ...
     if (today.getMinutes()%30 == 0) {
      ...
      _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        ...
        ObjKEYS.map((key)=>{
          ...
          strMessage += `${key}..\r${GetAlertsMessage1(Alerts)}`;
        })
        if (strMessage.length > 10) {
          console.log(ObjKEYS,WhatsAppGroup1,strMessage.length)
          decoder.WhatMateMessage(WhatsAppGroup1,strMessage);
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        ...
        ObjKEYS.map((key)=>{
          let Alerts = nameAlerts[key];
          strMessage += `${key}..\r${GetAlertsMessage2(Alerts)}`;
        })
        if (strMessage.length > 10) {
          console.log(ObjKEYS,WhatsAppGroup1,strMessage.length)
          decoder.WhatMateMessage(WhatsAppGroup1,strMessage);
