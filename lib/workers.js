 ...
 */
 ...
 // -----------------------------------
 const AlertsGroup = {
   "WhatsAppGroup0" : 'IKN LoRa IOT Group',
...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  const CONFIG = {
    ...
    REPORTING: {
      ...
      SECONDARY: { weekday: 30, weekend: 60 } // OPS ROOM alerts
    },
  ...
  function processHourlyReport(date, dateStr, timeStr) {
    if (date.getHours() % 1 === 0 && CONFIG.HOURLY_REPORT_MINUTES.includes(date.getMinutes())) {
      ...
      decoders.TelegramGateway10(AlertsGroup.WhatsAppGroup0, `..${timeStr}..\tSERVER STATUS UPDATE..`);
    }
  }
