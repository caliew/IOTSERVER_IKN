 ...
 */
 ...
 workers.TDKSENSORSGROUP = function(nInterval,JSONFilename){
   ...
   LoopInterval = setInterval(function() {
     ...
     // SEND MODBUS CALLING COMMAND TO EACH ACTIVE SOCKETS IN SERVER
     // -------------------
     console.log(`[${'WORKERS.JS'.red}] ${time.blue} <${'TDKSENSORSGROUP'.green}> AT COMMAND LOOPING SOCKET CONNECTED <${_server.socketArr.length}> TIMER <${nInterval}>`);
     // ---------
     // TO RESET THE JSON OBJECT
     // decoder.RESET_ATCMND();
     // --------------------
     ...
     if (_server.socketArr){
       ...
       _server.socketArr.forEach((socket) => {
        ...
        console.log(`[${'WORKERS.JS'.red}] ${time.blue} <${'TDKSENSORSGROUP'.green}> GATEWAY ID = <${String(_GATEWAYTEXT).yellow}> ..`);
        _logs.append('_ERROR',`[WORKERS.JS] ${time} .. `,()=>{});
        _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  GATEWAY ID = <${_GATEWAYTEXT}> ..`,()=>{});
        // ------------------
       // AT COMMANDS TIMER2
       // -------------------
       _data.read("ATCOMMANDS",JSONFilename,function(err,ATCMNDSObj) {
          ...
          // -----------
          console.log(`[${'WORKERS.JS'.red}]  .. ${time.red} AT-COMMANDS SOCKETS = [${String(_ATSOCKETS).yellow}]`);
          _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  AT-COMMANDS SOCKETS = [${String(_ATSOCKETS)}] ..`,()=>{});
          // -----------------
          (_DIFFSOCKETS.length > 0) && console.log(`[${'WORKERS.JS'.red}]  .. ${time.red} *** MISSING SOCKETS <${String(_DIFFSOCKETS).yellow}> *** `);
          (_DIFFSOCKETS.length > 0) && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> *** MISSING SOCKETS <${String(_DIFFSOCKETS)}> ***`,()=>{});
          // -----------------
           ...
           _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
             ...
             // ---------------
              ...
              _data.create("rawData", 'ATCMND_SENT2', ACTIVE_CMND, function (err) {
                ...
                DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. `,()=>{});
                // ------------
                let nSTEP = 1;
                if (ACTIVE_CMND.length == 0) return;
                // -------------------
