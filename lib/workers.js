...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    if (hours%nHOURINT == 0 && minutes == 0) {
      ...
      blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} <SERVER_STATUS_UPDATE> ..<${hours}> CALLING WHATSAPPGATEWAY3..`);
    ...
    if (minutes%nTIMER1 == 0) {
      ...
      _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER1=[${nTIMER1.toString().yellow}]
              <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length}>
              CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..\n`);
    ...
    if (minutes%nTIMER2 == 0) {
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER2=[${nTIMER2.toString().yellow}] 
              <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length.toString().red}>
              CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..`);
        let nameAlerts = alertsData?.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
