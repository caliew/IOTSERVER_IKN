 ...
 */
 ...
 workers.TIMERLOOP1 = function(nInterval){
  ...
  LoopInterval = setInterval(function() {
    ...
    // -------------------
    consoleOutput && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'TIMERLOOP1'.yellow}> ... SOCKET CONNECTED <${_server.socketArr.length}> ...TIMER <${nInterval}>`);
    // ---------------------------------------
    // TO RESET THE _485SENSORS.JSON OBJECT
    ...
    if (_server.socketArr){
      ...
      _data.read("ATCOMMANDS",'_ATCOMMANDS_TIMER1',function(err,ATCMNDSObj) {
        ...
        if(err) {
         ...
         // -----------
         consoleOutput && console.log(`[${'WORKERS.JS'.green}]  .. AT COMMANDS READ FROM <${'_ATCOMMANDS_TIMER1'.yellow}> .. ACTIVE AT-COMMAND <${ACTIVE_CMND.length}/${ATCMNDSObj.length}>..`);
        // --------------
        ...
        _data.read("rawData","_485SENSORS",function(err,CMD_485RCV) {
          ...
          _data.read("rawData",'ATCMND_SENT1',function(err,CMD_SENT) {
            ...
            _data.create("rawData", 'ATCMND_SENT1', ACTIVE_CMND, function (err) {
              // ---------
              if (!err) {
                // console.log(`[${'WORKERS.JS'.green}]  .. FILE <${'ATCMND_SENT1'.green}> DELETED & RE-CREATED...`)
                // _logs.append('_ERROR',`[WORKERS.JS] .. FILE <ATCMND_SENT1> DELETED & RE-CREATED .. `,()=>{});

              } else {
                console.log(`[WORKERS.JS] .. COULD NOT CREATE FILE .. <ATCMND_SENT1> ..`);
                _logs.append('_ERROR',`[WORKERS.JS] COULD NOT CREATE FILE .. <ATCMND_SENT1> ..`,()=>{});
              }
              // -----------
              let nSTEP = 10;
              ...
              ATMCMNDLoopInterval = setInterval(function() {
                ...
                if (item.FLAG) {
                  ...
                  // ------------------------------------------
                  // console.log(`[${'WORKERS.JS'.red}] AT+TXH=${item.DTUID},${item.PREMODBUS ? item.PREMODBUS : '' }${HEXID}${item.MODBUS}${CRC1}`)
                  // --------------------------------
              ...
              // -----------
              consoleOutput && console.log(`[${'WORKERS.JS'.green}]  .. ATCMND_SENT1 COMPLETE..`);
              // -----------
            });
          });
        })
 ...
 workers.TDKSENSORSGROUP = function(nInterval){
   ...
   LoopInterval = setInterval(function() {
     ...
     // SEND MODBUS CALLING COMMAND TO EACH ACTIVE SOCKETS IN SERVER
     // -------------------
     console.log(`[${'WORKERS.JS'.red}] ${date.yellow} ${time.yellow} <${'TDKSENSORSGROUP'.green}> AT COMMAND LOOPING SOCKET CONNECTED <${_server.socketArr.length}> TIMER <${nInterval}>`);
     // ---------
     // TO RESET THE _485SENSORS.JSON OBJECT
     // decoder.RESET_ATCMND();
     // --------------------
     if (_server.socketArr){
       ...
       // --------
      //  _logs.append('_ERROR',`[WORKERS.JS] <${time}> ..TDKSENSORSGROUP..OPENED... <${_server.socketArr.length}> ${_GATEWAYTEXT}`,()=>{});
       // -------------------
       // AT COMMANDS TIMER2
       // -------------------
           ...
           _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
             // ---------
              ...
              _data.create("rawData", 'ATCMND_SENT2', ACTIVE_CMND, function (err) {
                // ---------
                console.log(`[${'WORKERS.JS'.red}]  .. FILE <${'ATCMND_SENT2'.green}> DELETED & RE-CREATED...`)
                // ---------
                if (!err) {
                ...
                TDKSENSORSGROUP = setInterval(function() {
                  ...
                  if (TDKSENSORSGROUP_COUNTER > ACTIVE_CMND.length -1) {
                    ...
                    _logs.append('_ERROR',`[WORKERS.JS] ${_time0}>>${time} .. <TDKSENSORSGROUP> ..  ${_GATEWAYTEXT}`,()=>{});
                    // ----------------------------
                  }
                  // --------------
                },1000 * nSTEP);
                // -----------
                console.log(`[${'WORKERS.JS'.red}]  .. ATCMND_SENT2 COMPLETE..`);
                // ------------
       ...
       sensors.map((sensor,index) => {
         ...
         SensorStats.find(query).exec( (error,_sensorStats) => {
           ...
           // --------------------------
             ...
             nCOUNTER += 1;
             // ---- POWER METER ---
             if (sensorData[0].SENSORTYPE === 'PWR-METER-POWER') {
               // --------
               let nCOUNT = sensorData.length;
               let sensorKey = `${sensorData[0].DTUID}_${sensorData[0].SENSORID} `
               let _dateTime0 = new Date(sensorData[0].TIMESTAMP);
              //  console.log(`${sensorKey} <${nCOUNT}> ...${_dateTime0.getDate()}::${sensorData[0].TIMESTAMP}||${sensorData[0].RCV_BYTES}| >>> ${sensorData[nCOUNT-1].TIMESTAMP}||${sensorData[nCOUNT-1].RCV_BYTES}|...`);
              let _pwrdata = [];
              let _date0  = -1;
             ...
             if (sensor.dtuId === '-1' ) {
               ...
               if (Number.isNaN(factor) && sensor.type ==="PWRMTR(485)") {
                 // ----------------------------------
                 console.log(index,sensor.type,sensor)
                 // ----------------------------------
               }
            ...
            sensorData[0].SENSORTYPE === "PWR-METER-POWER" && ProcessPWRMTR(sensorData);
					})
           // --------------
 		   	})
	     })
 ...
 function ProcessPWRMTR(sensorData) {
   let nCOUNT = sensorData.length;
   ...
   Object.entries(sensorData).map(([index, data]) => {
    ...
    resultObj[`WK${_WeekDay}`] = _READING;
    resultObj[nMONTH+'-'+nDAY] = _READING;
   })
   // -------------------
   var logString = JSON.stringify(resultObj);
   let logFileName = `STAT_${resultObj.DTUID}-${resultObj.SENSORID}`
   _data.create("STATS", logFileName, resultObj, function (err) {
    //  console.log(`..${logFileName} ...${err} ...C R E A T E D ...`)
   })
   // console.log(resultObj)
  //  console.log(`..[${'STATS'.green}].. ...[${strDateTIme1} > ${strDateTIme0}] ..KEY=<${sensorData[0].DTUID}|${sensorData[0].SENSORID}> ... ${PWRMTR0} > ${PWRMTR1} = ${PWRMTR1-PWRMTR0}`);
 }
...
workers.init = function(){
  ...
  // ----------------------
workers.TIMERLOOP1(1);
// ------------
  // TDK SENSORS
  // -----------
  workers.TDKSENSORSGROUP(20);
  // --------------
  // DAY STATISTICS
  // --------------
  workers.STATSANALYSIS(30);
  // -------------S
