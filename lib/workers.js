...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    if (hours%nHOURINT == 0 && minutes == 0) {
      ...
      decoder.WhatMateMessage('IKN LoRa IOT Group',`..${hours}:00..\t${nHOURINT} HOURLY UPDATE..`);
    }
    // ---------------------
    // --- ALERT SUMMARY ---
    ...
    if (minutes%nTIMER1 == 0) {
      ...
      _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        ...
        if (strMessage1.length > 10) {
          console.log(WhatsAppGroup1,strMessage1.length,ObjKEYS);
          decoder.WhatMateMessage(WhatsAppGroup1,strMessage1);
        }
      })
    }
    // -- 20 MIN REPORTING ---
    ...
    if (minutes%nTIMER2 == 0) {
      ...
      // TDK ALERTS
      // ----------
      // console.log(`\n[${'WORKERS.JS'.green}] ..TIMER2=${nTIMER2.toString().yellow}..<${_day1.toTimeString().blue}><${_day0.toTimeString().yellow}>`)
      // -------
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        ...
        if (strMessage2.length > 10) {
          ...
          decoder.WhatMateMessage(WhatsAppGroup2,strMessage2);
        }
      })
    }
    // ---------------
