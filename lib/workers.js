 ...
 */
 ...
 workers.TDKSENSORSGROUP = function(nInterval,JSONFilename) {
   ...
   console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'SCANNING TDKSENSORSGROUP'.cyan}> .. [${nInterval}]MIN ..`);
   let LoopInterval = setInterval(function() {
     // ---------------------------------
     // -------------------
     // SEND MODBUS CALLING COMMAND TO EACH ACTIVE SOCKETS IN SERVER
     ...
     if (_server.socketArr){
       ...
       _data.read("ATCOMMANDS",JSONFilename,function(err,ATCMNDSObj) {
        ...
        if(err) {
          ...
          // --------------------
          blnDEBUG && console.log(`\n[${'WORKERS.JS'.red}] ${time} <${'TDKSENSORSGROUP'.green}> <${JSONFilename.yellow}> TIMER=<${nInterval}> \nSOCKET=<${_server.socketArr.length}> GATEWAY<${String(_GATEWAYTEXT).yellow}>\nREAD FROM <${'_ATCOMMANDS_TIMER2'.yellow}> SOCKETS=[${String(_ATSOCKETS).yellow}|${String(_DIFFSOCKETS).yellow}>  ACTIVE AT-COMMAND <${String(ACTIVE_CMND.length).green}/${String(ATCMNDSObj.length).red}>..\n`);
              ...
              _data.create("rawData", 'ATCMND_SENT2', {...ACTIVE_CMND, time}, function (err) {
                ...
                TDKSENSORSGROUP = setInterval(function() {
                  ...
                  if (TDKSENSORSGROUP_COUNTER > ACTIVE_CMND.length -1) {
                    ...
                    blnDEBUG && console.log(`[${'WORKERS.JS'.red}] ${time} <${'TDKSENSORSGROUP'.green}> AT-COMMAND SENT=<${String(TDKSENSORSGROUP_COUNTER)}|${ACTIVE_CMND.length}>`);
 ...
 function ProcessSTATANALYSIS() {
  ...
  let time = String(today.getHours()).padStart(2,"0") + ":" + String(today.getMinutes()).padStart(2,"0");
  // -------------------
  blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. PROCESSING SENSORS STATS ..`);
  Sensor.find({}).sort({date: -1}).exec( (error,sensors) => {
    // ------------------
    if (Sensor === undefined) {
      blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. SENSORS NOT FOUND IN DATABASE ..`);
      return;
    }
    let nCOUNTER = 0;
    blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. TOTAL SENSORS IN DATABASE <${String(sensors?.length).yellow}> ..`);
    ...
    _data.read('stats','DATA_PWRMTER',function(err,data) {
      if (typeof data !== 'undefined') {
        blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. POWER METER DATA <${String(data?.length).yellow}> ..`);
...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    if (hours%nHOURINT == 0 && (minutes == nMINUTEInt1 || minutes == nMINUTEInt2)) {
      ...
      blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} <SERVER_STATUS_UPDATE> ..SERVER RESPONSE CHECK.. `);
    ...
    if (minutes%nTIMER1 == 0) {
      ...
      false && _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER1=[${nTIMER1.toString().yellow}] <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..\n`);
    ...
    // MON-FRI  19:00-24:00 : 60MIN STATUS UPDATE
    // SAT-SUN  00:00-24:00 : 60MIN STATUS UPDATE
    let nTIMER2 = (hours > 7 && hours < 19) ? ((days > 0 && days < 6) ? 20 : 60 ): 60;
    // -------
    if (minutes%nTIMER2 == 0) {
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        blnDEBUG && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER2=[${nTIMER2.toString().yellow}] <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length.toString().red}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..`);
        let nameAlerts = alertsData?.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
            map[key] = [];
...
workers.SERVER_RESTART = function() {
  ...
  let date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  let time = today.getHours() + ":" + today.getMinutes();  
  console.log(`[${'WORKERS.JS'.green}] ${time}...${String(`SERVER_RESTART ..<${blnDEBUG}>..`).yellow}....`);
  ...
  const calculateNextRestart = () => {
    ...
    const nextHours = Math.ceil(hours/12)*12;
    ...
    if (nextRestart <= now) {
      nextRestart.setHours(nextRestart.getHours() + 12);
    }
    return nextRestart;
  };
  ...
  console.log(`${time}...Next restart scheduled at: ${nextRestartTime}`);
  // FOR FIRST RESTART ...
