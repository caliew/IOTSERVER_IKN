...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    let nRPORTTIMER = (hours > 7 && hours < 19) ? ((days > 0 && days < 6) ? 20 : 60 ): 60;
    // -------
    if (minutes%nRPORTTIMER == 0) {
      let _day0 = new Date();
      let _day1 = new Date();
      _day1.setHours(today.getHours(),today.getMinutes(),0);
      _day0.setHours(today.getHours(),today.getMinutes()-nRPORTTIMER-1,0);
      // --------------
      // TDK IOT ALERTS
      // --------------
