...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    if (minutes%60 == 0) {
      ...
      if (hours%3 == 2) {
        decoder.WhatMateMessage('IKN LoRa IOT Group',`..${today.getMinutes()}..3 HOURLY UPDATE..`);
      }
    }
    // -------------
    // 30 MIN UPDATE
    ...
    // --- 30 MIN REPOTING ---
    let nTotalLines = 100;
    let nTIMER1 = (today.getHours() > 7 && today.getHours() < 19) ? ((today.getDay() > 0 && today.getDay() < 6) ? 30 : 60 ) : 60;
    if (minutes%nTIMER1 == 0) {
    let _day0 = new Date();
    let _day1 = new Date();
    _day1.setHours(today.getHours(),today.getMinutes(),0)
    _day0.setHours(today.getHours(),today.getMinutes()-nTIMER1-1,0);
    // ------------------
    // TEAWAREHOUSEALERTS
    // ------------------
    ...
    // -- 20 MIN REPORTING ---
    let nTIMER2 = (today.getHours() > 7 && today.getHours() < 19) ? ((today.getDay() > 0 && today.getDay() < 6) ? 20 : 60 ): 60;
...
const GetAlertsMessage2 = (Alerts) => {
  ...
  Alerts.map((alert)=>{
    ...
    let _DATE = new Date(alert.TIMESTAMP);
    let _DATETIME = `${_DATE.getHours()}:${_DATE.getMinutes()}`;
    strTEXT += `${_DATETIME} ${alert.sensorType} ${alert.VALUE} ${alert.sign} ${alert.LIMITS}%\r`;
  })
  return strTEXT;
}
// -------
