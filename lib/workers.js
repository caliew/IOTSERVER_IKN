  ...
  let LoopInterval = setInterval(function() {
    ...
    if (minutes%nTIMER2 == 0) {
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        blnDEBUG && console.log(`\n[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER2=[${nTIMER2.toString().yellow}] <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length.toString().red}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..`);
        let nameAlerts = alertsData?.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
            map[key] = [];
