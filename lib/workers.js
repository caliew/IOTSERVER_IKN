  ...
  let LoopInterval = setInterval(function() {
    ...
    if (minutes%nTIMER1 == 0) {
      ...
      false && _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        blnDEBUG && console.log(`\n[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER1=[${nTIMER1.toString().yellow}] <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..\n`);
