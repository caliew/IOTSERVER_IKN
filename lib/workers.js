 ...
 */
 ...
 function ProcessSTATANALYSIS() {
  ...
  let time = String(today.getHours()).padStart(2,"0") + ":" + String(today.getMinutes()).padStart(2,"0");
  // -------------------
  console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. PROCESSING SENSORS STATS ..`);
  Sensor.find({}).sort({date: -1}).exec( (error,sensors) => {
    // ------------------
    if (Sensor === undefined) {
      console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. SENSORS NOT FOUND IN DATABASE ..`);
      return;
    }
    let nCOUNTER = 0;
    ...
    _data.read('stats','DATA_PWRMTER',function(err,data) {
      if (typeof data !== 'undefined') {
        console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. POWER METER DATA <${String(data?.length).yellow}> ..`);
