...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    console.log(`[${'WORKERS.JS'.green}] DAYS=[${days}]-HOURS=[${hours}]-MINUTES=[${minutes}]..  ..TIMER1=${nTIMER1} ..TIMER2=${nTIMER2}..`)
    if (minutes%nTIMER2 == 0) {
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} TIMER=${nTIMER2} <SERVER_STATUS_UPDATE> 
              CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}.. DATA FOUND=<${alertsData.length}>...`);
        let nameAlerts = alertsData.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
        ...
        if (strMessage2.length > 10) {
          console.log(WhatsAppGroup2,strMessage2.length,ObjKEYS);
...
const GetAlertsMessage2 = (Alerts) => {
  let strTEXT = '';
  getSymbol = (type) => { return type.toUpperCase.includes('TEMP') ? 'C' : type.toUpperCase.includes('RH') ? '%' : ''}
  Alerts.map((alert)=>{
    let _FLAG = alert.FLAG;
    let _DATE = new Date(alert.TIMESTAMP);
    let _DATETIME = `${_DATE.getHours()}:${_DATE.getMinutes()}`;
    strTEXT += `${_DATETIME} ${alert.sensorType} ${alert.VALUE} ${alert.sign} ${alert.LIMITS} ${getSymbol(alert.sensorType)}\r`;
  })
  return strTEXT;
}
// -------
