...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    let strTEXT = `<${'SERVER STATUS UPDATE'.green}> ${date.yellow} ${time.green}`;
    // -------------
    // 3 HOURLY STATUS REPORTING
    // -------------
    let nHOURINT = 1;
    let nMINUTEInt1 = 0;
    let nMINUTEInt2 = 30;
    if (hours%nHOURINT == 0 && (minutes == nMINUTEInt1 || minutes == nMINUTEInt2)) {
      // ---------------------
      // if (minutes%10 == 0) {
      // -------
      // String(today.getMinutes()).padStart(2,"0")
      _DEBUG_SERVERSTATUSUPDATE && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} <SERVER_STATUS_UPDATE> ..SERVER RESPONSE CHECK.. `);
      decoders.TelegramGateway(AlertsGroup.WhatsAppGroup0,`..${time}..\tSERVER STATUS UPDATE..`);
    }
    if (hours == 5 && minutes == 0) {
      const WHATSAPPGROUP = 'IKN OPS ROOM';
      const strMessage = 'Salam sejahtera team,Makluman awal untuk bacaan GOT 1, GOT 2, BOT 1 & BOT 2 sebelum operasi bermula. Mohon semua beri perhatian dan ambil tindakan segera jika ada bacaan luar julat atau alert yang dikesan.Kerjasama semua amat dihargai. TQ..';
      decoders.WhatsAppGateway2(WHATSAPPGROUP,`..${time}..\t${strMessage}`);
      console.log(`ALERT SENT TO ${WHATSAPPGROUP} HOUR=${hours} MIN=${minutes}`)
    }
    // ---------------------
    let nTotalLines = 100;
    // -- 20 MIN REPORTING ---
    // MON-FRI  07:00-19:00 : 20MIN STATUS UPDATE
    // MON-FRI  19:00-24:00 : 60MIN STATUS UPDATE
    // SAT-SUN  00:00-24:00 : 60MIN STATUS UPDATE
    let nRPORTTIMER1 = (hours > 7 && hours < 19) ? ((days > 0 && days < 6) ? 20 : 60 ): 60;
    // -- 20 MIN REPORTING ---
    // MON-FRI  07:00-19:00 : 30MIN STATUS UPDATE
    // MON-FRI  19:00-24:00 : 60MIN STATUS UPDATE
    // SAT-SUN  00:00-24:00 : 60MIN STATUS UPDATE
    let nRPORTTIMER2 = (hours > 7 && hours < 19) ? ((days > 0 && days < 6) ? 30 : 60 ): 60;
    // -------
    if (minutes%nRPORTTIMER1 == 0) {
      let _day0 = new Date();
      let _day1 = new Date();
      _day1.setHours(today.getHours(),today.getMinutes(),0);
      _day0.setHours(today.getHours(),today.getMinutes()-nRPORTTIMER1-1,0);
      // --------------
      // TDK IOT ALERTS
      // --------------
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        let nameAlerts = alertsData?.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
            map[key] = [];
            map[key].push(alert);
        ...
        // --------
        let strMessage2 = '';
        let ObjKEYS = Object.keys(nameAlerts);
        ObjKEYS.map((key)=>{
          let Alerts = nameAlerts[key];
          strMessage2 += `${key}\n\r${GetAlertsMessage2(Alerts)}\n`;
        })
        _DEBUG_SERVERSTATUSUPDATE && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..<_TDKALERTS>.. TIMER2=[${nRPORTTIMER1.toString().yellow}]\n <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length.toString().red}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..`);
        _DEBUG_SERVERSTATUSUPDATE && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..<_TDKALERTS>.. [${strMessage2.length}] ..${strMessage2}`);
        if (strMessage2.length > 10) {
          decoders.TelegramGateway(AlertsGroup.WhatsAppGroup2,strMessage2);
        }
      })
    }
    if (minutes%nRPORTTIMER2 == 0) {
      let _day0 = new Date();
      let _day1 = new Date();
      _day1.setHours(today.getHours(),today.getMinutes(),0);
      _day0.setHours(today.getHours(),today.getMinutes()-nRPORTTIMER1-1,0);
      ...
      // TDK IOT ALERTS
      // --------------
      _logs.read('_IKN_OPROOMALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        let nameAlerts = alertsData?.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
        ...
        if (strMessage2.length > 10) {
          const WHATSAPPGROUP = 'IKN OPS ROOM';
          decoders.WhatsAppGateway2(WHATSAPPGROUP,strMessage2);
        }
      })
    }
    // ---------------
...
const GetAlertsMessage2 = (Alerts) => {
  ...
  Alerts.map((alert)=>{
    let _FLAG = alert.FLAG;
    let _DATE = new Date(alert.TIMESTAMP);
    getSymbol = () => { return alert.sensorType.toUpperCase().includes('TEMP') ? 'C' : alert.sensorType.toUpperCase().includes('RH') ? '%' : ''}
    let _DATETIME = `${_DATE.getHours()}:${_DATE.getMinutes()}`;
    count += 1;
    if (count < 3) strTEXT += `${_DATETIME} ${alert.sensorType} ${alert.VALUE} ${alert.sign} ${alert.LIMITS} ${getSymbol()}\n\r`;
    if (count == 4) strTEXT += `..`;
  })
  return strTEXT;
}
