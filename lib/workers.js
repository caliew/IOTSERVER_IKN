...
workers.SERVER_RESTART = function() {
  ...
  const calculateNextRestart = () => {
    ...
    nextRestart.setHours(Math.ceil(now.getHours() / 3) * 3);
    ...
    if (nextRestart <= now) {
      nextRestart.setHours(nextRestart.getHours() + 3);
    }
    return nextRestart;
  };
  ...
  console.log(`${time}...Next restart scheduled at: ${nextRestartTime.toLocaleTimeString()}`);
