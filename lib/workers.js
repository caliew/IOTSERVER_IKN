...
const nTIMER2 = 10;    // TDKSENSORSGROUP
const nTIMER3 = 20;    //  STATSANALYSIS
// ----------------------
const flagTIMER1 = false;  // DEMO TEST SITE NIPPONGLASS
const flagTIMER2 = true;   // TDK SENSORS
// ---------------------
const ATCMND_CONSOLE = false;
const ATCMND_ERRORLOG = true;
const DEBUG_ERRORLOG = true;
const DEBUG_INTOUTPUT = false;
// -----------------------------------
const ATCMNDFILES = ['_ATCOMMANDS_TDK', '_ATCOMMANDS_TDK_CX'];
const timer2FileName = ATCMNDFILES[0];
// -----------------------------------
const AlertsGroup = {
  "TelegramGroup0": 'LoRa IOT Group',
  "WhatsAppGroup0": 'LoRa IOT Group',
  "WhatsAppGroup1": 'BALAKONG茶仓温控监管中心',
  "WhatsAppGroup2": 'TDK IOT Alert Group',
  "WhatsAppGroup3": 'Shinko IOT Monitoring System'
}
// ----------------------------------- 
...
//  --------------------------------------
const TIMERLOOP1_ERRORLOG = true;
workers.TIMERLOOP1 = function (nInterval) {
  ...
  let LoopInterval = setInterval(function () {
    ...
    // -------------------
    TIMERLOOP1_ERRORLOG && console.log(`[${'WORKERS.JS'.green}] ${date} ${time.yellow} <${'TIMERLOOP1'.red}> ... SOCKET CONNECTED <${_server.socketArr.length}> ...TIMER <${nInterval}>`);
    // ---------------------------------------
    // TO RESET THE JSON OBJECT
    // decoders.RESET_ATCMND();    
    // -----------------------
    if (_server.socketArr) {
      ...
      _data.read("ATCOMMANDS", '_ATCOMMANDS_TIMER1', function (err, ATCMNDSObj) {
        ...
        // -----------
        TIMERLOOP1_ERRORLOG && console.log(`[${'WORKERS.JS'.green}]  .. AT COMMANDS READ FROM <${'_ATCOMMANDS_TIMER1'.yellow}> .. ACTIVE AT-COMMAND <${String(ACTIVE_CMND.length).green}/${String(ATCMNDSObj.length).yellow}>..`);
        // --------------
          ...
          _data.read("rawData", 'ATCMND_SENT1', function (err, CMD_SENT) {
            ...
            _data.create("rawData", 'ATCMND_SENT1', ACTIVE_CMND, function (err) {
              ...
              if (!err) {
                ...
                TIMERLOOP1_ERRORLOG && _logs.append('_ERROR', `[WORKERS.JS] <TIMERLOOP1> COULD NOT CREATE FILE .. <ATCMND_SENT1> ..`, () => { });
              }
              // -----------
              let nSTEP = 10;
              if (ACTIVE_CMND.length == 0) return;
              // ------------------
              let ATMCMNDLoopInt = setInterval(function () {
                // ---------------------------------
                let BYTES = "";
                let StrATCMND = "";
                ...
                if (item.FLAG) {
                  ...
                  switch (item.TYPE.toUpperCase()) {
                    ...
                    case "485":
                      ...
                      let CHECKCMND = `${item.PREMODBUS ?? ''}${HEXID}${item.MODBUS}`;
                      // CRC1 = crc16.checkSum( `${item.PREMODBUS ?? '' }${HEXID}${item.MODBUS}`).toUpperCase();
                      // BYTES = `AT+TXH=${item.DTUID},${HEXID + item.MODBUS + CRC}\r\n`;
                      BYTES = `AT+TXH=${item.DTUID},${item.PREMODBUS ?? ''}${HEXID}${item.MODBUS}${CRC1}\r\n`;
                      StrATCMND = `AT+TXH=${item.DTUID},${item.PREMODBUS ?? ''}${HEXID}${item.MODBUS}${CRC1}`;
                      break;
                    case "DTU-STATE":
                      BYTES = `AT+TST=${item.DTUID},DTU STATE CHECK`;
                      StrATCMND = `AT+TST=${item.DTUID},DTU STATE CHECK`;
                      break;
                    case "SYSTEM":
                  ...
                  TIMERLOOP1_ERRORLOG && _logs.append('_ERROR', `[WORKERS.JS] <TIMERLOOP1> ${StrATCMND} `, () => { });
                  // --------------------------------
                ...
                if (TIMERLOOP1_COUNTER > ACTIVE_CMND.length - 1) {
                  // --------------------------------
                  clearInterval(ATMCMNDLoopInt);
                  // --------------------------------
                }
                // --------------
              }, 1000 * nSTEP);
              // -----------
              TIMERLOOP1_ERRORLOG && console.log(`[${'WORKERS.JS'.green}]  .. ATCMND_SENT1 COMPLETE..`);
              // -----------
            });
          });
        })
...
// ---------------------
const _DEBUG_TDKSENSORGroup = false;
workers.TDKSENSORSGROUP = async function (nInterval, JSONFilename) {
  // --------------
  let today = new Date();
  let date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  let time = String(today.getHours()).padStart(2, "0") + ":" + String(today.getMinutes()).padStart(2, "0");
  _DEBUG_TDKSENSORGroup && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'SCANNING TDKSENSORSGROUP'.cyan}> INTERVAL=[${String(nInterval).blue}] MIN`);
  // -------------
  let LoopInterval = setInterval(async function () {
    // -------------------
    decoders.RESET_ATCMND();
    // -----------------------
    let _GATEWAYIDArr = [];
    let _GATEWAYTEXT = `..(${String(_server.socketArr.length)})..|.`;
    if (_server.socketArr) {
      _server.socketArr
        .filter((socket) => socket.GATEWAYID && socket.ADDRESS)
        .forEach((socket) => {
          _GATEWAYIDArr.push(String(socket.GATEWAYID));
          _GATEWAYTEXT += `${socket.GATEWAYID}.|.`;
        });
    }
    const ATCMNDSObj = await new Promise((resolve, reject) => {
      _data.read("ATCOMMANDS", JSONFilename, (err, data) => {
        if (err) reject(err);
        else resolve(data);
      });
    });
    ...
    // ABSTRACT ACIVE COMMANDS
    // -----------------------
    let ACTIVE_CMND = ATCMNDSObj.filter(_CMD => { return _CMD.FLAG && _CMD.MODBUS });
    // -------------------   --------------------------
    // CHECK IF SOCKET IS CONNECTED FOR AT COMMAND LIST
    // -------------------   --------------------------
    let _ATSOCKETS = [];
    ACTIVE_CMND.forEach((elm, index) => {
      if (!_ATSOCKETS.includes(String(elm.GATEWAYID))) _ATSOCKETS.push(String(elm.GATEWAYID));
    })
    // -----------
    let _DIFFSOCKETS = _ATSOCKETS.filter(item => !_GATEWAYIDArr.includes(item));
    // -----------
    false && _logs.append('_ERROR', `[WORKERS.JS] ${time} <${JSONFilename}> GATEWAY= ${_GATEWAYTEXT}> TIMER<${nInterval}>.. SOCKETS=<${_server.socketArr.length}`, () => { });
    if (_DIFFSOCKETS.length == _ATSOCKETS.length) return;
    // --------------------
    time = String(today.getHours()).padStart(2, "0") + ":" + String(today.getMinutes()).padStart(2, "0");
    _DEBUG_TDKSENSORGroup && console.log(`[${'WORKERS.JS'.red}] ${time} 
      <${'TDKSENSORSGROUP'.green}> <${JSONFilename.yellow}> TIMER=<${nInterval}> SOCKET=[${_server.socketArr.length}]
      GATEWAY<${String(_GATEWAYTEXT).yellow}> READ FROM <${'_ATCOMMANDS_TIMER2'.yellow}> 
      SOCKETS=[${String(_ATSOCKETS).yellow}|${String(_DIFFSOCKETS).yellow}> ACTIVE AT-COMMAND <${String(ACTIVE_CMND.length).green}/${String(ATCMNDSObj.length).red}>..`);
    false && logs.append('_ERROR', `[WORKERS.JS] ${time} <${JSONFilename}> SOCKETS=<${String(_ATSOCKETS)}|${String(_DIFFSOCKETS)}> .AT-COMMAND=<${ACTIVE_CMND.length}|${ATCMNDSObj.length}>`, () => { });
    // --------------
    if (ACTIVE_CMND.length == 0) {
      return;
    }
    // ----------------------------
    // SENDING AT COMMAND WITH LOOP
    // ----------------------------
    const nSTEP = 1;
    let TIMER_TDKSENSORSGROUP
    let TDKSENSORSGROUP_COUNTER = 0;
    async function sendNextCommand() {
      if (TDKSENSORSGROUP_COUNTER >= ACTIVE_CMND.length) {
        clearInterval(TIMER_TDKSENSORSGROUP);
        return;
      }
      const item = ACTIVE_CMND[TDKSENSORSGROUP_COUNTER];
      if (item && item.FLAG) {
        const BYTES = getBYTES(item);
        await Promise.all(_server.socketArr.map((socket, index) => {
          if (socket.GATEWAYID == item.GATEWAYID && socket.SOCKET) {
      ...
      if (TDKSENSORSGROUP_COUNTER === ACTIVE_CMND.length) {
        false && _DEBUG_TDKSENSORGroup && console.log(`[${'WORKERS.JS'.red}] ${time} <${'TDKSENSORSGROUP'.green}> AT-COMMAND SENT=<${String(TDKSENSORSGROUP_COUNTER)}|${ACTIVE_CMND.length}>`);
      }
    }
    TIMER_TDKSENSORSGROUP = setInterval(sendNextCommand, 1000 * nSTEP);
    // -----------------------
    workers.gatherAllChecks();
    // -----------------------
  }, 1000 * 60 * nInterval);
...
function getBYTES(item) {
  ...
  switch (item.TYPE.toUpperCase()) {
    ...
    case "485":
      ...
      // CRC = crc16.checkSum(`${item.PREMODBUS ? item.PREMODBUS : '' }${HEXID}${item.MODBUS}`).toUpperCase();
      BYTES = `AT+TXH=${item.DTUID},${item.PREMODBUS ? item.PREMODBUS : ''}${HEXID}${item.MODBUS}${CRC}\r\n`;
      StrATCMND = `AT+TXH=${item.DTUID},${item.PREMODBUS ? item.PREMODBUS : ''}${HEXID}${item.MODBUS}${CRC}`;
      break;
    case "DTU-STATE":
      BYTES = `AT+TST=${item.DTUID},DTU-STATE CHECK\r\n`;
      StrATCMND = `AT+TST=${item.DTUID},DTU-STATE CHECK`;
...
const _DEBUG_PROCESSSTATSANALYSIS = false;
function ProcessSTATANALYSIS() {
  let today = new Date();
  let date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  let time = String(today.getHours()).padStart(2, "0") + ":" + String(today.getMinutes()).padStart(2, "0");
  // -------------------
  _DEBUG_PROCESSSTATSANALYSIS && console.log('..PROCESS STATS ANALYSIS...')
  _DEBUG_PROCESSSTATSANALYSIS && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. PROCESSING SENSORS STATS ..`);
  Sensor.find({ type: "PWRMTR(485)" }).sort({ date: -1 }).exec((error, sensors) => {
    // ------------------
    if (Sensor === undefined) {
      _DEBUG_PROCESSSTATSANALYSIS && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. SENSORS NOT FOUND IN DATABASE ..`);
      return;
    }
    let nCOUNTER = 0;
    _DEBUG_PROCESSSTATSANALYSIS && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. TOTAL SENSORS IN DATABASE <${String(sensors?.length).yellow}> ..`);
    _logs.append('_STATLOGS', `[WORKERS.JS] ${date} ${time} .. <TOTAL SENSORS IN DATABASE> ..  <${String(sensors?.length)}>`, () => { });
    // -------
    sensors && sensors.map((sensor, index) => {
      ...
      SensorStats.find(query).exec((error, _sensorStats) => {
        ...
        let _day0 = new Date();
        let _day1 = new Date();
        _day1.setHours(today.getHours(), today.getMinutes(), 0);
        _day0.setMonth(today.getMonth() - 6);
        // --------------------------
        _logs.read(key, totalLines, _day0, _day1, false, function (err, sensorData) {
          // -------------
          // DTUID = -1 -> WISENSOR
          // DTUID > 0  -> 485 SENSORS
          // -------------------------
          ...
          let sensorKey = null;
          if (!err) {
            // _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <STATSANALYSIS> ..  FILE NOT FOUND=<${key}> ..`,()=>{});
            return;
          }
          // ---- POWER METER ---
          if (sensor.type === 'PWRMTR(485)') {
            // --------
            sensorKey = `${sensor.dtuId}_${sensor.sensorId}`;
            // ------
            let _pwrdata = [];
            let _date0 = -1;
            // --------------
          ...
          // ---------------------------------------
          let sensorName = '';
          if (sensorKey) { sensorName = getSensorName(sensors, sensor.dtuId, sensor.sensorId); }
...
function ProcessSENSORS(sensorKey, sensorName, sensorData, type, dtuId, sensorId) {
  ...
  // ------------
  _data.read('STATS', logFileName, function (err, ObjState) {
    let newObj = { ...ObjState, ...resultObj };
    _data.update("STATS", logFileName, newObj, function (err) { })
  })
}
function ProcessPWRMTR(sensorKey, sensorName, sensorData, type) {
  ...
  // --------
  if (sensorKey === '1_1') {
    _logs.append('_STATLOGS', `[${sensorKey}.${sensorName}..${type}`, () => { });
    _logs.append('_STATLOGS', `${JSON.stringify(sensorData[0])}`, () => { });
    _logs.append('_STATLOGS', `${JSON.stringify(sensorData[sensorData.length - 1])}`, () => { });
  }
  // console.log(`[${'WORKERS.JS'.green}] .. PROCESSING SENSORS STATS ..KEY=${sensorKey}.. NAME=${sensorName}.. TYPE=${type} ..DATA=${sensorData?.length}`);
  // --------
  sensorData.forEach((item, index) => {
    let _date = new Date(item.TIMESTAMP);
    if (isToday(_date)) {
...
// SERVER STATUS 
// -------------
const _DEBUG_SERVERSTATUSUPDATE = false;
workers.SERVER_STATUS_UPDATE = function (nInterval) {
  // Constants
  const CONFIG = {
    WORK_HOURS: { start: 5, end: 19 },
    WEEKDAYS: [1, 2, 3, 4, 5], // Monday to Friday
    REPORTING: {
      PRIMARY: { weekday: 20, weekend: 60 }, // TDK alerts
      SECONDARY: { weekday: 30, weekend: 60 } // OPS ROOM alerts 30-10
    },
    MORNING_ALERT: { hour: 5, minute: 0 },
    HOURLY_REPORT_MINUTES: [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55],
    MAX_ALERT_LINES: 100
  };
  ...
  const monitorInterval = setInterval(() => {
    ...
    processHourlyReport(now, dateStr, timeStr);
    processMorningAlert(now, timeStr);

    // Process dynamic interval alerts
    const intervals = calculateReportingIntervals(now);
    processTdkAlerts(now, intervals.primary, dateStr, timeStr);
    processOpsRoomAlerts(now, intervals.secondary, dateStr, timeStr);
  ...
  function calculateReportingIntervals(date) {
    const hour = date.getHours();
    const isWorkHours = hour >= CONFIG.WORK_HOURS.start && hour <= CONFIG.WORK_HOURS.end;
    const isWeekday = CONFIG.WEEKDAYS.includes(date.getDay());

    return {
      primary: isWorkHours && isWeekday ? CONFIG.REPORTING.PRIMARY.weekday : CONFIG.REPORTING.PRIMARY.weekend,
      secondary: isWorkHours && isWeekday ? CONFIG.REPORTING.SECONDARY.weekday : CONFIG.REPORTING.SECONDARY.weekend
    };
  }

  function processHourlyReport(date, dateStr, timeStr) {
    if (date.getHours() % 1 === 0 && CONFIG.HOURLY_REPORT_MINUTES.includes(date.getMinutes())) {
      _DEBUG_SERVERSTATUSUPDATE && console.log(
        `[${'WORKERS.JS'.green}] ${dateStr.yellow} ${timeStr.green} ` +
        `<SERVER_STATUS_UPDATE> ..SERVER RESPONSE CHECK..`
      );
      // EXPIRED
      // decoders.TelegramGateway10(AlertsGroup.TelegramGroup0, `..${timeStr}..\tSERVER STATUS UPDATE..`);
      decoders.Telegrambot(AlertsGroup.TelegramGroup0, `..${timeStr}..\tSERVER STATUS UPDATE..`);
      // decoders.Telegrambot("MRE IOT Monitoring System", `..${timeStr}..\tSERVER STATUS UPDATE..`);
    }
  }

  function processMorningAlert(date, timeStr) {
    if (date.getHours() === CONFIG.MORNING_ALERT.hour &&
      date.getMinutes() === CONFIG.MORNING_ALERT.minute) {
      const message = 'Salam sejahtera team,Makluman awal untuk bacaan GOT 1, GOT 2, BOT 1 & BOT 2 sebelum operasi bermula. Mohon semua beri perhatian dan ambil tindakan segera jika ada bacaan luar julat atau alert yang dikesan.Kerjasama semua amat dihargai. TQ..';
      decoders.WhatsAppGateway28('IKN OPS ROOM', `..${timeStr}..\t${message}`);
      console.log(`ALERT SENT TO IKN OPS ROOM AT ${timeStr}`);
    }
  }

  function processTdkAlerts(date, interval, dateStr, timeStr) {
    if (date.getMinutes() % interval !== 0) return;

    const { startTime, endTime } = createTimeRange(date, interval);

    _logs.read('_TDKALERTS', CONFIG.MAX_ALERT_LINES, startTime, endTime, false, (err, alertsData) => {
      if (err || !alertsData?.length) return;

      const groupedAlerts = groupAlertsByName(alertsData);
      const alertMessage = formatAlertMessage(groupedAlerts);

      logAlertDetails('_TDKALERTS', alertsData.length, startTime, endTime, alertMessage, interval, dateStr, timeStr);

      if (alertMessage.length > 10) {
        decoders.TelegramGateway10(AlertsGroup.WhatsAppGroup2, alertMessage);
      }
    });
  }

  function processOpsRoomAlerts(date, interval, dateStr, timeStr) {
    if (date.getMinutes() % interval !== 0) return;

    const { startTime, endTime } = createTimeRange(date, interval);

    _logs.read('_IKN_OPROOMALERTS', CONFIG.MAX_ALERT_LINES, startTime, endTime, false, (err, alertsData) => {

      // Filter alerts by time range first
      const filteredAlerts = alertsData.filter(alert => {
        const alertTime = new Date(alert.TIMESTAMP).getTime();
        return alertTime >= startTime.getTime() && alertTime <= endTime.getTime();
      });

      _DEBUG_SERVERSTATUSUPDATE && console.log('Time range:', startTime.toISOString(), 'to', endTime.toISOString());
      _DEBUG_SERVERSTATUSUPDATE && console.log('Alerts within time range:', filteredAlerts.length);
      if (!filteredAlerts.length) return;


      // Then process the filtered alerts
      const groupedAlerts = groupAlertsByName(filteredAlerts);
      const alertMessage = formatAlertMessage(groupedAlerts);
      logAlertDetails('_IKN_OPROOMALERTS', alertsData.length, startTime, endTime, alertMessage, interval, dateStr, timeStr);

      if (filteredAlerts.length > 0) {
        decoders.WhatsAppGateway28('IKN OPS ROOM', alertMessage);
      }
    });
  }

  function createTimeRange(date, interval) {
    const endTime = new Date(date);
    const startTime = new Date(date.getTime() - (interval + 1) * 60000);
  ...
  function groupAlertsByName(alertsData) {
    return alertsData.reduce((map, alert) => {
      (map[alert.NAME] ||= []).push(alert);
      return map;
    }, {});
  }

  function formatAlertMessage(groupedAlerts) {
    return Object.entries(groupedAlerts)
      .map(([name, alerts]) => `${name}\n\r${GetAlertsMessage2(alerts)}\n`)
      .join('');
  }

  function logAlertDetails(alertType, count, startTime, endTime, message, interval, dateStr, timeStr) {
    if (!_DEBUG_SERVERSTATUSUPDATE) return;

    const timeRange = `FROM=${String(startTime.getHours()).yellow}:${String(startTime.getMinutes()).yellow} ` +
      `TO=${String(endTime.getHours()).yellow}:${String(endTime.getMinutes()).yellow}`;

    console.log(
      `[${'WORKERS.JS'.green}] ${dateStr.yellow} ${timeStr.green} ` +
      `..<${alertType}>.. TIMER=[${interval.toString().yellow}]\n` +
      `<SERVER_STATUS_UPDATE> DATA FOUND=<${count.toString().red}> ` +
      `CHECK ALERTS SUMMARY ..${timeRange}..`
    );
    console.log(
      `[${'WORKERS.JS'.green}] ${dateStr.yellow} ${timeStr.green} ` +
      `..<${alertType}>.. [${message.length}] ..${message}`
    );
  }
};

// -------
const GetAlertsMessage1 = (Alerts) => {
  let strTEXT = '';
  Alerts.map((alert) => {
    let _FLAG = alert.FLAG;
    let _DATE = new Date(alert.TIMESTAMP);
    let _DATETIME = `${_DATE.getHours()}:${_DATE.getMinutes()}`;
    if (_FLAG[0] > 0) strTEXT += `${_DATETIME} 温度${alert.TEMP}C > ${alert.LIMITS.Temperature.max}C\r`;
    if (_FLAG[1] > 0) strTEXT += `${_DATETIME} 温度${alert.TEMP}C < ${alert.LIMITS.Temperature.min}C\r`;
    if (_FLAG[2] > 0) strTEXT += `${_DATETIME} 湿度${alert.RH}% > ${alert.LIMITS.Humidity.max}%\r`;
    if (_FLAG[3] > 0) strTEXT += `${_DATETIME} 湿度${alert.RH}% < ${alert.LIMITS.Humidity.min}%\r`;
    if (_FLAG[4] > 0) strTEXT += `${_DATETIME} 二氧化碳${alert.CO2}ppm > ${alert.LIMITS.CO2.max}%\r`;
    if (_FLAG[5] > 0) strTEXT += `${_DATETIME} 二氧化碳${alert.CO2}ppm < ${alert.LIMITS.CO2.min}%\r`;
  })
  return strTEXT;
}
const GetAlertsMessage2 = (Alerts) => {
  ...
  Object.entries(alertsByType).forEach(([type, typeAlerts]) => {
    ...
    // Add up to 3 most recent alerts
    typeAlerts.slice(0, 3).forEach(alert => {
      const _DATE = new Date(alert.TIMESTAMP);
      const _DATETIME = `${String(_DATE.getHours()).padStart(2, '0')}:${String(_DATE.getMinutes()).padStart(2, '0')}`;
      strTEXT += `${_DATETIME} ${alert.MESSAGE}\n\r`;
    });

    // Add ellipsis if there are more alerts
    if (typeAlerts.length > 2) {
      strTEXT += `...\n\r`;
    }
...
workers.restartApp = function () {
  ...
  // Append to the _ERROR log file
  _logs.append('_ERROR', `[WORKERS.JS] <${time}> [RESTARTAPP]....\n`, () => { });
  // --------
  let baseDir = path.join(__dirname, '/../');
  fs.open(baseDir + 'restartlog.json', 'a+', function (err, fileDescriptor) {
    if (!err && fileDescriptor) {
      ...
      // Append the new data
      fs.write(fileDescriptor, stringData + '\n', null, 'utf8', (err) => {
        if (!err) {
          fs.close(fileDescriptor, function (err) {
            if (err) {
              console.error("Error closing file after writing.", err);
            }
          });
        } else {
...
//   RESTART THE SERVER
// --------------------
const _DEBUG_RESTARTSERVER = false;
workers.SERVER_RESTART = function () {
  // --------------------
  let today = new Date();
  let date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  let time = today.getHours() + ":" + today.getMinutes();
  console.log(`[${'WORKERS.JS'.green}] ${time}  <${'SERVER_RESTART'.red}> STATUS=${String(_DEBUG_RESTARTSERVER).toUpperCase().yellow}`);
  if (!_DEBUG_RESTARTSERVER)
    return;
  // --------
  _logs.append('_ERROR', `  <${time}> .SERVER INITIALIZED....`, () => { { } });
  // --------------------
  // Calculate the time until the next restart (next hour that is a multiple of 3)
  const calculateNextRestart = () => {
    const now = new Date();
    const nextRestart = new Date(now);
    const minutes = now.getMinutes();
    const hours = now.getHours();
    const nextHours = Math.ceil(hours / 1) * 1;
    // Round up to the next multiple of 12 hours
    nextRestart.setHours(nextHours);
    nextRestart.setMinutes(0);
    nextRestart.setSeconds(0);
    nextRestart.setMilliseconds(0);
    // If the next restart time is in the past, move to the next 3-hour interval
    if (nextRestart <= now) {
      nextRestart.setHours(nextRestart.getHours() + 1);
    }
    return nextRestart;
  };
  // Calculate the delay until the next restart
  const nextRestartTime = calculateNextRestart();
  const delay = nextRestartTime - new Date();
  console.log(`             SCHEDULE RESTART AT ${nextRestartTime}`);
  // ---------------------
  // FOR FIRST RESTART ...
  // Set a timeout to restart the server at the calculated time
  setTimeout(function () {
    console.log(`${nextRestartTime.toLocaleTimeString()}...INIT RESTART BACKGROUND TASK...`);
    _DEBUG_RESTARTSERVER && workers.restartApp();
    // After the first restart, set the interval for subsequent restarts every 3 hours
    setInterval(function () {
      console.log(`${new Date().toLocaleTimeString()}...INIT RESTART BACKGROUND TASK...`);
      _DEBUG_RESTARTSERVER && workers.restartApp();
    }, 1000 * 60 * 60 * 12);
  }, delay);
};
...
workers.init = function () {
  ...
  let date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  let time = String(today.getHours()).padStart(2, "0") + ":" + String(today.getMinutes()).padStart(2, "0");
  console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} <${'INIT BACKGROUND WORKERS'.yellow}>`);
  // ----------------------------------
  ...
  // TDK SENSORS
  // -----------
  false && flagTIMER2 && workers.TDKSENSORSGROUP(nTIMER2, timer2FileName);
  // --------------
  // DAY STATISTICS
  // --------------
  ...
  // --------------------------------
  // POWER TRACKER OFFLINE MONITORING
  // --------------------------------
  setInterval(function () {
    powerTracker.checkOfflineStatus(30);
  }, 1000 * 60 * 5);
  // ------
