 ...
 */
 ...
 // ---------------------
 const _DEBUG_TDKSENSORGroup = true;
 workers.TDKSENSORSGROUP = async function(nInterval,JSONFilename) {
   ...
   let LoopInterval = setInterval(async function() {
    ...
    // --------------------
    _DEBUG_TDKSENSORGroup && console.log(`[${'WORKERS.JS'.red}] ${time} <${'TDKSENSORSGROUP'.green}> <${JSONFilename.yellow}> TIMER=<${nInterval}>
      SOCKET=[${_server.socketArr.length}] ..GATEWAY<${String(_GATEWAYTEXT).yellow}>
      READ FROM <${'_ATCOMMANDS_TIMER2'.yellow}> SOCKETS=[${String(_ATSOCKETS).yellow}|${String(_DIFFSOCKETS).yellow}>
      ACTIVE AT-COMMAND <${String(ACTIVE_CMND.length).green}/${String(ATCMNDSObj.length).red}>..`);
    ...
    async function sendNextCommand() {
      ...
      if (TDKSENSORSGROUP_COUNTER === ACTIVE_CMND.length) {
        _DEBUG_TDKSENSORGroup && console.log(`[${'WORKERS.JS'.red}] ${time} <${'TDKSENSORSGROUP'.green}> AT-COMMAND SENT=<${String(TDKSENSORSGROUP_COUNTER)}|${ACTIVE_CMND.length}>`);
      }
    }    
  ...
  Sensor.find({}).sort({date: -1}).exec( (error,sensors) => {
    ...
    _data.read('stats','DATA_PWRMTER',function(err,data) {
      if (typeof data !== 'undefined') {
        _DEBUG_PROCESSSTATSANALYSIS && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. POWER METER DATA <${String(data?.length).yellow}> ..`);
...
// SERVER STATUS 
// -------------
const _DEBUG_SERVERSTATUSUPDATE = true;
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    if (hours%nHOURINT == 0 && (minutes == nMINUTEInt1 || minutes == nMINUTEInt2)) {
      ...
      // decoders.WhatsAppGateway3(AlertsGroup.WhatsAppGroup0,`..${time}..\tSERVER STATUS UPDATE..`);
      decoders.TelegramGateway(AlertsGroup.WhatsAppGroup0,`..${time}..\tSERVER STATUS UPDATE..`)
    }
    // ---------------------
    // --- ALERT SUMMARY ---
    // MON-FRI  07:00-19:00 : 60MIN STATUS UPDATE
    // MON-FRI  19:00-24:00 : 60MIN STATUS UPDATE
    // SAT-SUN  00:00-24:00 : 60MIN STATUS UPDATE
    let nTotalLines = 100;
    let nTIMER1 = (hours > 7 && hours < 19) ? ((days > 0 && days < 6) ? 60 : 60 ) : 10;
    if (minutes%nTIMER1 == 0) {
      let _day0 = new Date();
      let _day1 = new Date();
      _day1.setHours(hours,minutes,0)
      _day0.setHours(hours,minutes-nTIMER1-1,0);
      // ------------------
      // TEAWAREHOUSEALERTS
      // ------------------
      false && _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        _DEBUG_SERVERSTATUSUPDATE && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER1=[${nTIMER1.toString().yellow}] <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..\n`);
        ...
        ObjKEYS.map((key)=>{
          ...
          strMessage1 += `${key}\r${GetAlertsMessage1(Alerts)}`;
        })
        if (strMessage1.length > 10) {
          decoders.WhatsAppGateway1(AlertsGroup.WhatsAppGroup1,strMessage1);
        }
      })
    }
    // -- 20 MIN REPORTING ---
    ...
    if (minutes%nRPORTTIMER == 0) {
      ...
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        _DEBUG_SERVERSTATUSUPDATE && console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} ..TIMER2=[${nRPORTTIMER.toString().yellow}] <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length.toString().red}> CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}..`);
        let nameAlerts = alertsData?.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
            map[key] = [];
...
const GetAlertsMessage2 = (Alerts) => {
  ...
  Alerts.map((alert)=>{
    ...
    let _DATETIME = `${_DATE.getHours()}:${_DATE.getMinutes()}`;
    strTEXT += `${_DATETIME} ${alert.sensorType} ${alert.VALUE} ${alert.sign} ${alert.LIMITS} ${getSymbol()}\n\r`;
  })
  return strTEXT;
}
// -------
...
workers.SERVER_RESTART = function() {
  ...
  let date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  let time = today.getHours() + ":" + today.getMinutes();  
  console.log(`[${'WORKERS.JS'.green}] ${time}  <${'SERVER_RESTART'.blue}> STATUS=${String(_DEBUG_RESTARTSERVER).toUpperCase().yellow}`);
...
workers.init = function(){
  ...
  // TDK SENSORS
  // -----------
  flagTIMER2 && workers.TDKSENSORSGROUP(nTIMER2,timer2FileName);
  // --------------
  // DAY STATISTICS
  // --------------
