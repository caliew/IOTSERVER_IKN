 ...
 */
 ...
 const flagTIMER1 = false;  
 const flagTIMER2 = true;

 const TIMERLOOP1_ERRORLOG = true;

 const ATCMND_CONSOLE   = false;
 const ATCMND_ERRORLOG  = false;

 const DEBUG_ERRORLOG   = true;
 const DEBUG_INTOUTPUT  = false;

 const timer2FileName = '_ATCOMMANDS_TDK';
 let WhatsAppGroup0 = 'IKN LoRa IOT Group';
 let WhatsAppGroup1 = 'BALAKONG茶仓温控监管中心';
 let WhatsAppGroup2 = 'TDK Alerts Group'
 let WhatsAppGroup3 = 'Shinko IOT Monitoring System';
 ...
 workers.TDKSENSORSGROUP = function(nInterval,JSONFilename) {
   ...
   let LoopInterval = setInterval(function() {
     ...
     // SEND MODBUS CALLING COMMAND TO EACH ACTIVE SOCKETS IN SERVER
     // -------------------
     console.log(`[${'WORKERS.JS'.red}] ${time.blue} <${'TDKSENSORSGROUP'.green}> TIMER <${nInterval}> ..SOCKET CONNECTED <${_server.socketArr.length}> `);
     console.log(`[${'WORKERS.JS'.red}] ${time.blue} <${'TDKSENSORSGROUP'.green}> ..FILENAME <${JSONFilename.yellow}>..`);
     // ---------
     // TO RESET THE JSON OBJECT
     // decoder.RESET_ATCMND();
     // --------------------
     ...
     if (_server.socketArr){
       ...
       _server.socketArr.forEach((socket) => {
        ...
        console.log(`[${'WORKERS.JS'.red}] ${time.blue} <${'TDKSENSORSGROUP'.green}> .. GATEWAY  <${String(_GATEWAYTEXT).yellow}> ..`);
        _logs.append('_ERROR',`[WORKERS.JS] ${time} .. FILENAME <${JSONFilename}>.. `,()=>{});
        _logs.append('_ERROR',`[WORKERS.JS] ${time} .. GATEWAY  <${_GATEWAYTEXT}> ..`,()=>{});
        // ------------------
       // AT COMMANDS TIMER2
       // -------------------
       _data.read("ATCOMMANDS",JSONFilename,function(err,ATCMNDSObj) {
          // --------------
          console.log(`[${'WORKERS.JS'.red}] ${time.red} <${'TDKSENSORSGROUP'.green}> AT COMMAND FILE =<${JSONFilename}> ..`);
          _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  AT COMMAND FILE =<${JSONFilename}> ..`,()=>{});
          // -----------  
          if(err) {
            ...
            return;
					...
					// -----------------------
					let ACTIVE_CMND = ATCMNDSObj.filter( _CMD =>{ return _CMD.FLAG });
          console.log(`[${'WORKERS.JS'.red}] ${time.red} <${'TDKSENSORSGROUP'.green}> AT COMMAND =<${String(ACTIVE_CMND.length).red}/${String(ATCMNDSObj.length)}> ..`);
          // -------------------   --------------------------
          // CHECK IF SOCKET IS CONNECTED FOR AT COMMAND LIST
          // -------------------   --------------------------
          let _ATSOCKETS = [];
          console.log(`[${'WORKERS.JS'.red}]  .. ${time.red} GATEWAY ID =<${String(_GATEWAYIDArr).green}>`);
          ...
          // -----------
          console.log(`[${'WORKERS.JS'.red}]  .. ${time.red} AT-COMMANDS SOCKETS = [${String(_ATSOCKETS).yellow}] .MISSING SOCKETS <${String(_DIFFSOCKETS).yellow}> ...`);
          _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  AT-COMMANDS SOCKETS = [${String(_ATSOCKETS)}] ..MISSING SOCKETS <${String(_DIFFSOCKETS)}> .`,()=>{});
          // -----------------
          if (_DIFFSOCKETS.length == _ATSOCKETS.length) {
            return;
          } 
          console.log(`[${'WORKERS.JS'.red}]  .. ${time.red} AT COMMANDS READ FROM <${'_ATCOMMANDS_TIMER2'.yellow}> .. ACTIVE AT-COMMAND <${String(ACTIVE_CMND.length).green}/${String(ATCMNDSObj.length).red}>..`);
          _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  AT COMMANDS READ FROM <_ATCOMMANDS_TIMER2> .. ACTIVE AT-COMMAND <${ACTIVE_CMND.length}/${ATCMNDSObj.length}>..`,()=>{});
         // --------------
         ...
         _data.read("rawData","_485SENSORS",function(err,CMD_485RCV) {
           ...
           // 1. CHECK ON NUMBER OF RETURN 485 SENSORS RESPONSE IN LAST REQUESTS
           // ------------------------------------------------------------------
           DEBUG_INTOUTPUT  && console.log(`[${'WORKERS.JS'.red}]  .. [1] DATA READ _485SENSORS.JSON <${String(err).toUpperCase().yellow}>`);
           DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  [1] READ RAWDATA\\_485SENSORS.JSON ..<${String(err).toUpperCase()}>..`,()=>{});
           // ------------------
           _data.read("rawData",'ATCMND_SENT2',function(err,CMD_SENT) {
             // ---------
             DEBUG_INTOUTPUT  && console.log(`[${'WORKERS.JS'.red}]  .. [2] DATA READ RAWDATA\\ATCMND_SENT2.JSON <${String(err).toUpperCase().yellow}>`);
             DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  [2] READ DATA\\RAWDATA\\ATCMND_SENT2.JSON ..`,()=>{});
             // --------                          
             ...
             // ---------------
              if (!err) {
                // ---------
                DEBUG_INTOUTPUT  && console.log(`[${'WORKERS.JS'.red}]  .. [3] FILE RAWDATA\\ATCMND_SENT2.JSON DELETED`);
                DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  [3] FILE DATA\\RAWDATA\\ATCMND_SENT2.JSON DELETED..`,()=>{});
              ...
              _data.create("rawData", 'ATCMND_SENT2', ACTIVE_CMND, function (err) {
                // ---------
                if (!err) {
                  DEBUG_INTOUTPUT  && console.log(`[${'WORKERS.JS'.red}]  .. [4] FILE <${'ATCMND_SENT2'.green}> CREATED <ATCMND_SENT2>.`);
                  DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..      RAWDATA\\ATCMND_SENT2.JSON FILE CREATED`,()=>{});
                } else {
                  DEBUG_INTOUTPUT  && console.log(`[${'WORKERS.JS'.red}]  .. [4] FILE <${'ATCMND_SENT2'.green}> COULD BE NOT CREATED`);
                  DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..      RAWDATA\\ATCMND_SENT2.JSON COULD NOT BE CREATED`,()=>{});
                }
                // -----------
                DEBUG_INTOUTPUT  && console.log(`[${'WORKERS.JS'.red}]  .. [5] ATCMND_SENT2 CREATE COMPLETE..`);
                DEBUG_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  [4] ATCMND_SENT2 CREATE COMPLETE ..`,()=>{});                
                // ------------
                let nSTEP = 1;
                if (ACTIVE_CMND.length == 0) return;
                // -------------------
                ...
                TDKSENSORSGROUP = setInterval(function() {
                  ...
                  if (item && item.FLAG) {
                    ...
                    // ------------------
                    ATCMND_CONSOLE  && console.log(`[${'WORKERS.JS'.red}] <TDKSENSORSGROUP::ATCMND_SENT2> >> ${StrATCMND}..`)
                    ATCMND_ERRORLOG && _logs.append('_ERROR',`[WORKERS.JS] <TDKSENSORSGROUP::ATCMND_SENT2> >> ${StrATCMND} `,()=>{});
                    // ------------------
                    _server.socketArr.forEach((socket,index) => {
                      // ------------------------
                      // FILTER THE GATEWAY WHERE AT-COMMAND IS TO BE SENT
                  ...
                  if (TDKSENSORSGROUP_COUNTER > ACTIVE_CMND.length -1) {
                    ...
                    // console.log(`[${'WORKERS.JS'.red}]  .. [6] AT-COMMAND SENT .. SET INTERVAL KEY ${TDKSENSORSGROUP} ..<${String(TDKSENSORSGROUP_COUNTER)}|${ACTIVE_CMND.length}>`);
                    // _logs.append('_ERROR',`[WORKERS.JS] ${time} .. <TDKSENSORSGROUP::ATCMND_SENT2> ..  [6] AT-COMMAND SENT .. <${TDKSENSORSGROUP_COUNTER}>`,()=>{});
                    // ----------------------------
                  }
                  // --------------
                },1000 * nSTEP);
 ...
 function ProcessSTATANALYSIS() {
  ...
  Sensor.find({}).sort({date: -1}).exec( (error,sensors) => {
    // ------------------
    let nCOUNTER = 0;
    console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.yellow} <${'STATSANALYSIS'.green}> .. TOTAL SENSORS IN DATABASE <${String(sensors.length).yellow}> ..`);
...
workers.SERVER_STATUS_UPDATE = function(nInterval) {
  ...
  let LoopInterval = setInterval(function() {
    ...
    // -------------
    let nHOURINT = 3
    if (hours%nHOURINT == 0 && minutes == 0) {
      // -------
      console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} <SERVER_STATUS_UPDATE> ..<${hours}>.. SEND MAIL..`);
      decoder.WhatMateMessageGateway2(WhatsAppGroup0,`..${hours}:00..\t${nHOURINT} HOURLY UPDATE..`);
    }
    // ---------------------
    // --- ALERT SUMMARY ---
    ...
    let nTotalLines = 100;
    let nTIMER1 = (hours > 7 && hours < 19) ? ((days > 0 && days < 6) ? 60 : 60 ) : 60;
    if (minutes%nTIMER1 == 0) {
      ...
      // TEAWAREHOUSEALERTS
      // ------------------
      _logs.read('_TEAWAREHOUSEALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        console.log(`[${'WORKERS.JS'.green}] ${date.yellow} ${time.green} TIMER=${nTIMER1} <SERVER_STATUS_UPDATE> 
              CHECK ALERTS SUMMARY ..FROM=${String(_day0.getHours()).yellow}:${String(_day0.getMinutes()).yellow} TO=${String(_day1.getHours()).yellow}:${String(_day1.getMinutes()).yellow}.. DATA FOUND=<${alertsData.length}>...`);
        ...
        if (strMessage1.length > 10) {
          decoder.WhatMateMessageGateway2(WhatsAppGroup1,strMessage1);
        }
      })
    }
    // -- 20 MIN REPORTING ---
    ...
    // MON-FRI  19:00-24:00 : 60MIN STATUS UPDATE
    // SAT-SUN  00:00-24:00 : 60MIN STATUS UPDATE
    let nTIMER2 = (hours > 7 && hours < 19) ? ((days > 0 && days < 6) ? 20 : 30 ): 60;
    // console.log(`\n[${'WORKERS.JS'.green}] ..<${days}|${hours}|${minutes}>..  ..TIMER1=<${nTIMER1.toString().yellow}>..TIMER2=<${nTIMER2.toString().red}>..`)
    // -------
    if (minutes%nTIMER2 == 0) {
      let _day0 = new Date();
      let _day1 = new Date();
      _day1.setHours(today.getHours(),today.getMinutes(),0);
      _day0.setHours(today.getHours(),today.getMinutes()-nTIMER2-1,0);
      // ----------
      // TDK ALERTS
      // ----------
      _logs.read('_TDKALERTS',nTotalLines,_day0,_day1,false,function(err,alertsData){
        // -------
        console.log(`[${'WORKERS.JS'.green}] ..TIMER2=${nTIMER2.toString().yellow} <SERVER_STATUS_UPDATE> DATA FOUND=<${alertsData.length.toString().red}>...`);
        let nameAlerts = alertsData.reduce((map,alert)=>{
          let key = alert.NAME;
          if (map[key] == null) {
            map[key] = [];
        ...
        ObjKEYS.map((key)=>{
          let Alerts = nameAlerts[key];
          strMessage2 += `${key}\r${GetAlertsMessage2(Alerts)}`;
        })
        if (strMessage2.length > 10) {
          console.log(`[${'WORKERS.JS'.green}]..`,ObjKEYS);
          decoder.WhatMateMessageGateway1(WhatsAppGroup2,strMessage2);
        }
      })
    }
    // ---------------
...
const GetAlertsMessage2 = (Alerts) => {
  ...
  Alerts.map((alert)=>{
    ...
    let _DATETIME = `${_DATE.getHours()}:${_DATE.getMinutes()}`;
    strTEXT += `${_DATETIME} ${alert.sensorType} ${alert.VALUE} ${alert.sign} ${alert.LIMITS} ${getSymbol()}\r`;
  })
  return strTEXT;
}
// -------
...
workers.SERVER_RESTART = function(){
  ...
  let date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
  let time = today.getHours() + ":" + today.getMinutes();
  console.log(`[${'WORKERS.JS'.green}] ...${'SERVER_RESTART'.yellow}....`);
  _logs.append('_ERROR',`[WORKERS.JS] <${time}> .SERVER INITIALIZED....`,()=>{{}});
  // --------------------
