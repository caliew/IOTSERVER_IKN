 ...
 */
 ...
 function ProcessSTATANALYSIS() {
  ...
  Sensor.find({}).sort({date: -1}).exec( (error,sensors) => {
    ...
    _logs.append('_STATLOGS',`[WORKERS.JS] ${time} .. <TOTAL SENSORS IN DATABASE> ..  <${String(sensors.length)}>`,()=>{});
    // -----------------------
    ...
    sensors.map((sensor,index) => {
      ...
      SensorStats.find(query).exec( (error,_sensorStats) => {
        ...
        _day0.setMonth(today.getMonth() - 4);
          // --------------------------
        _logs.read(key,totalLines,_day1,_day0,false,function(err,sensorData) {
          // -------------
          // DTUID = -1 -> WISENSOR
          // DTUID > 0  -> 485 SENSORS
          // -------------------------
          ...
          if (sensor.type === 'PWRMTR(485)') {
            ...
            //  --------------------
            if (sensorKey == '1_1') { 
              console.log('..ProcessSTATANALYSIS..',nCOUNT);
