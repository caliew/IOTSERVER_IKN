...
server.tcpServer1008 = net.createServer(function (socket) {
  ...
  server.tcpServer1008.getConnections(function (error, count) {
    // ----------------------------
    // console.log(`[${'SERVER.JS'.yellow}]  PORT <${'1008'.green}> SOCKET [${String(server.socketArr.length).yellow}] <PUSH> ..SOCKET.. <${String(socket.GATEWAYID).green}>`);
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      CONSOLE_PRINT_BUFFER && (buffer.length) > 35 && (buffer.length) < 50 && 
        console.log(`[${'SERVER.JS'.yellow}]  PORT <${'1008'.green}> BUFFER <${String(buffer.length).blue}>:<${String(buffer).substring(0,15).rainbow}...> MATCHKEY=${String(MATCHKEY).substring(0,15).rainbow}..HEX STR=<${hex_to_ascii(MATCHKEY).substring(0,15).yellow}..>`);
      LOG_PRINT_BUFFER && (buffer.length) > 35 && (buffer.length) < 50 && 
        _logs.append('_ERROR', `[SERVER.JS]  PORT <1008> BUFFER <${String(buffer.length)}>:<${String(buffer).substring(0,15)}...> MATCHKEY=${String(MATCHKEY).substring(0,15)}..HEX STR=<${hex_to_ascii(MATCHKEY).substring(0,15)}..>`,()=>{});
      // -----------------------------------
      if (MATCHKEY == "88888888" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // ---------------------------------------------------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${'SERVER.JS'.yellow}] PORT ID=<${'1008'.green}> DEVICE ID=<${deviceID.green}> KEY=<${MATCHKEY.green}> REMOTE PORT=<${String(socket.remotePort).green}> REMOTE ADDRESS=<${String(socket.remoteAddress).blue}>`);
        _logs.append('_ERROR', `[SERVER.JS]  PORT ID=<${'1008'}> DEVICE ID=<${deviceID}> KEY=<${MATCHKEY}> REMOTE PORT=<${String(socket.remotePort)}> REMOTE ADDRESS=<${String(socket.remoteAddress)}>`,()=>{});
        // -------------
        if (deviceID) {
        ...
        if (DataArr.length < 4) {
          ...
          decoders.decode485Sensor(1008,DataArr,function(statusCode,payload) {
            ...
            if (statusCode == 200)  {
              ...
              try {
                _LOGS_SENSORS_RCV && _logs.append('_ERROR', `[SERVER.JS]  1008 <${hourNow}:${minuteNow}> ..<${statusCode}|${FUNCID}>  ` + 
              ...
              // ---------
              try {
                strINFO += `<${statusCode}|${FUNCID}>  DTU.ID=[${String(DTUID).padStart(3,"0")}] ` + 
              ...
              // SENSOR.TYPE=NULL
              console.log(`[SERVER.JS] ${"1008".green} CALLBACK ..DECODE485SENSOR.. STATUS CODE=<${String(statusCode).yellow}> ${String(payload).green}`)
            }
            // --------------------------------------------------
...
server.tcpServer1009 = net.createServer(function (socket) {
  ...
  server.tcpServer1009.getConnections(function (error, count) {
    // ---------------------------
    // console.log(`[${'SERVER.JS'.yellow}]  PORT <${'1009'.yellow}> SOCKET [${String(server.socketArr.length).yellow}] <PUSH> ..SOCKET.. <${String(socket.GATEWAYID).yellow}>`);
    // server.socketArr.push(socket);
    // ---------------------------
  });
  //  ---------------------
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      // --------
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      CONSOLE_PRINT_BUFFER && (buffer.length) > 35 && (buffer.length) < 50 && console.log(`[${'SERVER.JS'.yellow}]  PORT <${'1009'.yellow}> BUFFER <${String(buffer.length).blue}>:<${String(buffer).substring(0,15).rainbow}...> MATCHKEY=${String(MATCHKEY).substring(0,15).rainbow}..HEX STR=<${hex_to_ascii(MATCHKEY).substring(0,15).yellow}..>`);
      LOG_PRINT_BUFFER && (buffer.length) > 35 && (buffer.length) < 50 && _logs.append('_ERROR', `[SERVER.JS]  PORT <1009> BUFFER <${String(buffer.length)}>:<${String(buffer).substring(0,25)}...> MATCHKEY=${String(MATCHKEY).substring(0,15)}..HEX STR=<${hex_to_ascii(MATCHKEY).substring(0,15)}..>`,()=>{});
      // ------
      // 78563412 - TDK
      // 71589301 - JAMES HOME
      if (MATCHKEY == '32004851' || MATCHKEY == '2345678901' ||MATCHKEY == "83709728" || MATCHKEY == "71589301" || MATCHKEY == "50512412" ) {
        ...
        if (deviceID) {
          ...
          console.log('1009 SOCKETARR PUSH - GATEWAYDATA',server.socketArr.length,String(gatewayData.GATEWAYID));
...
server.tcpServer1010 = net.createServer(function (socket) {
  ...
  server.tcpServer1010.getConnections(function (error, count) {
    // ---------------------------
    // console.log(`[${'SERVER.JS'.yellow}]  PORT <${'1010'.red}> SOCKET [${String(server.socketArr.length).yellow}] <PUSH> ..SOCKET.. <${String(socket.GATEWAYID).red}>`);
    // server.socketArr.push(socket);
    // ---------------------------
  });
  //  ---------------------
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY=='80A0001B' || MATCHKEY == "37518050" || MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // -----------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${'SERVER.JS'.yellow}] PORT ID=<${'1010'.red}> DEVICE ID=<${deviceID.red}> KEY=<${MATCHKEY.red}> REMOTE PORT=<${String(socket.remotePort).red}> REMOTE ADDRESS=<${String(socket.remoteAddress).blue}>`);
        _logs.append('_ERROR', `[SERVER.JS]  PORT ID=<${'1010'}> DEVICE ID=<${deviceID}> KEY=<${MATCHKEY}> REMOTE PORT=<${String(socket.remotePort)}> REMOTE ADDRESS=<${String(socket.remoteAddress)}>`,()=>{});
        // -------------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
