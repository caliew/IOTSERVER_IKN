...
function startTCPServer(FileName,PortID) {
  ...
  const server = net.createServer(function(socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID).yellow}> ${MATCHKEY}|${hex_to_ascii(MATCHKEY)}|${_APKEY}`);
        blnDEBUG && _logs.append(_RAWFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PortID}> ${MATCHKEY}|${hex_to_ascii(MATCHKEY)}|${_APKEY}`,()=>{});
        // -----------------------------------
    ...
    socket.on("error", function (err) {
      ...
      _logs.append('_SERVER', `[SERVER.JS]  ${hourNow}:${minuteNow} <${String(_PortID)}>  SOCKET ERROR ${stringify(err)}`,()=>{});
    socket.destroy();
    });
    // 
    socket.setTimeout(1000*60*30, function () {
  ...
  // ------------------------------------
  server.listen(PortID,function () {console.log(`[${'SERVER.JS'.yellow}]  INIT TCP SERVER PORT ${String(PortID).green} - DOMAIN <${FileName}>`);} );
}
// ---------------------------
