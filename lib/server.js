...
function startTCPServer(FileName,PortID,blnDEBUG) {
  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    //  ON DATA RECEIVED ....
    //  --------------------- 
    blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] <${_PORTID}> <${clientAddress}>`,()=>{});
    socket.on("data", function (data) {
      // --------------------------
      buffer = decoder.write(data);
      buffer = buffer.replace("undefined", "");
      ...
      if (buffer.length > 4) {
        ...
        ( false && _PORTID == '1088') && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> <KEY=${MATCHKEY}|AP=${_APKEY}> BUFFER=${buffer}`,()=>{});
        ( true && blnDEBUG ) && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> <KEY=${MATCHKEY}|AP=${_APKEY}> BUFFER=${buffer}`,()=>{});
        // -----------------------------------
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          // ---------------------------------------------------
          let DEVICEID = hex_to_ascii(buffer.substring(10, 30));
          console.log(`[${"SERVER.JS".yellow}] ${GetTIMEStamp()} <${String(_PORTID).green}> DEVICE=<${DEVICEID.green}> [${String(_RAWFile).cyan}].. APKEY=[${_APKEY.yellow}] MATCHKEY=[${MATCHKEY.cyan}]`);
          _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> DEVICE=<${DEVICEID}> KEY=${MATCHKEY} AP-KEY=${_APKEY} `,()=>{});
          // -------------
          if (DEVICEID) {
        ...
        } else if (hex_to_ascii(MATCHKEY) == "+RCV") {  // TRANS MODE
          // ---------------
          // AT (TRANS) MODE
          // ---------------
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              decoders.decodeF8L10ST_ATMODE(_PORTID,buffer,function(statusCode,payload) {
                // ------
                _DEBUGINT && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:330 ${String(_PORTID).cyan} ${String(FileName).green} -decodeF8L10ST_ATMODE- .${String(statusCode).blue}.`);
                // ------
                blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> [${'decodeF8L10ST_ATMODE'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER}]`);
                blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [decodeF8L10ST_ATMODE] HEADER=[${_HEADER}] <${statusCode}>`,()=>{});
              ...
              decoders.decodeWISensorV2(_PORTID,buffer,function (statusCode, payload) {
                ...
                blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV2'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}] [${String(modelID).toUpperCase().green}]`);
                blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [decodeWISensorV2] HEADER=[${_HEADER}] <${statusCode}> [${modelID}]`,()=>{});
                // ------
                _DEBUGINT && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:354 ${String(_PORTID).cyan} ${String(FileName).green} -decodeWISensorV2- .${String(statusCode).blue}. ${String(modelType).cyan}|${String(modelID).toUpperCase().magenta}|${String(Temperature).red}|${String(Humidity).cyan}`);
                // ------
                if (statusCode == 407 )  {                  
              ...
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                ...
                _DEBUGINT && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:485 ${String(_PORTID).cyan} ${String(FileName).green} -decode485Sensor- .${String(statusCode).blue}. ${String(DTUID).magenta}|${String(SENSORID).red}`);
                // ------
                if (statusCode == 200)  {
                  blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> ..[${String(_RAWFile).cyan}].. [${'decode485Sensor'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}]..${payload.DTUID}|${payload.SENSORID}|${payload.SENSORTYPE}`);
            ...
            decoders.decodeWISensorV1(1008,DataArr,function (statusCode, payload) {
              ...
              let _HUMD = Humidity ?? '';
              _DEBUGINT && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:506 ${String(_PORTID).cyan} ${String(FileName).green} -decodeWISensorV1- .${String(statusCode).blue}. ${String(_MODELTYPE).cyan}|${String(_MODELID.toUpperCase()).magenta}|${String(_TEMP).red}|${String(_HUMD).cyan}`);
              // ------
              if (statusCode == 200)  {
                blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV1'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}] [${String(modelID).toUpperCase().green}]`);
            ...
            if (statusCode == 407) {
              ...
              if (_MODE == 'HEART-BEAT') {
                ...
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  if (_ALERT && _FLAG) {
                    _ALERTObj['MESSAGE'] = _MESSAGE;
                    (false && (PortID == '1013')) && console.log(`${FileName} --${_ALERT}|${_FLAG}-- MESSAGE=${String(_MESSAGE).toUpperCase()}`);
                    if (statusCode == 407) {
                      //  ---------------------
                  ...
                  // ----
                  _DEBUGINT && console.log(`[${"SERVER.JS".yellow}] [${"-AP-".red}] LINE:683 ${String(_PORTID).cyan} ${String(FileName).green} -decodeF8L10ST- .${String(statusCode).blue}. ${String(_MODE).red}|${String(_TYPE).blue}|${String(_DTUID).magenta}|${String(_NAME).cyan}`);
                  // -----
                  var jsonPayload = JSON.stringify(sensorObj);
                  if (payload._SENSORID >= 0) _logs.append(_RAWFile,jsonPayload,()=>{});  
                });
          ...
          blnDEBUG && console.log(`[${'SERVER.JS'.yellow}] ${GetTIMEStamp()} <${String(_PORTID).red}> MISS-MATCH KEY <${MATCHKEY.red}>`);
          _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID)}> MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
        }
      } else {
        // -----------------
        // GATEWAY HEARTBEAT
...
server.init = function () {
  ...
  startTCPServer('IKN_OPROOM',config.tcpPort1009,false);
  startTCPServer('IKN_HOSPITAL',config.tcpPort1010,false);
  startTCPServer('SHINKO',config.tcpPort1011,false);
  startTCPServer('SNOWCITY',config.tcpPort1012,true);
