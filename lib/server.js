...
server.tcpServer1883 = net.createServer(function(socket){
  ...
  socket.on("data", function (data) {
    ...
    _logs.append(_PortIDFile, `[SERVER.JS]  ${hourNow}:${minuteNow} PORT ID <${_PortID}> BUFFER <${buffer}>`,()=>{});
  });
  //  -----------------------------
  //  When Data Transmission End...
  //  -----------------------------
  socket.once("end", function () {
    console.log(`[${'SERVER.JS'.yellow}] PORT ID <${"1883".green}> ...${'GATEWAY SOCKET END'.red}...`);
    _logs.append('_ERROR', `[SERVER.JS]  PORT ID <${'1883'}> SOCKET END ...${clientAddress}`,()=>{});
  });
  //  --------------------------
  ...
  socket.once("close", function () {
    try {
      // ------
      console.log(`[${'SERVER.JS'.yellow}] PORT ID <${"1883".green}> ...${'GATEWAY SOCKET CLOSE'.red}...`);
      _logs.append('_ERROR', `[SERVER.JS]  PORT ID <${'1883'}> SOCKET CLOSE ...${clientAddress}`,()=>{});
      // ------
  ...
  socket.setTimeout(1000*60*30, function () {
    console.log(".....[TIMER 1883] DISCONNECTING CLIENT ...".rainbow);
    socket.end(".....[TIMER 1883] DISCONNECTING CLIENT ...");
  });
})
server.tcpServer8883 = net.createServer(function(socket){
  ...
  //  -----------------------------
  socket.once("end", function () {
    console.log(`[${'SERVER.JS'.yellow}] PORT ID <${"8883".green}> ...${'GATEWAY SOCKET END'.red}...`);
    _logs.append('_ERROR', `[SERVER.JS]  PORT ID <${'8883'}> SOCKET END ...${clientAddress}`,()=>{});
  });
  //  --------------------------
  //  When Connection Closed ...
  //  --------------------------
  socket.once("close", function () {
    try {
      // ------
      console.log(`[${'SERVER.JS'.yellow}] PORT ID <${"8883".green}> ...${'GATEWAY SOCKET CLOSE'.red}...`);
      _logs.append('_ERROR', `[SERVER.JS]  PORT ID <${'8883'}> SOCKET CLOSE ...${clientAddress}`,()=>{});
      // ------
      server.socketArr.pop(Socket);
    } catch (e) {}
    // --------
  ...
  // 
  socket.setTimeout(1000*60*30, function () {
    console.log(".....[TIMER 8883] DISCONNECTING CLIENT ...".rainbow);
    socket.end(".....[TIMER 8883] DISCONNECTING CLIENT ...");
  });
})
// ---------------------------
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        ...
        _logs.append('_1008', `[SERVER.JS] ${hourNow}:${minuteNow} <${'1008'}> DEVICE=<${deviceID}> KEY=<${MATCHKEY}|${_CHECKAPKEY}> REMOTE=<${String(socket.remotePort)}:${String(socket.remoteAddress)}>`,()=>{});
        // -------------
        if (deviceID) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        ...
        _logs.append(_PortIDFile, `[SERVER.JS]  PORT ID <${'1009'}> DEVICE ID <${deviceID}> KEY <${MATCHKEY}|${_CHECKAPKEY}> PORT ID <${String(socket.remotePort)}><${String(socket.remoteAddress)}>`,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        ...
        _logs.append('_1010', `[SERVER.JS] ${hourNow}:${minuteNow} <${'1010'}> DEVICE=<${deviceID}> KEY=<${MATCHKEY}|${_CHECKAPKEY}> REMOTE=<${String(socket.remotePort)}:${String(socket.remoteAddress)}>`,()=>{});
        // -------------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
...
server.tcpServer1011 = net.createServer(function (socket) {
  ...
  let _SHINKOIDs = [
    ...
    3010,3020,3031,3041,5010,5020,5031,3053,5030,
    1102,1092,3062,6013,6023,6033
  ]
  let _PortID = 1011;
  let _PortIDFile = `_${_PortID}`;
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        ...
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${'1011'}> DEVICE=<${deviceID}> KEY=<${MATCHKEY}|${_CHECKAPKEY}> REMOTE=<${String(socket.remotePort)}:${String(socket.remoteAddress)}>`,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
        ...
        let _HEADER = DataArr1[1].substring(0, 2).toString("UTF-8").toUpperCase();
        // --------
        console.log('TCPSERVER-1011',hex_to_ascii(MATCHKEY),_HEADER,DataArr.length);
        // --------
        if (DataArr.length < 4) {
          if (_HEADER === 'FA') {
              ...
              if (statusCode == 407 )  {
                ...
                try
                {
                  ...
                  _data.read('shinko','settings',function(err,settingData) {
                    ...
                    // console.log('....',_MODE,_ALERT,_FLAG,_MESSAGE);
                    //  -------------
                    if (_ALERT && _FLAG) {
            ...
            decoders.decode485Sensor(1008,DataArr,function(statusCode,payload) {
              console.log('TCPSERVER-1011',payload);
              if (statusCode != 200)  {
                try {
                  const {FUNCID,DTUID,SENSORID,NDATA,RCV_BYTES,DATAS,} = payload;
                  _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} DECODE 485 SENSOR <${statusCode}> ${DTUID}|${SENSORID}`,()=>{});
          ...
          if (statusCode == 200) {
            ...
            if (_SHINKOIDs.includes(Number(payload._DTUID))) {
              ...
              _data.read('shinko','settings',function(err,settingData) {
                ...
                //  -------------
                (blnFlag) && (_TYPE==='TEMP & RH') && console.log(`  TCPSERVER 1011.. SHINKO ID=${Number(payload._DTUID)} ..NAME=${_NAME} ..GROUP=${_GROUP} TYPE=${_TYPE} <FLAG=${_FLAG}|ALERT=${_ALERT}> ..${_READINGTEMP}=${_TEMPMAX}|${_TEMPMIN}..`);
                // -----------------
                ...
                if (_FLAG) {
                  _ALERTObj['MESSAGE'] = _MESSAGE;
                  let jsonPayload = JSON.stringify(_ALERTObj);
                  (blnFlag) && (_TYPE==='TEMP & RH') && console.log(`  TCPSERVER 1011.. ${_MESSAGE}`);
        ...
        _logs.append(_PortIDFile, `[SERVER.JS]  PORT ID <${'1011'}> MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
      }
    } 
  });
  //  -----------------------------
...
server.tcpServer1012= net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      let _CHECKAPKEY = buffer.substring(0, 2).toString("UTF-8").toUpperCase();
      _logs.append(_PortIDFile, `[SERVER.JS]  PORT ID <${_PortID}> ${hourNow}:${minuteNow} <${MATCHKEY}|${_CHECKAPKEY}>..${buffer}`,()=>{});
      // -------------------------------------
      // 78563412 - TDK, 71589301 - JAMES HOME
      // -------------------------------------
      // ---------
...
// INTERNAL TEST GATEWAY (FORMER SETUP FOR IJN)
// ---------------------------
          ...
          if (statusCode == 200 || statusCode == 407) {
            ...
            _data.read('nipponglass','settings',function(err,settingData) {
              ...
              if (_ALERT && _FLAG) {
                ...
                let jsonPayload = JSON.stringify(_ALERTObj);
                decoders.WhatsAppGateway2(_FoundSensor.ALERTGROUP,_MESSAGE)
                _logs.append('_NIPPONGLASSALERTS',jsonPayload,()=>{});
              }
              if (_MODE != 'RS485') return;
            })      
            if (payload._SENSORID != 0) _logs.append('_NIPPONGLASS',jsonPayload,()=>{});
          } else {
...
server.tcpServer1021= net.createServer(function (socket) {
  // --------
  let buffer;
  let _PortID = 1021;
  let _PortIDFile = `_${_PortID}`;
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // ---------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        _logs.append(_PortIDFile, `[SERVER.JS]  PORT ID <${_PortID}> ${hourNow}:${minuteNow} DEVICE ID=<${deviceID}>`,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
            ...
            if (_DTUIDs.includes(Number(payload._DTUID))) {
              ...
              _data.read('IJN','settings',function(err,settingData) {
                ...
                if (_ALERT && _FLAG) {
                  ...
                  decoders.WhatsAppGateway1('IJN IOT Monitoring System',_MESSAGE)
                }
              });
