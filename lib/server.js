...
// GPIO helper — loaded lazily to avoid circular require
function _gpioAlert(siteName, dtuid, portId, stateOn) {
  try {
    const stateVal = stateOn ? 1 : 0;
    const atCommand = `AT+NS1=${dtuid},${portId},${stateVal}\r\n`;
    const arr = server.socketArr || [];
    const activeSockets = arr.filter(s => s.SOCKET && typeof s.SOCKET.write === 'function');
    if (activeSockets.length === 0) {
      console.warn(`[SERVER.JS] [GPIO] ⚠️ No active sockets for GPIO alert siteName=${siteName}`);
      return;
    }

    const sockets2001 = activeSockets.filter(s => s.PORT === 2001);
    if (sockets2001.length > 0) {
      const portNum = Number(portId) || 1;
      const frameRaw = stateOn 
        ? (portNum === 2 ? Buffer.from([0x01, 0x05, 0x00, 0x02, 0xFF, 0x00, 0x2D, 0xFA]) : Buffer.from([0x01, 0x05, 0x00, 0x01, 0xFF, 0x00, 0xDD, 0xFA]))
        : (portNum === 2 ? Buffer.from([0x01, 0x05, 0x00, 0x02, 0x00, 0x00, 0x6C, 0x0A]) : Buffer.from([0x01, 0x05, 0x00, 0x01, 0x00, 0x00, 0x9C, 0x0A]));
      const escaped = MODBUS_PROTOCOL.escape(frameRaw);
      sockets2001.forEach(s => {
        try {
          s.SOCKET.write(escaped);
          console.log(`[SERVER.JS] [GPIO 2001] ✅ SENT MODBUS PORT=${portId} STATE=${stateOn} GW=${s.GATEWAYID}`);
          _logs.append('_GPIO', `[SERVER.JS] GPIO ALERT 2001 siteName=${siteName} DTUID=${dtuid} PORT=${portId} STATE=${stateVal} GW=${s.GATEWAYID}`, () => { });
        } catch (e) {
          console.error(`[SERVER.JS] [GPIO 2001] ❌ FAILED GW=${s.GATEWAYID}`, e.message);
        }
      });
    } else {
      activeSockets.forEach(s => {
        try {
          ...
          console.log(`[SERVER.JS] [GPIO] ✅ SENT GATEWAY=${s.GATEWAYID} CMD=[${atCommand.trim()}]`);
          _logs.append('_GPIO', `[SERVER.JS] GPIO ALERT siteName=${siteName} DTUID=${dtuid} PORT=${portId} STATE=${stateVal} GW=${s.GATEWAYID}`, () => { });
        } catch (e) {
          console.error(`[SERVER.JS] [GPIO] ❌ FAILED GATEWAY=${s.GATEWAYID}`, e.message);
        }
      });
    }
  } catch (err) {
    console.error('[SERVER.JS] [GPIO] _gpioAlert error:', err.message);
  }
}
// ---------------------------------
...
server.tcpServer1883 = net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer) rawBufferRecorder.record(_PortID, 'MQTT', buffer, clientAddress);
    // ---------------------
...
server.tcpServer8883 = net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    buffer = decoder.write(data);
    buffer = buffer.replace("undefined", "");
    if (buffer) rawBufferRecorder.record(_PortID, 'MQTT_SSL', buffer, clientAddress);
    // ---------------------
    _logs.append(_PortIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PortID)}> BUFFER <${buffer}>`, () => { });
  });
  //  -----------------------------
...
function startTCPServer(FileName, PortID, blnDEBUG) {
  ...
  const tcpServer = net.createServer(function (socket) {
    ...
    socket.on("data", function (data) {
      ...
      buffer = buffer.replace("undefined", "");
      if (buffer) {
        rawBufferRecorder.record(_PORTID, FileName, buffer, clientAddress);
      }
      // --------------------------------------
      if (buffer.length > 4) {
        // -----------
        ...
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          ...
          decoders.decodeF8L10ST_APIMODE(_PORTID, buffer, blnDEBUG, function (statusCode, payload) {
            ...
            if (statusCode == 407) {
              if (_MODE == 'HEART-BEAT') {
                // ─────────────────────────────────────────────────────────────
                // HEART-BEAT: log battery/status only. If the DTU piggybacks
                // embedded RS485 bytes (_HB_DATAHEX), they are recorded here
                // for diagnostics but are NEVER sent to alert evaluation.
                // ─────────────────────────────────────────────────────────────
                const _hbNote = payload._HB_DATAHEX
                  ? `EMBEDDED RS485 BYTES (NOT EVALUATED): ${payload._HB_DATAHEX}`
                  : 'NORMAL';
                var jsonPayload = JSON.stringify({
                  ...sensorObj,
                  HB_DATA: payload._HB_DATAHEX ?? null,
                  HB_NOTE: _hbNote,
                });
                ...
                _data.read(FileName, 'settings', function (err, settingData) {
                  ...
                  // if (payload._SENSORID >= 0) 
                  _logs.append(_RAWFile, jsonPayload, () => { });
...
// ----------------------------------------------
// MODBUS PROTOCOL & TCP SERVER ON PORT 2001
// ----------------------------------------------
const MODBUS_PROTOCOL = {
  ESCAPE_MARKER: 0xFD,
  ESCAPE_SEQUENCES: {
    0xFD: 0xED,
    0xFE: 0xEE
...
function startModbus2001Server(PortID) {
  const _PortID = PortID || config.tcpPort2001 || 2001;
  const _PortIDFile = `_${_PortID}`;
  const FileName = 'AEROSOFT';

  const tcpServer = net.createServer(function (socket) {
    const clientAddress = `${socket.remoteAddress}>${socket.remotePort}`;
    console.log(`[${'SERVER.JS'.yellow}] 🔌 [PORT ${_PortID}] DTU MODBUS CONNECTED (${clientAddress})`);
    _logs.append(_PortIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PortID}> DTU MODBUS CONNECTED ...${clientAddress}`, () => {});

    const gatewayData = {
      PORT: _PortID,
      FILENAME: FileName,
      MATCHKEY: 'MODBUS_2001',
      GATEWAYID: 'DTU_2001',
      ADDRESS: clientAddress,
      TIMESTAMP: new Date(),
      SOCKET: socket,
    ...
    socket.on("data", function (data) {
      gatewayData.lastSeen = Date.now();
      let rawHex = decoder.write(data).replace("undefined", "");
      if (rawHex) {
        rawBufferRecorder.record(_PortID, FileName, rawHex, clientAddress);
      }

      const decoded = MODBUS_PROTOCOL.unescape(data);

      if (decoded.length === 1 && decoded[0] === 0xFE) {
        console.log(`[${'SERVER.JS'.yellow}] 💓 [PORT ${_PortID}] RECV HEARTBEAT from ${clientAddress}`);
        _logs.append(_PortIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PortID}> HEARTBEAT from ${clientAddress}`, () => {});
        return;
      }

      if (decoded.length >= 21 && decoded[15] === 0x00) {
        try {
          const deviceId = decoded.readUInt32LE(0);
          const phone = decoded.slice(4, 15).toString('ascii').replace(/\0/g, '');
          gatewayData.GATEWAYID = String(deviceId);
          gatewayData.PHONE = phone;
          console.log(`[${'SERVER.JS'.yellow}] 📝 [PORT ${_PortID}] REGISTER deviceId=${deviceId} phone=${phone}`);
          _logs.append(_PortIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PortID}> REGISTER deviceId=${deviceId} phone=${phone}`, () => {});
        } catch (e) {
          console.error(`[SERVER.JS] Error parsing register message:`, e.message);
        }
      } else {
        console.log(`[${'SERVER.JS'.yellow}] 📥 [PORT ${_PortID}] RECV HEX => ${rawHex}`);
        _logs.append(_PortIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PortID}> RECV HEX => ${rawHex}`, () => {});
      }
    });

    socket.once("end", function () {
      console.log(`[${'SERVER.JS'.yellow}] ❌ [PORT ${_PortID}] DTU SOCKET END (${clientAddress})`);
      _logs.append('_ERROR', `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PortID}> SOCKET END ...${clientAddress}`, () => {});
    });

    socket.once("close", function () {
      console.log(`[${'SERVER.JS'.yellow}] ❌ [PORT ${_PortID}] DTU SOCKET CLOSE (${clientAddress})`);
      _logs.append('_ERROR', `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PortID}> SOCKET CLOSE ...${clientAddress}`, () => {});
      const idx = server.socketArr.indexOf(gatewayData);
      if (idx > -1) {
        server.socketArr.splice(idx, 1);
      }
    });

    socket.on("error", function (err) {
      console.error(`[${'SERVER.JS'.yellow}] ⚠️ [PORT ${_PortID}] SOCKET ERROR: ${err.message}`);
      socket.destroy();
    });
  ...
  tcpServer.listen(_PortID, function () {
    console.log(`[${'SERVER.JS'.yellow}] INIT MODBUS DTU TCP SERVER PORT ${String(_PortID).yellow} - AEROSOFT 2001`);
  });
}
...
function DISPATCH_ALERT(FileName, _PortALERTFILE, _FoundSensor, _ALERTObj, _ALERTFLAG, _ALERT) {
  ...
  _logs.read(_PortALERTFILE, 10, null, null, false, function (err, alertsdata) {
    ...
    // Always record to incident logs regardless of whether external messaging is enabled
    if (_FoundSensor.DTUID) {
      fileStores.recordIncident(FileName, {
        macId: _FoundSensor.DTUID,
        sensorName: _FoundSensor.NAME || _FoundSensor.DTUID,
        alertType: _FoundSensor.TYPE || 'LIMIT_BREACH',
        message: _ALERTObj.MESSAGE,
        timestamp: _ALERTObj.TIMESTAMP || new Date().toISOString()
      });
    }
    ...
    // ---------------------------------------------------------------
    // PHYSICAL GPIO ALARM: turn ON any GPIO port configured under
    // IOT_SENSORS.GPIO in the site's settings.json
    //   settings.GPIO = { "1": { NAME:"Relay 1", ALERT_PORT: true, DTUID: 102 }, ... }
    // ---------------------------------------------------------------
    _data.read(FileName, 'settings', function (err, settingData) {
      if (!err && settingData && settingData.IOT_SENSORS && settingData.IOT_SENSORS.GPIO) {
        const gpioConfig = settingData.IOT_SENSORS.GPIO;
        Object.keys(gpioConfig).forEach(portId => {
          const portCfg = gpioConfig[portId];
          // Only trigger ports flagged with ALERT_PORT:true
          if (portCfg.ALERT_PORT && portCfg.DTUID) {
            _gpioAlert(FileName, portCfg.DTUID, Number(portId), true);
          }
        });
      }
    });
          ...
          fs.rename(oldpath, newpath, function (err) {
            ...
            _data.read("uploads", "ESP32CAMData", function (err, data) {
              ...
              _data.update("uploads", "ESP32CAMData", ObjPHOTOS, function (err) {
                if (err) {
                  _data.create("uploads", "ESP32CAMData", ObjPHOTOS, function (err) {
                    if (!err) {
...
server.router = {
  ...
  "api/fileStores/rollups": handlers.fileStoresRollups,
  "api/fileStores/sync": handlers.fileStoresSync,
  "api/incidents": handlers.incidents,
  "api/powerTracker/status": handlers.powerTrackerStatus,
...
server.init = function () {
  ...
  blnFLAG && startTCPServer('IKN_PATHOLOGY', config.tcpPort1011, false);    // AP
  ...
  blnFLAG && startModbus2001Server(config.tcpPort2001 || 2001);          // MODBUS DTU 2001
