    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              decoders.decodeF8L10ST_ATMODE(_PORTID,buffer,blnDEBUG,function(statusCode,payload) {
                ...
                true && console.log(`[SERVER.JS] ${String(PortID).yellow} RAW_FILE=<${String(_RAWFile).green}> STATUS CODE=${String(statusCode).red} DTU.ID=${_DTUID} SENSOR.ID=${_SENSORID} <${_RAWFILENAME}> HEX_DATA=${payload._DATAHEX} ${_ARRAY}`);
                // ------
                const ObjSENSOR = {
                  DTUID:_DTUID,
                  SENSORID: _SENSORID,
              ...
              decoders.decodeWISensorV2(_PORTID,blnDEBUG,buffer,function (statusCode, payload) {
                ...
                !blnSKIPWISENSOR2 && blnDEBUG && _logs.append(_PORTIDFile, `[${"SERVER.JS"}] [${_CMODE}] LINE:383 ${String(_PORTID)} ${String(FileName)} =decodeWISensorV2= .${String(statusCode)}. HEADER[${String(_HEADER)}] |${String(modelType??'-')}|${String(modelID??'-').toUpperCase()}|${String(Temperature??'-')}|${String(Humidity??'-')}|${String(BATT??'-')}|${String(INTERVAL??'-')}|`,()=>{});
                // ------
                if (statusCode == 407 )  {
                  _data.read(FileName,'settings',function(err,settingData) {
                    ...
                    let _AMPMAX = _FoundSensor.AMP_MAX ?? 0;
                    let _AMPMIN = _FoundSensor.AMP_MIN ?? 0;
                    let _PRESSMAX = _FoundSensor.PRESS_MAX ?? 0;
                    let _PRESSMIN = _FoundSensor.PRESS_MIN ?? 0;
                    let _TEMPMAX = _FoundSensor.TEMP_MAX ?? 0;
                    let _TEMPMIN = _FoundSensor.TEMP_MIN ?? 0;
                    let _RHMAX = _FoundSensor.RH_MAX ?? 0;
                    let _RHMIN = _FoundSensor.RH_MIN ?? 0;
                    let _DEWMAX = _FoundSensor.DEW_MAX ?? 0;
                    let _DEWMIN = _FoundSensor.DEW_MIN ?? 0;
                    ...
                    _ALERT = String(_ALERT).toUpperCase();
                    console.log('DECODEWISENSORV2',_TYPE,_NAME,_GROUP,_UNITSYSTEM,_ALERT)
                    //  --------------
                    let _FLAG = false;
                    let _ALERTObj = {};
          ...
          _CMODE = 'APMOD'
          false && blnDEBUG && _logs.append(_PORTIDFile, `[${'SERVER.JS'}] ${GetTIMEStamp()} <${String(_PORTID)}> [${String(_CMODE)}] .LINE 586. APKEY=${String(_APKEY)} CHECK [FA,FE,FD]...`,()=>{});
          true && blnDEBUG && console.log(`[${'SERVER.JS'.red}] ${GetTIMEStamp()} <${String(_PORTID).green}> [${String(_CMODE).red}] .LINE 587. APKEY=${String(_APKEY).green} CHECK [FA,FE,FD]...`)
