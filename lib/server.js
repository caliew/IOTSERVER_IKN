  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        ( true && blnDEBUG ) && console.log(`[${"SERVER.JS".yellow}] ${hourNow}:${minuteNow} <${String(_PORTID).green}> ..[${String(_RAWFile).cyan}].. APKEY=[${_APKEY.yellow}] MATCHKEY=[${MATCHKEY.red}] `);
        // -----------------------------------
            ...
            if (_HEADER === 'FA') {
              ...
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                ...
                if (statusCode == 200)  {
                  ...
                  blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> ..[${String(_RAWFile).cyan}].. [${'decode485Sensor'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER}] ${SENSORTYPE} [${FUNCID}|${DTUID}|${SENSORID}]`);
