...
server.tcpServer1008 = net.createServer(function (socket) {
  ...
  // --------------
  console.log(`[${'SERVER.JS'.yellow}] PORT <${'1008'.green}> REMOTE.ADD      <${socket.remoteAddress.blue}> PORT<${socket.remotePort}>`);
  // --------------------------------------------------
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY == "88888888" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        ...
        // ---------------------------------------------------
        console.log(`[${'SERVER.JS'.yellow}] PORT <${'1008'.green}> ..... MATCH KEY <${MATCHKEY.green}> ... DEVICE ID <${deviceID.green}>`);
        // -------------
        if (deviceID) {
...
server.tcpServer1009 = net.createServer(function (socket) {
  ...
  // --------------
  var clientAddress = `${socket.remoteAddress}>${socket.remotePort}`;
  console.log(`[${'SERVER.JS'.yellow}] PORT <${'1009'.yellow}> REMOTE.ADD      <${socket.remoteAddress.blue}> PORT<${socket.remotePort}>`);
  // --------------------------------------------------
  ...
  socket.on("data", function (data) {
    ...
    // ----------------------------------
    // console.log(`[${"SERVER.JS".rainbow}] --<${'1009'.yellow}>--<${hourNow}:${minuteNow}>--${'BUFFER'.rainbow}-- ${String(buffer).toUpperCase()}`)
    if (buffer.length > 4) {
      // --------
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      // ----------- 011 5418 0032 
      ...
      if (MATCHKEY == '32004851' || MATCHKEY == '2345678901' ||MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // GATEWAY ------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".yellow}] PORT <${'1009'.yellow}> ..... MATCH KEY <${MATCHKEY.yellow}> ... DEVICE ID <${deviceID.yellow}>`);
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
        ...
        // AP MODE -----
        // -------------
        console.log(`[${"SERVER.JS".rainbow}] --<${'1009'.yellow}>--<${hourNow}:${minuteNow}>-- ${MATCHKEY} > DECODERS.decodeF8L10ST`)        
...
server.tcpServer1010 = net.createServer(function (socket) {
  ...
  // ---------------------------
  var clientAddress = `${socket.remoteAddress}>${socket.remotePort}`;
  console.log(`[${'SERVER.JS'.yellow}] PORT <${'1010'.red}> REMOTE.ADD      <${socket.remoteAddress.blue}> PORT<${socket.remotePort}>`);
  // --------------------------------------------------
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY=='80A0001B' || MATCHKEY == "37518050" || MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // GATEWAY -----
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${'SERVER.JS'.yellow}] PORT <${'1010'.red}> ..... MATCH KEY <${MATCHKEY.red}> ... DEVICE ID <${deviceID.red}>`);
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
            gatewayData = {
