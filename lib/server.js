  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        ( blnDEBUG && (_APKEY!="0D")) && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [-----] APKEY=[${_APKEY}] REGKEY=${MATCHKEY} BUFFER=[${buffer.length}]${buffer}`,()=>{});
        ( blnDEBUG && (_APKEY!="0D")) && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).rainbow}> [-----] APKEY=[${String(_APKEY).red}] REGKEY=[${String(MATCHKEY).green}] BUFFER=[${String(buffer.length).yellow}]${String(truncateString(buffer,35)).rainbow}`);
        // -----------------------------------
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
                ...
                if (statusCode == 407 )  {
                  _data.read(FileName,'settings',function(err,settingData) {
                    ...
                    if (_ALERT && _FLAG) {
                      ...
                      console.log(`[SERVER.JS] LINE:537 ${_MESSAGE}`)
                      true && blnDEBUG && console.log(`[SERVER.JS] LINE:536 ${String('DECODEWISENSORV2').green} [${String(_PORTID).yellow}] <${FileName.red}|${String(_FLAG).toUpperCase()}|${String(_ALERT).toUpperCase()}> [${_UNITSYSTEM.green}] <${_TYPE.green}> ${_MESSAGE.red} `)
                      DISPATCH_ALERT(FileName,_PortALERTFILE,_FoundSensor,_ALERTObj,_ALERTFLAG);
                      }
                  });
                }
              });
            ...
            decoders.decodeWISensorV1(1008,DataArr,function (statusCode, payload) {
              ...
              if (statusCode == 407 )  {
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  if (_ALERT && _FLAG) {
                    _ALERTObj['MESSAGE'] = _MESSAGE;
                    true && console.log(`${String('DECODEWISENSORV1').green} [${String(_PORTID).yellow}] <${FileName.red}|${String(_FLAG).toUpperCase()}|${String(_ALERT).toUpperCase()}> [${_UNITSYSTEM.green}] <${_TYPE.green}> ${_MESSAGE.red} `)
                    DISPATCH_ALERT(FileName,_PortALERTFILE,_FoundSensor,_ALERTObj,_ALERTFLAG);
                  }
                });
              }
          ...
          _CMODE = 'APMOD'
          false && blnDEBUG && _logs.append(_PORTIDFile, `[${'SERVER.JS'}] ${GetTIMEStamp()} <${String(_PORTID)}> [${String(_CMODE)}] LINE:717 APKEY=${String(_APKEY)} CHECK [FA,FE,FD]...`,()=>{});
          ...
          decoders.decodeF8L10ST(_PORTID,buffer,blnDEBUG,function(statusCode,payload) {
            ...
            let _MODE = payload._MODE ?? '';
            let _HEX = payload._DATAHEX ?? '';
            blnDEBUG && console.log(`  >>[SERVER.JS] LINE:731 <${String(_PORTID).green}> [${String(_CMODE).red}] STATUS CODE=[${String(statusCode).yellow}] MODE=[${_MODE}] APKEY=[${String(_APKEY).green}] MAC ID=[${String(_DTUID).blue}] FILENAME=[${String(FileName).red}] PAYLOAD=${String(_HEX).cyan}`)
            // -------------------------
            ...
            if (statusCode == 407) {
              ...
              if (_MODE == 'HEART-BEAT') {
                ...
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  // -------
                  !blnSKIP485SENSOR && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${String(_CMODE).red}] LINE:945 PORT ID=[${String(_PORTID).green}] ${String(FileName).green} =decodeF8L10ST= .${String(statusCode).blue}. |${String(_MODE).red}|${String(_TYPE).blue}|${String(_DTUID).magenta}|${String(_NAME).cyan}||${_READINGCURRENT??'-'}A|${_READINGPRESSURE??'-'}Pa|${_READINGTEMP??'-'}C|${_READINGRH??'-'}%|${_READINGDEW??'-'}C|`);
                  !blnSKIP485SENSOR && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${String(_CMODE).red}] LINE:946 ALERT=[${String(_ALERT).red}] FLAG=[${String(_FLAG).yellow}] MESSAGE=[${String(_MESSAGE).cyan}]`);
                  // -------
                });
              }
            }
          ...
          blnDEBUG && console.log(`[${'SERVER.JS'}] <${String(_PORTID).rainbow}> ${GetTIMEStamp()} ${String('LINE:744 !!!MISSING GATEWAY!!! ..REG KEY..').yellow} PORT=${String(_PORTID).green} REGKEY=${MATCHKEY.yellow} _APKEY=${String(_APKEY).blue}`);
...
function DISPATCH_ALERT(FileName,_PortALERTFILE,_FoundSensor,_ALERTObj,_ALERTFLAG) {
  ...
  _logs.read(_PortALERTFILE,10,null,null,false,function(err,alertsdata){
    ...
    let filteredData = [];
    false && console.log(`  LINE:1035 <${FileName}> ALERTFILE=[${_PortALERTFILE}] ${now.toISOString()} DTU.ID=${_FoundSensor.DTUID}  FLAG=${_ALERTFLAG} ALERTS LOG=${alertsdata.length}`);
    if (_ALERTFLAG > 0) {
      ...
      true && console.log(`  LINE:1044 <${FileName}> ..#ALERTS COUNT=${AlertCount}  FLAG=${_ALERTFLAG}`);
      if ( AlertCount <= _ALERTFLAG){
        true && console.log(`  LINE:1045  ${AlertCount} < ${_ALERTFLAG}`)
        return;
      }
    }
    switch (FileName) {
...
server.init = function () {
  ...
  blnFLAG && startTCPServer('NIPPONGLASS',config.tcpPort1013,true);   // API
  blnFLAG && startTCPServer('EPSON',config.tcpPort1014,false);         // AP
  blnFLAG && startTCPServer('NIPPONGLASS_BOILER',config.tcpPort1015,false);         // AP
