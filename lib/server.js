...
// UTILITIES FUNCTIONS
// -------------------
function writeLog(_PORTFile,string) {
...
// ----------------------------------------------
function startTCPServer(FileName,PortID) {
  // ----------------------------------------
  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    let _PortALERTFILE = `_${FileName}ALERT`;
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        // -----------------------------------
        if (_REGISTERKEYS[_PORTID].includes(MATCHKEY)) {
          // ---------------------------------------------------
          let DEVICEID = hex_to_ascii(buffer.substring(10, 30));
          console.log(`[${"SERVER.JS".yellow}] ${hourNow}:${minuteNow} <${String(_PORTID).green}> DEVICE=<${DEVICEID.green}> KEY=${MATCHKEY} AP-KEY=${_APKEY} `);
          ...
          // -----------------------------------
          // AT (TRANS) MODE - DATA PACKET ARRAY
          // -----------------------------------
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              decoders.decodeF8L10ST_ATMODE(_PORTID,buffer,function(statusCode,payload) {
                // -------------
                blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [decodeF8L10ST_ATMODE] <${statusCode}>`);
                blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [decodeF8L10ST_ATMODE] <${statusCode}>`,()=>{});
              ...
              decoders.decodeWISensorV2(_PORTID,buffer,function (statusCode, payload) {
                ...
                blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [decodeWISensorV2] <${statusCode}> [${modelID}]`);
                blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [decodeWISensorV2] <${statusCode}> [${modelID}]`,()=>{});
                if (statusCode != 200 )  {
                  _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [decodeWISensorV2] <${statusCode}> [${modelID}]`,()=>{});                }
              });
              // --------------
            } else {
              // -----------------------   
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                ...
                if (statusCode == 200)  {
                  ...
                  blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [decode485Sensor] <${statusCode}> ${SENSORTYPE} [${FUNCID}|${DTUID}|${SENSORID}]`);
                  blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [decode485Sensor] <${statusCode}> ${SENSORTYPE} [${FUNCID}|${DTUID}|${SENSORID}]`,()=>{}); 
                } else {
                  _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow}  <${_PORTID}> decode485Sensor <${statusCode}> ..`,()=>{});
                }
              });
            }
            // ---------------
            ...
            decoders.decodeWISensorV1(1008,DataArr,function (statusCode, payload) {
              if (statusCode == 200)  {
                const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
                blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [decodeWISensorV1] <${statusCode}> [${modelID}]`);
                blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [decodeWISensorV1] <${statusCode}> [${modelID}]`,()=>{});
              } else {
                _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [decodeWISensorV1] <${statusCode}>`,()=>{});
              }
            });
          }
        } else if (["FE"].includes(_APKEY)) {
          blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [GATEWAY HEARTBEAT] <${_APKEY}>`);
          blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [GATEWAY HEARTBEAT] <${_APKEY}>`,()=>{});
        } else {
          if (MATCHKEY =='0D0A4F4B') return;
          console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PORTID).red}> MISS-MATCH KEY <${MATCHKEY.red}>`);
    ...
    //  -------------------------
    socket.on("error", function (err) {
      console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PORTID).green}> ...${'GATEWAY SOCKET ERROR'}...`);
      console.log(err);
      _logs.append('_SERVER', `[SERVER.JS]  ${hourNow}:${minuteNow} <${String(_PORTID)}>  SOCKET ERROR ${err}`,()=>{});
    socket.destroy();
    });
    // 
    socket.setTimeout(1000*60*30, function () {
...
server.tcpServer1008 = net.createServer(function (socket) {
  ...
  let buffer;
  let blnDEBUG = false;
  let _PortID = socket.address().port;
  let _PortIDFile = `_${_PortID}`;
  let _RAWDATAFILE = '_TDK';
  let _PortALERTFILE = '_TDKALERT';
  let _LOGS_SENSORS_RCV = false;
  let _REGISTERKEYS = ["88888888","78563412","50512412","37518050","83709728","2345678901"];
  // -----------------------------
  let datetimeNow = new Date();
  let hourNow = String(datetimeNow.getHours()).padStart(2,"0");
  let minuteNow = String(datetimeNow.getMinutes()).padStart(2,"0");
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID).green}> ${MATCHKEY}|${hex_to_ascii(MATCHKEY)}|${_CHECKAPKEY}`);
      blnDEBUG && _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PortID}> ${MATCHKEY}|${hex_to_ascii(MATCHKEY)}|${_CHECKAPKEY}`,()=>{});
      // -----------------------------------
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // ---------------------------------------------------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".yellow}] ${hourNow}:${minuteNow} <${String(_PortID).green}> DEVICE=<${deviceID.green}> KEY=${MATCHKEY} AP-KEY=${_CHECKAPKEY} `);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PortID}> DEVICE=<${deviceID}> KEY=${MATCHKEY} AP-KEY=${_CHECKAPKEY} `,()=>{});
        // -------------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
...
server.tcpServer1009 = net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // ---------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".yellow}] ${hourNow}:${minuteNow} <${String(_PortID).yellow}> DEVICE=<${deviceID.yellow}> KEY=<${MATCHKEY.yellow}>`);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> DEVICE ID <${deviceID}> KEY <${MATCHKEY}|${_CHECKAPKEY}> `,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
        ...
        decoders.decodeF8L10ST(1009,buffer,function(statusCode,payload) {
          ...
          if (statusCode == 200) {
            ...
            //  -----------
            _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${'1009'}> F8L10ST DTU.ID<${payload._DTUID}> MODE<${payload._MODE}>`,()=>{});
        ...
        console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PortID).yellow}> MISS-MATCH KEY <${MATCHKEY.red}>`);
        _logs.append('_ERROR', `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
  ...
  socket.once("close", function () {
    try {
      // ------
      console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PortID).yellow}> ...${'GATEWAY DISCONNECTED'.red}...`);
      _logs.append('_ERROR', `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> GATEWAY DISCONNECTED...${clientAddress}`,()=>{});
      // ------
      server.socketArr.pop(Socket);
    } catch (e) {}
  });
...
server.tcpServer1010 = net.createServer(function (socket) {
  ...
  let _PortIDFile = `_${_PortID}`;
  var decoder = new StringDecoder("hex");
  let _REGISTERKEYS = ["50512412","37518050","83709728"];
  // --------------------------
  var datetimeNow = new Date();
  var hourNow = String(datetimeNow.getHours()).padStart(2, "0");
  var minuteNow = String(datetimeNow.getMinutes()).padStart(2, "0");
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // -----------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".yellow}] ${hourNow}:${minuteNow} <${String(_PortID).red}> DEVICE=<${String(deviceID).red}> KEY=${MATCHKEY} AP-KEY=${_CHECKAPKEY} `);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PortID}> DEVICE=<${deviceID}> KEY=${MATCHKEY} AP-KEY=${_CHECKAPKEY} `,()=>{});
        // -------------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
        ...
        decoders.decodeF8L10ST(_PortID,buffer,function(statusCode,payload) {
          ...
          if (statusCode == 200) {
            // -------------------------
            payloadString = JSON.stringify(payload);
            console.log(`..${_PortID}...DECODE F8L10ST...${statusCode }`)
            // console.log(payload)
  ...
  socket.once("close", function () {
    try {
      // -----
      console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PortID).red}> ...${'GATEWAY DISCONNECTED'.red}...`);
      _logs.append('_ERROR', `[SERVER.JS]  ${hourNow}:${minuteNow} <${String(_PortID)}> GATEWAY DISCONNECTED...${clientAddress}`,()=>{});
      // ------
      server.socketArr.pop(Socket);
    } catch (e) {}
...
server.tcpServer1011 = net.createServer(function (socket) {
  // --------
  let buffer;
  let _SHINKOIDs = [ 11,552,553,1010,1020,1030,1041,1052,1063,1073,1083,55,
    3010,3020,3031,3041,5010,5020,5031,3053,5030,1102,1092,3062,6013,6023,6033 ]
  let blnDEBUG = false;
  let _PortID = socket.address().port;
  let _PortIDFile = `_${_PortID}`;
  let _SETTINGFILE = 'shinko';
  let _RAWDATAFILE = '_SHINKO';
  let _PortALERTFILE = '_SHINKOALERTS';
  let _REGISTERKEYS = ["32004851","2345678901","83709728","71589301","50512412","78563412"];
  // ----------------------------------------
  var datetimeNow = new Date();
  var hourNow = String(datetimeNow.getHours()).padStart(2, "0");
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // ---------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} <${String(_PortID).blue}> DEVICE=<${deviceID.blue}> KEY=<${MATCHKEY.blue}> REMOTE=<${String(socket.remotePort).blue}:${String(socket.remoteAddress)}>`);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PortID}> DEVICE=<${deviceID}> KEY=<${MATCHKEY}|${_CHECKAPKEY}> REMOTE=<${String(socket.remotePort)}:${String(socket.remoteAddress)}>`,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
        ...
        let _HEADER = DataArr1[1].substring(0, 2).toString("UTF-8").toUpperCase();
        // --------
        console.log(`TCPSERVER-${_PortID}`,hex_to_ascii(MATCHKEY),_HEADER,DataArr.length);
        // --------
        if (DataArr.length < 4) {
          if (_HEADER === 'FA') {
            ...
            decoders.decodeWISensorV2(_PortID,buffer,function (statusCode, payload) {
              // _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} DECODERS DECODEWISENSORV2 <${statusCode}>...`,()=>{})
              const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
              if (statusCode == 407 )  {
                // 407 NO SENSOR ID FOUND IN DATABASE FOR ALERTS TRIGGER
                // NEED TO CHECK PORT SPECIFIC CLIENT SENSORS SETTINGS
                try
                {
                  // console.log(statusCode,_PortIDFile,modelID,modelType,Temperature,BATT,INTERVAL);
                  _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} ..${modelID}|${Temperature}|${BATT}|${INTERVAL}`,()=>{})
                  //  ---------------
                  //  CHECK ALERTS ..
                  //  ---------------
                  _data.read(_SETTINGFILE,'settings',function(err,settingData) {
                    //  -------------
                    // WI SENSOR ONLY
                    // --------------
                    ...
                    // console.log(`PAYLOAD _DTUID=${_DTUID} MODE=${_MODE} HEX=${_HEX}`)
                    ...
                    if (_ALERT && _FLAG) {
                      ...
                      _logs.append('_MREALERTS',jsonPayload,()=>{});
            ...
            decoders.decode485Sensor(_PortID,DataArr,function(statusCode,payload) {
              console.log(`TCPSERVER-${_PortID}`,payload);
              if (statusCode != 200)  {
                try {
                  const {FUNCID,DTUID,SENSORID,NDATA,RCV_BYTES,DATAS,} = payload;
                  _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} DECODE 485 SENSOR <${statusCode}> ${DTUID}|${SENSORID}`,()=>{});
              ...
              _data.read(_SETTINGFILE,'settings',function(err,settingData) {
                ...
                switch (_TYPE) {
                  ...
                  case 'TEMP & RH':
                    ...
                    // console.log(`.<${_NAME}>. .BEFORE.${_READINGTEMP}|${_OFFSET_TEMP}|..  ..${_READINGRH}|${_OFFSET_RH}|..`);
                    _READINGTEMP += _OFFSET_TEMP;
                    _READINGRH += _OFFSET_RH;
                    _ALERTObj['TEMP'] = _READINGTEMP;
                    _ALERTObj['RH'] = _READINGRH;
                    // console.log(`.<${_NAME}>. .AFTER.${_READINGTEMP}|${_OFFSET_TEMP}|..  ..${_READINGRH}|${_OFFSET_RH}|..`);
                    let _FLAGTEMPRH = false;
                    _FLAG = (_TEMPMAX == null) ? false : (_READINGTEMP > _TEMPMAX) ? true: false;
                    _FLAG && (_MESSAGE += ` TEMP=${_READINGTEMP}>${_TEMPMAX}C`);
                    _FLAGTEMPRH = _FLAGTEMPRH || _FLAG;
                    ...
                    _FLAG && (_MESSAGE += ` RH=${_READINGRH}<${_RHMIN}%`);
                    _FLAG = _FLAGTEMPRH || _FLAG;
                    // console.log(`.<${_NAME}>.. ${_FLAG} .. ${_MESSAGE}`);
                    break;
                  case 'DEW PT.METER':
                    let _FLAGDEW = false;
                    _READINGRH = hexToSignedInt(_HEX.slice(0,4))/10;
                ...
                //  -------------
                (blnDEBUG) && (_TYPE==='TEMP & RH') && console.log(`..<${_PortID}> (${_RAWDATAFILE}) ID[${Number(payload._DTUID)}] NAME=${_NAME} GROUP=${_GROUP} TYPE=${_TYPE} <${_FLAG}|${_ALERT}> ${_READINGTEMP}C=[${_TEMPMAX}C|${_TEMPMIN}C]..`);
                // ----------------- 
                if (_MODE != 'RS485') return;
                if (_FLAG) {
                  _ALERTObj['MESSAGE'] = _MESSAGE;
                  let jsonPayload = JSON.stringify(_ALERTObj);
                  (blnDEBUG) && (_TYPE==='TEMP & RH') && console.log(`..<${_PortID}>.. ${_MESSAGE}`);
                  try {
                    _logs.append(_PortALERTFILE,jsonPayload,()=>{});
                    _ALERT && decoders.WhatsAppGateway2('Shinko IOT Monitoring System',_MESSAGE)
                  } catch (e) {
                    console.log(`..<${_PortID}> (${_RAWDATAFILE}) ${e}`)
                  }
                }
              })      
              if (payload._SENSORID != 0) _logs.append(_RAWDATAFILE,jsonPayload,()=>{});
            } else {
              console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID).green}> ${payload._DTUID}`);
              _logs.append('_ERROR',jsonPayload,()=>{});
            }
          }
        })
        // -----------
        ...
        console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PortID).yellow}> MISS-MATCH KEY <${MATCHKEY.yellow}>`);
        _logs.append(_PortIDFile, `[SERVER.JS]  ${hourNow}:${minuteNow} <${String(_PortID)}> MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
      }
    } 
  });
  ...
  socket.setTimeout(1000*60*30, function () {
    console.log();
    console.log(String(`.....[TIMER ${_PortID}] DISCONNECTING CLIENT ...`).green);
...
server.tcpServer1012= net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // ---------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} <${String(_PortID).cyan}> DEVICE=<${deviceID.cyan}> KEY=${MATCHKEY} AP-KEY=${_CHECKAPKEY} `);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PortID}> DEVICE=<${deviceID}> KEY=${MATCHKEY} AP-KEY=${_CHECKAPKEY} `,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
        ...
        let _HEADER = DataArr1[1].substring(0, 2).toString("UTF-8").toUpperCase();
        // --------
        console.log(`  ..  [SERVER.JS] ${hourNow}:${minuteNow} PORT 1012 SNOWCITY.. HEX TO ASCII:<${hex_to_ascii(MATCHKEY)}>`);
        // ----------
        ...
        if (DataArr.length < 4) {
          if (_HEADER === 'FA') {
            ...
            // ---------
            decoders.decodeWISensorV2(_PortID,buffer,function (statusCode, payload) {
              _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} SNOWCITY decodeWISensorV2 <${statusCode}> ${payload}`,()=>{})
              if (statusCode != 200 )  {
                try
                {
                  const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
          ...
          // ---------
          _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} decodeWISensorV1 <${buffer.length}>`,()=>{})
          // ---------
          decoders.decodeWISensorV1(_PortID,DataArr,function (statusCode, payload) {
            if (statusCode != 200)  {
              try
        ...
        // -------------
        // AP MODE -----
        // THE CALLING TO WISENSOR IS VIA DECODEF8L10ST..
        // -------------
        decoders.decodeF8L10ST(_PortID,buffer,function(statusCode,payload) {
          // -------------------
          if (statusCode == 200) {
            ...
            var jsonPayload = JSON.stringify(sensorObj);
            // ---------------------------------------
            // PROCESS BOTH WI-SENSOR AND F8L10ST DATA
            _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} APKEY <${_CHECKAPKEY}> DTU.ID=${payload?._DTUID} MODELID=${payload?.modelID}`,()=>{})
            //  -----------
            if (_SNOWCITYIDs.includes(Number(payload._DTUID))) {
              ...
              _data.read(_SETTINGFILE,'settings',function(err,settingData) {
                ...
                if (_ALERT && _FLAG) {
                  ...
                  decoders.WhatsAppGateway1('SNOWCITY IOT Monitoring System',_MESSAGE);
                }
              })      
  ...
  socket.once("close", function () {
    ...
    // ----------
    console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PortID)}> ...${'GATEWAY DISCONNECTED'.red}...`);
  });
  //  -------------------------
  //  When There Is An Error...
  //  -------------------------
...
// ----------------------------
//  INSTANTIATE TCP SERVER 1013
//  NIPPONGLASS
//  ---------------------------
server.tcpServer1013= net.createServer(function (socket) {
  // --------
  let buffer;
  let blnDEBUG = false;
  let _PortID = 1013;
  let _PortIDFile = `_${_PortID}`;
  let _SETTINGFILE = 'nipponglass';
  let _PortRAWDATAFILE = `_MRE`;
  let _PortALERTFILE = `_MREALERTS`;
  let _LOGFle = '_NIPPONGLASS';
  let _DTUIDs = [100,11];
  let _REGISTERKEYS = ["32004851","2345678901","83709728","71589301","50512412","78563412","12345678","88888888","FDEE2244"];
  // -----------------------------
  var datetimeNow = new Date();
  var hourNow = String(datetimeNow.getHours()).padStart(2, "0");
  var minuteNow = String(datetimeNow.getMinutes()).padStart(2, "0");
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      let _CHECKAPKEY = buffer.substring(0, 2).toString("UTF-8").toUpperCase();
      blnDEBUG && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${_PortID.toString().green}> ..KEY=<${MATCHKEY.red}>..${buffer}.`);
      _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> <${MATCHKEY}|${_CHECKAPKEY}>..${buffer}`,()=>{});
      // -------------------------------------
      // 78563412 - TDK, 71589301 - JAMES HOME
      // -------------------------------------
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // ---------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> DEVICE ID=<${deviceID}>`,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
          ...
          (nArray > -1) && server.socketArr.pop(nArray);
          server.socketArr.push(gatewayData);
          gatewayTracker.INSERT(gatewayData)
        } else {
          console.log("ERROR PROCESSING PACKET [%s] FROM LORA GATEWAY[%s] >>",buffer,clientAddress);
      ...
      } else if (["FA","FE","FD"].includes(_CHECKAPKEY) ) {
        // -------------
        // AP MODE -----
        blnDEBUG && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${_PortID.toString().green}> ..<${_CHECKAPKEY.red}>..`);
        // -------------
        decoders.decodeF8L10ST(_PortID,buffer,function(statusCode,payload) {     
          if (statusCode == 200) {
            //  ------------------------------------
            ...
            let sensorObj = getSensorObj(payload.PORTID,payload._DTUID,payload._MODE,payload._SENSORID,payload._DATAHEX);
            var jsonPayload = JSON.stringify(sensorObj);
            blnDEBUG && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${_PortID.toString().green}> ..DECODEF8L10ST..DTU=<${payload._DTUID}>..DATA=${payload._DATAHEX}`);
            //  -----------
            if (_DTUIDs.includes(Number(payload._DTUID))) {
              ...
              _data.read(_SETTINGFILE,'settings',function(err,settingData) {
                ...
                _FoundSensor = _FoundSensor["1"];
                blnDEBUG && console.log(_FoundSensor);
                //  -------------------------
                let _TYPE = _FoundSensor.TYPE;
                let _NAME = _FoundSensor.NAME;
                let _GROUP = _FoundSensor.GROUP;
                ...
                if (_ALERT && _FLAG) {
                  _ALERTObj['MESSAGE'] = _MESSAGE;
                  let jsonPayload = JSON.stringify(_ALERTObj);
                  // _logs.append('_AEROSOFTALERTS',jsonPayload,()=>{});
                  // decoders.WhatsAppGateway1('IOT Monitoring System',_MESSAGE)
                }
              });
        ...
        console.log(`[${'SERVER.JS'.white}] ${hourNow}:${minuteNow} <${String(_PortID).green}> MISS-MATCH KEY <${MATCHKEY.red}>`);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
      }
    } 
  });
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        ...
        // -----------------------
        let _HEADER = DataArr1[1].substring(0, 2).toString("UTF-8").toUpperCase();
        blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID).green}> ${_HEADER}`);
        blnDEBUG && _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PortID}> ${_HEADER}`,()=>{});
        // -------
        if (DataArr.length < 4) {
          if (_HEADER === 'FA') {
            _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} DECODE F8L10ST ATMODE`,()=>{})
            ...
            decoders.decodeWISensorV2(_PortID,buffer,function (statusCode, payload) {
              const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
              blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} [${statusCode}] ${modelType} <${String(modelID).green}> `);
              blnDEBUG && _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} [${statusCode}] <${_PortID}> ${modelType} <${modelID}> `,()=>{});    
              if (statusCode == 407 )  {
                try
                {
                  // 407 NO SENSOR ID FOUND IN DATABASE FOR ALERTS TRIGGER
                  // NEED TO CHECK PORT SPECIFIC CLIENT SENSORS SETTINGS
                  blnDEBUG && _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} ..${modelID}|${Temperature}|${BATT}|${INTERVAL}`,()=>{})
                  //  ---------------
                  //  CHECK ALERTS ..
                  //  ---------------
                  _data.read(_SETTINGFILE,'settings',function(err,settingData) {
        ...
        // THE CALLING TO WISENSOR IS VIA DECODEF8L10ST..
        // -------------
        console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} AP MODE <decodeF8L10ST>.`);
        decoders.decodeF8L10ST(_PortID,buffer,function(statusCode,payload) {
          // -------------------
          if (statusCode == 200 || statusCode == 407) {
            //  ------------------------------------
            payloadString = JSON.stringify(payload);
            let sensorObj = getSensorObj(payload.PORTID,payload._DTUID,payload._MODE,payload._SENSORID,payload._DATAHEX);
            var jsonPayload = JSON.stringify(sensorObj);
            console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} <${_PortID.toString().magenta}> STATUS CODE <${statusCode}> ..${payload.modelID}`);
            //  ---------------
            //  CHECK ALERTS ..
            //  ---------------
            _data.read(_SETTINGFILE,'settings',function(err,settingData) {
              ...
              switch (_TYPE) {
                ...
                case 'DEW PT.METER':
                  ...
                  break;
                case 'WISENSOR':
                  _ALERTObj['TEMP'] = payload.Temperature;
                  _ALERTObj['RH'] = payload.Humidity;
                  _FLAG = (_TEMPMAX == null) ? false : (payload.Temperature > _TEMPMAX) ? true: false;
                  _FLAG && (_MESSAGE += ` TEMP=${payload.Temperature}>${_TEMPMAX}C`);
                  _FLAGTEMPRH = _FLAGTEMPRH || _FLAG;
                  _FLAG = (_TEMPMIN == null) ? false : (payload.Temperature < _TEMPMIN) ? true: false;
                  _FLAG && (_MESSAGE += ` TEMP=${payload.Temperature}<${_TEMPMIN}C`);
                  _FLAGTEMPRH = _FLAGTEMPRH || _FLAG;
                  _FLAG = (_RHMAX == null) ? false : (payload.Humidity > _RHMAX) ? true: false;
                  _FLAG && (_MESSAGE += ` RH=${payload.Humidity}>${_RHMAX}%`);
                  _FLAGTEMPRH = _FLAGTEMPRH || _FLAG;
                  _FLAG = (_RHMIN == null) ? false : (payload.Humidity < _RHMIN) ? true: false;
                  _FLAG && (_MESSAGE += ` RH=${payload.Humidity}<${_RHMIN}%`);
                  _FLAG = _FLAGTEMPRH || _FLAG;
                  break;
                default:
                  break;
              ...
              //  -------------
              _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} <${_TYPE}> DTU.ID=${payload._DTUID} <${buffer.length}|${buffer}> HEX=${payload._DATAHEX}`,()=>{})
              if (_ALERT && _FLAG) {
                _ALERTObj['MESSAGE'] = _MESSAGE;
                let jsonPayload = JSON.stringify(_ALERTObj);
                decoders.WhatsAppGateway2(_FoundSensor.ALERTGROUP,_MESSAGE)
            ...
            if (payload._SENSORID != 0) _logs.append(_PortRAWDATAFILE,jsonPayload,()=>{});
          } else {
            console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} ..<${statusCode}>..`);
            // const {modelID,modelType,Temperature,Humidity} = payload
            // _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} WISENSOR<${modelID}> <${Temperature}|${Humidity}> ..`,()=>{})
          }
        })
        ...
        // -----
        console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PortID).cyan}> MISS-MATCH KEY <${MATCHKEY.cyan}>`);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}>  MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
      }
    } 
  });
...
// ---------------------------
// INSTANTIATE TCP SERVER 1088
// INTERNAL TEST GATEWAY
// ---------------------------
server.tcpServer1088= net.createServer(function (socket) {
  // --------
  let buffer;
  let blnDEBUG = true;
  let _PortID = socket.address().port;
  let _PortIDFile = `_${_PortID}`;
  let _SETTINGFILE = 'AEROSOFT';
  let _PortRAWDATAFILE = `_AEROSOFT`;
  let _PortALERTFILE = `_AEROSOFT`;
  let _REGISTERKEYS = ["32004851","2345678901","83709728","71589301","50512412","78563412","12345678","88888888","0300002F"];
  // -----------------------------
  var datetimeNow = new Date();
  var hourNow = String(datetimeNow.getHours()).padStart(2, "0");
  var minuteNow = String(datetimeNow.getMinutes()).padStart(2, "0");
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      let _CHECKAPKEY = buffer.substring(0, 2).toString("UTF-8").toUpperCase();
      blnDEBUG && console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} <${String(_PortID).bgGreen}> ..<${MATCHKEY.red}>..${buffer}.`);
      _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> <${MATCHKEY}|${_CHECKAPKEY}>..${buffer}`,()=>{});
      // -------------------------------------
      // 78563412 - TDK, 71589301 - JAMES HOME
      // -------------------------------------
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        ...
        console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} <${String(_PortID).bgGreen}> DEVICE=<${deviceID}> KEY=<${MATCHKEY.blue}> REMOTE=<${String(socket.remotePort).blue}:${String(socket.remoteAddress)}>`);
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
        ...
        let _HEADER = DataArr1[1].substring(0, 2).toString("UTF-8").toUpperCase();
        // --------
        // console.log(`  ..  [SERVER.JS] ${hourNow}:${minuteNow} ${_PortID}.. HEX TO ASCII:<${hex_to_ascii(MATCHKEY)}>`);
        // _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} ..${hex_to_ascii(MATCHKEY)} == +RCV`,()=>{})
        // -------
        if (DataArr.length < 4) {
          if (_HEADER === 'FA') {
        ...
        decoders.decodeF8L10ST(_PortID,buffer,function(statusCode,payload) {
          ...
          if (statusCode == 200 || statusCode == 407) {
            ...
            _data.read(_SETTINGFILE,'settings',function(err,settingData) {
              ...
              //  -------------
              console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} <${_PortID.toString().magenta}> <${_TYPE}> ${_ALERT}|${_FLAG}  ..${_FoundSensor.ALERTGROUP}.. ${_MESSAGE}`);
              _logs.append(_PortIDFile,`[SERVER.JS] ${hourNow}:${minuteNow} <${_TYPE}> DTU.ID=${payload._DTUID} <${buffer.length}|${buffer}> HEX=${payload._DATAHEX}`,()=>{})
              if (_ALERT && _FLAG) {
                _ALERTObj['MESSAGE'] = _MESSAGE;
                let jsonPayload = JSON.stringify(_ALERTObj);
        ...
        // -----
        console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PortID).green}> MISS-MATCH KEY <${MATCHKEY.green}>`);
        _logs.append(_PortIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PortID)}> MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
      }
    } 
  });
...
server.init = function () {
  ...
  // ---------------------------------------
  startTCPServer('TDK',config.tcpPort1008);
  startTCPServer('IKN',config.tcpPort1010);
