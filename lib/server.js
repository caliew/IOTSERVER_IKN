...
function startTCPServer(FileName,PortID,blnDEBUG) {
  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              decoders.decodeWISensorV2(_PORTID,buffer,function (statusCode, payload) {
                ...
                blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [decodeWISensorV2] HEADER=[${_HEADER}] <${statusCode}> [${modelID}]`);
                ...
                if (statusCode == 407 )  {                  
                  _data.read(FileName,'settings',function(err,settingData) {
                    ...
                    //  -------------
                    blnDEBUG && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${_PORTID.toString().green}> ..FOUND SENSOR..ALERT=${_ALERT} FLAG=<${_FLAG}>..MESSAGE=${_MESSAGE}`);
          ...
          // AP MODE ----
          // ------------
          blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [AP MODE] ${_RAWFile} <${_APKEY}>`);
          blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [AP MODE] ${_RAWFile} <${_APKEY}>`,()=>{});
          // -------------
          let _DTUIDs = [100,11,269];
          ...
          decoders.decodeF8L10ST(_PORTID,buffer,function(statusCode,payload) {     
            if (statusCode == 200) {
              ...
              blnDEBUG && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${_PORTID.toString().green}> ..DECODEF8L10ST..${_RAWFile.cyan}.. DTU=<${payload._DTUID}>..DATA=${payload._DATAHEX}`);
              blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> ..DECODEF8L10ST..${_RAWFile}.. DTU=<${payload._DTUID}>..DATA=${payload._DATAHEX}`,()=>{});
              // ------------
              // CHECK ALERTS
              // ------------
              _data.read(FileName,'settings',function(err,settingData) {
    ...
    //  -------------------------
    socket.on("error", function (err) {
      console.log(`[${'SERVER.JS'.yellow}] ${hourNow}:${minuteNow} <${String(_PORTID).green}> ...${'GATEWAY SOCKET ERROR'}..${err.errno}|${err.code}..`);
