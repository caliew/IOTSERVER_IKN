  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY == "88888888" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        ...
        _logs.append('_ERROR', `[SERVER.JS]  PORT <${'1008'}> DEVICE ID <${deviceID}> KEY <${MATCHKEY}> PORT <${String(socket.remotePort)}>`,()=>{});
        // -------------
        if (deviceID) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY == '32004851' || MATCHKEY == '2345678901' ||MATCHKEY == "83709728" || MATCHKEY == "71589301" || MATCHKEY == "50512412" ) {
        ...
        _logs.append('_ERROR', `[SERVER.JS]  PORT <${'1009'}> DEVICE ID <${deviceID}> KEY <${MATCHKEY}> PORT <${String(socket.remotePort)}>`,()=>{});
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY=='80A0001B' || MATCHKEY == "37518050" || MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        ...
        _logs.append('_ERROR', `[SERVER.JS]  PORT <${'1010'}> DEVICE ID <${deviceID}> KEY <${MATCHKEY}> PORT <${String(socket.remotePort)}>`,()=>{});
        // -------------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
