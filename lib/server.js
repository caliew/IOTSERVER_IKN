  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        ( false && blnDEBUG ) && console.log(`[${"SERVER.JS".yellow}] ${hourNow}:${minuteNow} <${String(_PORTID).green}> >${String(_RAWFile).cyan}< MATCH KEY=${hex_to_ascii(MATCHKEY)} APKEY=${_APKEY}`);
        // -----------------------------------
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          ...
          // AP MODE ----
          // ------------
          // blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> >${String(_RAWFile).cyan}< [${String("AP MODE").magenta}] <${_APKEY}>`);
          // blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> [AP MODE] ${_RAWFile} <${_APKEY}>`,()=>{});
          // -------------
          // let _DTUIDs = [100,11,269];
          // let _SHINKOIDs = [ 11,552,553,1010,1020,1030,1041,1052,1063,1073,1083,55,3010,3020,3031,3041,5010,5020,5031,3053,5030,1102,1092,3062,6013,6023,6033 ]
          ...
          decoders.decodeF8L10ST(_PORTID,buffer,function(statusCode,payload) {     
            if (statusCode == 200 || statusCode == 407) {
              ...
              _data.read(FileName,'settings',function(err,settingData) {
                ...
                //  B0-B4-1A-17-9E-33
                //  B0-7E-29-E2-71-A1
                (blnDEBUG) && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${_PORTID.toString().green}> >${String(_RAWFile).cyan}< [${String(statusCode).yellow}] ..DECODEF8L10ST.. [A|F=${String(_ALERT).yellow}|${String(_FLAG).cyan}] <${String(_TYPE).yellow}:${_DTUID}> ::${String(_MESSAGE).bgGreen}`);
                if (_ALERT && _FLAG) {
                  _ALERTObj['MESSAGE'] = _MESSAGE;
                  let jsonPayload = JSON.stringify(_ALERTObj);
                  if (statusCode == 407) {
...
server.init = function () {
  ...
  startTCPServer('AEROSOFT',config.tcpPort1088,true);
