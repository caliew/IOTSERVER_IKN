  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY == "88888888" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        ...
        console.log(`[${'SERVER.JS'.yellow}] PORT <${'1008'.green}> DEVICE ID <${deviceID.green}> KEY <${MATCHKEY.green}> ...PORT<${String(socket.remotePort).green}>... `);
        // -------------
        if (deviceID) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY == '32004851' || MATCHKEY == '2345678901' ||MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // ---------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".yellow}] PORT <${'1009'.yellow}> DEVICE ID <${deviceID.yellow}> KEY <${MATCHKEY.yellow}> ...PORT<${String(socket.remotePort).yellow}>... `);
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
        ...
        decoders.decodeF8L10ST(1009,buffer,function(statusCode,payload) {
          ...
          if (payload) {
            ...
            console.log(`[${"SERVER.JS".green}] <${'1009'.yellow}> STATUS CODE=${statusCode}  ...decodeF8L10ST... MODE=<${payload._MODE}>.. 
              ...
              ...HEX=${payload._DATAHEX}..INT=${payload._DATAINT} <${String(payload._READING1).red}||${String(payload._READING2).red}`)
            _logs.append('_NIPPONDEMO',`${payloadString}`,()=>{});
          }
        })
        // -----------
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY=='80A0001B' || MATCHKEY == "37518050" || MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // -----------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${'SERVER.JS'.yellow}] PORT <${'1010'.red}> DEVICE ID <${deviceID.red}> KEY <${MATCHKEY.red}> ...PORT<${String(socket.remotePort).red}>... `);
        // -------------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
