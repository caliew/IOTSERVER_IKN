...
server.tcpServer1008 = net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (false && buffer.length < 45) console.log(`[${"SERVER.JS".yellow}] PORT ID=<${'1008'.green}> ${buffer.toString("UTF-8").toUpperCase()}....${MATCHKEY.green}`);
      // -----------------------------------
...
server.tcpServer1009 = net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      // --------
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      if (false && buffer.length < 45) console.log(`[${"SERVER.JS".yellow}] PORT ID=<${'1009'.yellow}> ${buffer.toString("UTF-8").toUpperCase()}....${MATCHKEY.yellow}`);
      // ------
      // 78563412 - TDK
      // 71589301 - JAMES HOME
      // FE
        ...
        decoders.decodeF8L10ST(1009,buffer,function(statusCode,payload) {
          ...
          if (statusCode == 200) {
            ...
            */            
            ...
            if (SHINKOIDs.includes(payload._DTUID)) {
...
server.tcpServer1010 = net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      // -----------
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      if (false && buffer.length < 45) console.log(`[${"SERVER.JS".yellow}] PORT ID=<${'1010'.red}> ${buffer.toString("UTF-8").toUpperCase()}....${MATCHKEY.red}`);
      // ------------
