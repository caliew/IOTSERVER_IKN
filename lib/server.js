      ...
      if (buffer.length > 4) {
        ...
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                ...
                _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:485 ${String(_PORTID).cyan} ${String(FileName).green} -decode485Sensor- .${String(statusCode).blue}. HEADER[${String(_HEADER).red}] |${String(DTUID??'-').magenta}|${String(SENSORID??'-').red}|`);
                // ------
                if (statusCode == 200)  {
            ...
            decoders.decodeWISensorV1(1008,DataArr,function (statusCode, payload) {
              ...
              let _HUMD = Humidity ?? '';
              _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:506 ${String(_PORTID).cyan} ${String(FileName).green} -decodeWISensorV1- .${String(statusCode).blue}. HEADER[${String(_HEADER).red}] |${String(_MODELTYPE??'-').cyan}|${String(_MODELID??'-').toUpperCase().magenta}|${String(_TEMP??'-').red}|${String(_HUMD??'-').cyan}|`);
              // ------
              if (statusCode == 200)  {
          ...
          decoders.decodeF8L10ST(_PORTID,buffer,function(statusCode,payload) {
            ...
            _DEBUGINT && blnDEBUG && (statusCode != 407) && console.log(`[${"SERVER.JS".yellow}] [${"-AP-".red}] LINE:529 ${String(_PORTID).cyan} ${String(FileName).green} -decodeF8L10ST- .${String(statusCode).blue}. ${String(_MODE).red}|${String(_TYPE).blue}|${String(_DTUID).magenta}|${String(_NAME).cyan}`);
            // -----
            if (statusCode == 407) {
              //  ------------------------------------
              payloadString = JSON.stringify(payload);
              if (_MODE == 'HEART-BEAT') {
                ...
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  // -------
                  _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"-AP-".red}] LINE:728 ${String(_PORTID).cyan} ${String(FileName).green} -decodeF8L10ST- .${String(statusCode).blue}. |${String(_MODE).red}|${String(_TYPE).blue}|${String(_DTUID).magenta}|${String(_NAME).cyan}||${_READINGCURRENT??'-'}A|${_READINGPRESSURE??'-'}Pa|${_READINGTEMP??'-'}C|${_READINGRH??'-'}%|${_READINGDEW??'-'}C|`);
                  // -------
                  var jsonPayload = JSON.stringify(sensorObj);
                  if (payload._SENSORID >= 0) _logs.append(_RAWFile,jsonPayload,()=>{});  
                });
