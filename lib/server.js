...
function startTCPServer(FileName,PortID,blnDEBUG) {
  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] LINE:277 <${_PORTID}> <${clientAddress}>`,()=>{});
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        ( blnDEBUG ) && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [-] APKEY=[${_APKEY}] REGKEY=${MATCHKEY}:${ASCIIKEY??'ASCIIKEY'} ..BUFFER=${buffer.length}|${buffer}`,()=>{});
        ( blnDEBUG ) && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).rainbow}> [-] APKEY=[${String(_APKEY).red}] REGKEY=[${String(MATCHKEY).green}|${String(ASCIIKEY??'ASCIIKEY').red}] ..BUFFER=${String(buffer.length).yellow}|${String(truncateString(buffer,70)).green}`);
        // -----------------------------------
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          ...
          let _CHECKHEADERTEXT = DataArr1[1] ?? DataArr1[0];
          ...
          // --------------------------------------------------
          ( blnDEBUG ) && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [${String(_CMODE)}] HEADER=${String(_HEADER)} DATAARR=[${String(DataArr.length)}]`,()=>{});
          ( blnDEBUG ) && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] HEADER=${String(_HEADER).green} DATAARR=[${String(DataArr.length).yellow}]`);
          // ---------------------  
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              // F8L10ST - SELF BATTERY
              blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> [${_CMODE.red}] HEADER=${String(_HEADER).yellow} =decodeF8L10ST_ATMODE= [${String(DataArr).magenta}]`);
              blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID)}> [${_CMODE}] HEADER=${String(_HEADER)} =decodeF8L10ST_ATMODE= [${String(DataArr)}]`,()=>{});
              decoders.decodeF8L10ST_ATMODE(_PORTID,buffer,blnDEBUG,function(statusCode,payload) {
                // ------
                _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:350 ${String(_PORTID).yellow} ${String(FileName).green} =decodeF8L10ST_ATMODE= .${String(statusCode).blue}. HEADER[${String(_HEADER).red}]`);
                // ------
              ...
              // --------    
            } else if (["80","01","FD"].includes(_APKEY)) {
              // -------------------------------------------------------------
              // WISENSOR NEW PROTOCOL V2.10 HEADER = '80' > CALL NEW DECODER
              // -------------------------------------------------------------
              !blnSKIPWISENSOR2 && blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] HEADER=${String(_HEADER).yellow} =decodeWISensorV2= [${String(DataArr?.length).red}][${String(DataArr).magenta}]`);
              !blnSKIPWISENSOR2 && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID)}> [${_CMODE}] HEADER=${String(_HEADER)} =decodeWISensorV2= [${DataArr?.length}][${String(DataArr)}]`,()=>{});
              decoders.decodeWISensorV2(_PORTID,blnDEBUG,DataArr[0],function (statusCode, payload) {
                ...
                if (statusCode == 407 )  {                  
                  _data.read(FileName,'settings',function(err,settingData) {
                    ...
                    //  -------------
                    !blnSKIPWISENSOR2 && blnDEBUG && console.log(`[${"SERVER.JS".white}] ${GetTIMEStamp()} <${String(_PORTID).yellow}> -${_CMODE.cyan}- ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV2'.red}] <${String(statusCode).magenta}> [A|F=${String(_ALERT).yellow}|${String(_FLAG).cyan}] ..MESSAGE=${_MESSAGE}`);
                    if (_ALERT && _FLAG) {
              ...
              // ------------------
              !blnSKIP485SENSOR && blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> [${_CMODE.red}] HEADER=${String(_HEADER).yellow} =decode485Sensor= [${String(DataArr).magenta}]`);
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                ...
                !blnSKIP485SENSOR && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${_CMODE.cyan}] LINE:505 ${String(_PORTID).yellow} [${String(_CMODE).red}] ${String(FileName).green} =decode485Sensor= .${String(statusCode).blue}. HEADER[${String(_HEADER).yellow}] |${String(DTUID??'-').magenta}|${String(SENSORID??'-').red}|`);
                // ------
                if (statusCode == 200)  {
            ...
            // --------
            !blnSKIPWISENSOR1 && blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> [${_CMODE.red}] HEADER=${String(_HEADER).yellow} =decodeWISensorV1= [${String(DataArr?.length).red}][${String(DataArr).magenta}]`);
            !blnSKIPWISENSOR1 && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID)}> [${_CMODE}] HEADER=${String(_HEADER)} =decodeWISensorV1= [${DataArr?.length}][${String(DataArr)}]`,()=>{});
          ...
          _CMODE = 'AP'
          blnDEBUG && console.log(`[${'SERVER.JS'.red}] ${GetTIMEStamp()} <${String(_PORTID).green}> [${String(_CMODE).red}] .LINE 542. APKEY=${String(_APKEY).green} CHECK [FA,FE,FD]...`)
          decoders.decodeF8L10ST(_PORTID,buffer,blnDEBUG,function(statusCode,payload) {
            ...
            !blnSKIP485SENSOR && blnDEBUG && (statusCode != 407) && console.log(`[${"SERVER.JS".yellow}] [${String(_CMODE).red}] LINE:552 ${_DEBUGINT}/${blnDEBUG} ${String(_PORTID).green} ${String(FileName).green} =decodeF8L10ST= .${String(statusCode).blue}. ${String(_MODE).red}|${String(_TYPE).blue}|${String(_DTUID).magenta}|${String(_NAME).cyan}`);
            // -----
            if (statusCode == 407) {
              //  ------------------------------------
              payloadString = JSON.stringify(payload);
...
server.init = function () {
  ...
  blnFLAG && startTCPServer('AEROSOFT',config.tcpPort1088,true);      // API
