...
function startTCPServer(FileName, PortID, blnDEBUG) {
  ...
  const tcpServer = net.createServer(function (socket) {
    ...
    // ---------------------------------------------------------------------------------------
    const _CHECKSENSORS = ['B0-BC-82-C4-C4-41', 'B0-2F-E0-3F-DC-9B', 'BA-75-79-51-2B-0D', 'BA-B8-04-3C-BA-6A', 'BA-D3-39-13-11-BB', 'B0-B6-C7-46-4D-53', 'BA-10-F6-DE-16-1E'];
    var decoder = new StringDecoder("hex");
    var clientAddress = `${socket.remoteAddress}>${socket.remotePort}`;
    //  ---------------------
    //  ON DATA RECEIVED ....
        ...
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              // -------------------------------------------------------------
              true && !blnSKIPWISENSOR2 && blnDEBUG && console.log(`  >>[SERVER.JS] LINE:326 ${CLOG_GMODE(_GMODE)} ${CLOG_PORTID(_PORTID)} HEADER=${String(_HEADER).yellow} [${String(DataArr?.length).red}][${String(helpers.trimHexArray(DataArr)).magenta}]`);
              true && !blnSKIPWISENSOR2 && blnDEBUG && _logs.append(_PORTIDFile, `  >>[SERVER.JS] LINE:327 ${CLOG_PORTID(_PORTID)} [${_GMODE}] HEADER=${String(_HEADER)} [${DataArr?.length}][${String(helpers.trimHexArray(DataArr2C))}]`, () => { });
                ...
                if (statusCode == 407) {
                  _data.read(FileName, 'settings', function (err, settingData) {
                    ...
                    if (_FLAG) {
                      ...
                      // FileName,PortID
                      true && console.log(`${String('DECODEWISENSORV2').green} LINE:474 ${CLOG_PORTID(_PORTID)} ${CLOG_SOURCE(FileName)} ${CLOG_ALERT(_ALERT)}|${CLOG_FLAG(_FLAG)} [${_UNITSYSTEM.green}] <${_TYPE.green}> ${_MESSAGE.red} `)
              ...
              // ------------------
              true && blnDEBUG && console.log(`  >>[SERVER.JS] LINE:483 ${helpers.GetTIMEStamp()} ${CLOG_PORTID(_PORTID)} ${CLOG_GMODE(_GMODE)} HEADER=${String(_HEADER).green} DATAARR=[${String(DataArr.length).yellow}]`);
              // ---------------------  
              true && !blnSKIP485SENSOR && blnDEBUG && console.log(`  >>[SERVER.JS] LINE:485 ${helpers.GetTIMEStamp()} ${CLOG_PORTID(_PORTID)} ${CLOG_GMODE(_GMODE)} HEADER=${String(_HEADER).yellow} [${String(helpers.trimHexArray(DataArr)).magenta}]`);
            ...
            // --------
            true && !blnSKIPWISENSOR1 && blnDEBUG && console.log(`  >>[SERVER.JS] LINE:503 ${CLOG_GMODE(_GMODE)} ${CLOG_PORTID(_PORTID)} HEADER=${String(_HEADER).yellow} [${String(DataArr?.length).red}][${String(helpers.trimHexArray(DataArr)).magenta}]`);
          ...
          // -----------------------------------
          _GMODE = 'APMOD'
          true && blnDEBUG && console.log(`  >>[SERVER.JS] LINE:655 ${CLOG_GMODE(_GMODE)} ${CLOG_PORTID(_PORTID)} APKEY=${String(_APKEY).green} CHECK [FA,FE,FD]...`)
...
function DISPATCH_ALERT(FileName, _PortALERTFILE, _FoundSensor, _ALERTObj, _ALERTFLAG, _ALERT) {
  ...
  _logs.read(_PortALERTFILE, 10, null, null, false, function (err, alertsdata) {
    ...
    let filteredData = [];
    deBUG && console.log(`  >>[SERVER.JS] LINE:1047 <${String('DISPATCH ALERT').green}> ${CLOG_SOURCE(FileName)} .[${String(_FoundSensor.ALERTGROUP).yellow}]. -FLAG=${String(_ALERTFLAG).green}|${_ALERT}|${_ALERTObj.MESSAGE.red}-`)
    if (_ALERTFLAG > 0) {
      ...
      deBUG && console.log(`  >>[SERVER.JS] LINE:1000 ${CLOG_SOURCE(FileName)} ..#ALERTS COUNT=${AlertCount}  FLAG=${_ALERTFLAG}`);
      if (AlertCount <= _ALERTFLAG) {
        false && console.log(`  >>[SERVER.JS] LINE:1002 COUNT [${AlertCount}] < FLAG [${_ALERTFLAG}]`)
        return;
      }
    }
    deBUG && console.log(`  >>[SERVER.JS] LINE:1004 ${CLOG_SOURCE(FileName)} ${CLOG_ALERT(_ALERT)} ALERT.GROUP=[${String(_FoundSensor.ALERTGROUP).yellow}] `)
    if (!_ALERT) return;
    switch (FileName) {
      case 'MRE':
      ...
      // break;
      default:
        console.log(` ${String('ALERT MISSING').inverse} ...${CLOG_SOURCE(FileName)} ${String(_FoundSensor.ALERTGROUP).yellow}=${String(_ALERTObj.MESSAGE).red}`);
        break;
    }
  })
}
