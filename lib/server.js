              ...
              decoders.decodeWISensorV2(_PORTID, blnDEBUG, buffer, function (statusCode, payload) {
                ...
                if (statusCode == 407) {
                  _data.read(FileName, 'settings', function (err, settingData) {
                    ...
                    (IS_SENSORREGISTERED || blnDEBUG) && !blnSKIPWISENSOR2 && console.log(`  >>[SERVER.JS] LINE:471 ${'DECODEWISENSORV2'.bgCyan} ${CLOG_GMODE(_GMODE)} ${CLOG_PORTID(_PORTID)} ${CLOG_ALERT(_ALERT)}|${CLOG_FLAG(evalResult.isAlert)}|${CLOG_MODE(_MODE)}] MESSAGE=${evalResult.message}`);

                    if (evalResult.isAlert) {
                      blnDEBUG && console.log(`${String('DECODEWISENSORV2'.green)} LINE:474 ${CLOG_PORTID(_PORTID)} ${CLOG_SOURCE(FileName)} ${CLOG_ALERT(_ALERT)}|${CLOG_FLAG(true)} <${_TYPE.green}> ${evalResult.message.red}`);
            ...
            decoders.decodeWISensorV1(_PORTID, blnDEBUG, DataArr, function (statusCode, payload) {
              ...
              if (statusCode == 407) {
                _data.read(FileName, 'settings', function (err, settingData) {
                  ...
                  (IS_SENSORREGISTERED || blnDEBUG) && console.log(`  >>[SERVER.JS] LINE:643 ${'DECODEWISENSORV1'.bgRed} ${CLOG_GMODE(_GMODE)} ${CLOG_PORTID(_PORTID)} ${CLOG_STATUSCODE(statusCode)} ${CLOG_ALERT(_ALERT)}|${CLOG_FLAG(evalResult.isAlert)} <${String(_TYPE).yellow}:${_DTUID}> ::${String(evalResult.message).bgGreen}`);

                  if (evalResult.isAlert) {
                    (IS_SENSORREGISTERED || blnDEBUG) && console.log(`  >>[SERVER.JS] LINE:644 ${'DECODEWISENSORV1'.bgRed} ${CLOG_GMODE(_GMODE)} ${CLOG_ALERT(_ALERT)}|${CLOG_FLAG(true)}|${CLOG_MODE(_MODE)}> <${_TYPE.green}> ${evalResult.message.red}`);
  ...
  _logs.read(_PortALERTFILE, 10, null, null, false, function (err, alertsdata) {
    ...
    switch (FileName) {
      ...
      case 'NIPPONGLASS':
        ...
        console.log('  >>[SERVER.JS] LINE:1008 WHATSAPPGTEWAY 30 ', FileName, _FoundSensor.ALERTGROUP, _ALERTObj.MESSAGE);
      ...
      case 'INDOGUNA':
        ...
        console.log('  >>[SERVER.JS] LINE:1016 WHATSAPPGTEWAY 28 ', FileName, _FoundSensor.ALERTGROUP, _ALERTObj.MESSAGE);
      ...
      case 'SHINKO':
        dispatchedGateway = 'WhatsAppGateway30';
        console.log('  >>[SERVER.JS] LINE:1022 WHATSAPPGTEWAY 30 ', FileName, _FoundSensor.ALERTGROUP, _ALERTObj.MESSAGE);
        decoders.WhatsAppGateway30(_FoundSensor.ALERTGROUP, _ALERTObj.MESSAGE);
        break;
      default:
