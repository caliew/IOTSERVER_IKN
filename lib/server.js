  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        ( false && _PORTID == '1088') && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> <KEY=${MATCHKEY}|AP=${_APKEY}> BUFFER=${buffer}`,()=>{});
        ( true && blnDEBUG ) && _logs.append(_PORTIDFile, `[SERVER.JS] ${hourNow}:${minuteNow} <${_PORTID}> <KEY=${MATCHKEY}|AP=${_APKEY}> BUFFER=${buffer}`,()=>{});
        // -----------------------------------
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          // ---------------------------------------------------
          let DEVICEID = hex_to_ascii(buffer.substring(10, 30));
          console.log(`[${"SERVER.JS".yellow}] ${hourNow}:${minuteNow} <${String(_PORTID).green}> DEVICE=<${DEVICEID.green}> [${String(_RAWFile).cyan}].. APKEY=[${_APKEY.yellow}] MATCHKEY=[${MATCHKEY.cyan}]`);
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              decoders.decodeF8L10ST_ATMODE(_PORTID,buffer,function(statusCode,payload) {
                // -------------
                blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> [${'decodeF8L10ST_ATMODE'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER}]`);
              ...
              decoders.decodeWISensorV2(_PORTID,buffer,function (statusCode, payload) {
                ...
                blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV2'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}] [${String(modelID).toUpperCase().green}]`);
                ...
                if (statusCode == 407 )  {                  
                  _data.read(FileName,'settings',function(err,settingData) {
                    ...
                    //  -------------
                    blnDEBUG && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${String(_PORTID).green}> ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV2'.red}] <${String(statusCode).magenta}> [A|F=${String(_ALERT).yellow}|${String(_FLAG).cyan}] ..MESSAGE=${_MESSAGE}`);
                    if (_ALERT && _FLAG) {
              ...
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                if (statusCode == 200)  {
                  ...
                  blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> ..[${String(_RAWFile).cyan}].. [${'decode485Sensor'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}]..${payload.DTUID}|${payload.SENSORID}|${payload.SENSORTYPE}`);
            ...
            decoders.decodeWISensorV1(1008,DataArr,function (statusCode, payload) {
              if (statusCode == 200)  {
                const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
                blnDEBUG && console.log(`[SERVER.JS] ${hourNow}:${minuteNow} <${String(_PORTID).yellow}> ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV1'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}] [${String(modelID).toUpperCase().green}]`);
            ...
            if (statusCode == 407) {
              ...
              if (payload._MODE == 'HEART-BEAT') {
                ...
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  _FoundSensor = _FoundSensor["1"];
                  false && (_PORTID == '1088') && console.log('>>',FileName,err,_DTUID,'...',_RAWFile,_MODE,_HEX,_FoundSensor?.['TYPE']);
                  //  -------------------------
                  let _TYPE = _FoundSensor.TYPE;
                  ...
                  false && (_PORTID == '1088') && console.log('>>',_TYPE,_PRESSMAX,_PRESSMIN,_OFFSET_PRESS,_UNITSYSTEM,_ALERT);
                  //  --------------
                  let _FLAG = false;
                  let _ALERTObj = {};
                  _ALERTObj['TYPE'] = _TYPE;
                  ...
                  (blnDEBUG  && _FLAG) && console.log(`[${"SERVER.JS".white}] ${hourNow}:${minuteNow} <${_PORTID.toString().green}> ..[${String(_RAWFile).cyan}].. [${'DECODEF8L10ST'.red}] <${String(statusCode).magenta}> [A|F=${String(_ALERT).yellow}|${String(_FLAG).cyan}] <${String(_TYPE).yellow}:${_DTUID}> ::${String(_MESSAGE).bgGreen}`);
                  if (_ALERT && _FLAG) {
                    _ALERTObj['MESSAGE'] = _MESSAGE;
