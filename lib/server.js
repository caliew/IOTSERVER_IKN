...
server.AlertArr = {};
// ------------------------------
const PORT1008_INT_OUTPUT = false;
const PORT1009_INT_OUTPUT = false;
const PORT1010_INT_OUTPUT = false;
...
server.tcpServer1008 = net.createServer(function (socket) {
  ...
  var clientAddress = `${socket.remoteAddress}>${String(socket.remotePort).green}`;
  // --------------
  ...
  socket.on("data", function (data) {
    ...
    let secondNow = String(datetimeNow.getSeconds()).padStart(2,"0");
    // ----------------------------------------
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY == "88888888" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        ...
        // ---------------------------------------------------
        console.log(`[${'SERVER.JS'.yellow}] PORT <${'1008'.green}> MATCH KEY <${MATCHKEY.green}> ... DEVICE ID <${deviceID.green}>.PORT<${String(socket.remotePort).green}>.`);
        // -------------
        if (deviceID) {
        ...
        // DATA PACKET ARRAY
        // -----------------
        PORT1008_INT_OUTPUT && console.log(`[${"SERVER.JS".rainbow}] ${hourNow}:${minuteNow} <${'1008'.green}> <${hex_to_ascii(MATCHKEY)}> [..${String(hex_to_ascii(buffer)).substring(0,10).toUpperCase().yellow}..]`)
        // _logs.append('_ERROR',`[SERVER.JS] <1008> <${hourNow}:${minuteNow}> .+RCV...${hex_to_ascii(buffer.toString("UTF-8"))}`,()=>{});
        ...
        if (DataArr.length < 4) {
          ...
          // -------------------------
          decoders.decode485Sensor(1008,DataArr,function(status,payload) {
            // ---------------
            let strKEY = null;
            // -----------------------------
            if (payload)  {
              ...
              _LOGS_SENSORS_RCV && _logs.append('_ERROR', `[SERVER.JS]  1008 <${hourNow}:${minuteNow}:${secondNow}> ..<${status}|${FUNCID}>   ..DTU.ID=[${String(DTUID).padStart(3,"0")}]..SNR.ID=[${String(SENSORID).padStart(2,"0")}]...|${getBYTESTRING(RCV_BYTES)}|<${DATAS}>`,()=>{});
              // --------------------------------
              ...
              // ---------
              strINFO += `<${status}|${FUNCID}>  DTU.ID=[${String(DTUID).padStart(3,"0")}] SNR.ID=[${String(SENSORID).padStart(2,"0")}]...KEY=<${strKEY}> |${getBYTESTRING(RCV_BYTES)}|<${DATAS}>`;
            } 
            // --------------------------------------------------
            if (strKEY) server.sensorsArr[strKEY] = `${hourNow}:${minuteNow} .....${strINFO}`
            PORT1008_INT_OUTPUT && console.log(`${strINFO}`)
          });
          // -------------------------------
        } else {
          // ------------
            ...
            decoders.decodeWISensorV1(1008,DataArr,function (statusCode, payload) {
              ...
              if (payload)  {
                ...
                try {
                  ...
                  PORT1008_INT_OUTPUT && console.log(`[SERVER.JS] ${"1008".green} +RCV:${hourNow}:${minuteNow}. READING..<${String(modelID).toUpperCase()}> ..<.${String(payload["Temperature"]).padStart(2,"0")}C.|.${String(payload["Humidity"]).padStart(2,"0")}%.>`)
                } catch (err) {
                  PORT1008_INT_OUTPUT && console.log(`[SERVER.JS] ${"1008".green} +RCV:${hourNow}:${minuteNow}. READING..<${err}>..`)
                }
              }
              // if (!payload) console.log(`[SERVER.JS] ${"1008".green} decodeWISensorV1 > +RCV:${hourNow}:${minuteNow}. .<${statusCode}><${payload}>..`);
            });
          }
          return;
          // ---------------
...
server.tcpServer1009 = net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    // --------------------------------------
    // socket.write('AT+TX,255,01030102')
    // ----------------------------------
    if (buffer.length > 4) {
      // --------
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      // ----------- 011 5418 0032 
      // ----------- 5418 0032 NIPPON GLASS
      // console.log(`[${"SERVER.JS".rainbow}] --<${'1009'.yellow}>--<${hourNow}:${minuteNow}>-- ${MATCHKEY}`)
      // ------
      if (MATCHKEY == '32004851' || MATCHKEY == '2345678901' ||MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // GATEWAY ------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".yellow}] PORT <${'1009'.yellow}> MATCH KEY <${MATCHKEY.yellow}> ... DEVICE ID <${deviceID.yellow}>.PORT<${String(socket.remotePort).green}>. `);
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
        ...
        // DATA PACKET ARRAY
        // ----------------
        PORT1009_INT_OUTPUT && console.log(`[${"SERVER.JS".rainbow}] ${hourNow}:${minuteNow} <${'1009'.yellow}> <${hex_to_ascii(MATCHKEY)}> [..${String(buffer).substring(0,10).toUpperCase().yellow}..]`)
        // _logs.append('_ERROR',`[SERVER.JS] <1009> <${hourNow}:${minuteNow}> .+RCV...${hex_to_ascii(buffer.toString("UTF-8"))}`,()=>{});
        var DataArr = buffer.toString("hex").toUpperCase().split("0D");
        // --------------------------------
        let DataArr1 = DataArr[0].split("2C");
        let DataArr2 = DataArr1[0].split("3A");
        ...
        if (DataArr.length < 4) {
          ...
          decoders.decode485Sensor(1009,DataArr,function(statusCode,payload) {
            ...
            if (payload)  {
              ...
              // console.log(`[${"SERVER.JS".green}] PORT <${'1009'.yellow}>...`)
              // console.log(payload);
              console.log(`[${"SERVER.JS".green}] PORT <${'1009'.yellow}> ...decode485Sensor...
                ...${statusCode}:: DTU ID=${payload.DTUID} ..RCV_BYTES=${RCV_BYTES} 
                ...SENSOR ID=${payload.SENSORID} ..<${payload.DATAS[0]}|${payload.DATAS[1]}>`)
              // console.log(payload);
              // --------------------------------
          ...
          if ( DataArr.length > 4 ){
            ...
            decoders.decodeWISensorV1(1009,DataArr,function (statusCode, payload) {
              ...
              // if (!payload) console.log(`[SERVER.JS] ${"1009".yellow} decodeWISensorV1 > +RCV:${hourNow}:${minuteNow}. .<${statusCode}><${payload}>..`);
            });
          }
          return;
          // ---------------
        ...
        decoders.decodeF8L10ST(1009,buffer,function(statusCode,payload) {
          ...
          if (payload) {
            // -------------------------
            payloadString = JSON.stringify(payload);
            // console.log(`..1009...decodeF8L10ST...${statusCode }`)
            // console.log(payload)
            console.log(`[${"SERVER.JS".green}] ${statusCode}  ...decodeF8L10ST...
              ...MODE=<${payload._MODE}>.. 
              ...<${payload.PortID}|${payload._SENSORID}|${payload._DTUID}>. <${String(payload._DATA).yellow}> = 
              ...HEX=${payload._DATAHEX}..INT=${payload._DATAINT}
              ...<${String(payload._READING1).red}||${String(payload._READING2).red}>`)
            // console.log(payload);
            _logs.append('_NIPPONDEMO',`${payloadString}`,()=>{});
          }
        })
  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY=='80A0001B' || MATCHKEY == "37518050" || MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        // GATEWAY -----
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${'SERVER.JS'.yellow}] PORT <${'1010'.red}> MATCH KEY <${MATCHKEY.red}> ... DEVICE ID <${deviceID.red}>.PORT<${String(socket.remotePort).green}>. `);
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          if (nArray > -1) {
            gatewayData = {
        ...
        // DATA PACKET ARRAY
        // ----------------
        PORT1010_INT_OUTPUT && console.log(`[${"SERVER.JS".rainbow}] ${hourNow}:${minuteNow} <${'1010'.red}> <${hex_to_ascii(MATCHKEY)}> [..${String(buffer).substring(0,10).toUpperCase().yellow}..]`)
        // _logs.append('_ERROR',`[SERVER.JS] <1010> <${hourNow}:${minuteNow}> .+RCV...${hex_to_ascii(buffer.toString("UTF-8"))}`,()=>{});
        var DataArr = buffer.toString("hex").toUpperCase().split("0D");
        // -------------------------------
        let DataArr1 = DataArr[0].split("2C");
        let DataArr2 = DataArr1[0].split("3A");
        ...
        if (DataArr.length < 4) {
          ...
          if (_HEADER === 'FA') {
            ...
            if (_HEADER === '80') {
              ...
              decoders.decodeWISensorV2(1010,buffer,function (statusCode, payload) {
                ...
                if (!payload) console.log(`[SERVER.JS] ${"1010".red} decodeWISensorV2 > +RCV:${hourNow}:${minuteNow}. .<${statusCode}><${payload}>..`);
              });
              // --------------
            } else {
              decoders.decode485Sensor(1010,DataArr,function(status,payload) {
                // ------------------
                if (payload)  {
                  // ---------
                  const {FUNCID,DTUID,SENSORID,NDATA,RCV_BYTES,DATAS,} = payload;
                  // console.log(`..${"1010".red}...decodeF8L10ST_ATMODE...${status }`)
                  // console.log(payload);
                  // --------------------------------
                  let strKEY = `${DTUID}_${SENSORID}`
                  if (server.sensorsArr.hasOwnProperty(strKEY)) { 
                  ...
                  server.sensorsArr[strKEY] = `${hourNow}:${minuteNow} .....<${status}|${FUNCID}>  DTU.ID=[${DTUID.toString().padStart(3,"0")}] SNR.ID=[${SENSORID.toString().padStart(2,"0")}]...|${getBYTESTRING(RCV_BYTES)}|<${DATAS}>`
                  // --------------------------------------------------
                }
              });
            }
          ...
          if ( DataArr.length > 4 ){
            decoders.decodeWISensorV1(1010,DataArr,function (statusCode, payload) {
              ...
              // if (!payload) console.log(`[SERVER.JS] ${"1010".red} decodeWISensorV1 > +RCV:${hourNow}:${minuteNow}. .<${statusCode}><${payload}>..`);
            });
          }
          return;
          // ---------------
    ...
    case '/upload.php':
      ...
      formESP3CAM.parse(req, function (err, fields, files) {
        if (!err){
          ...
          let minuteNow = String(datetimeNow.getMinutes()).padStart(2, "0");
          let secondNow = String(datetimeNow.getSeconds()).padStart(2, "0");
          const indexLabel = `${datetimeNow.getDate()}.${datetimeNow.getMonth()}_${hourNow}.${minuteNow}.${secondNow}`;
          // ---------------------
          if (datetimeNow == null)
            return res.end();
          // ---------------------------------
...
server.init = function () {
  ...
  server.tcpServer1008.listen(config.tcpPort1008, function () {console.log("\x1b[34m%s\x1b[0m","The TCP server is running on port " + config.tcpPort1008); });
  
  // START TCP SERVER 1009 - HOSPITAL IOT NETWORK
  server.tcpServer1009.listen(config.tcpPort1009, function () {console.log("\x1b[34m%s\x1b[0m","The TCP server is running on port " + config.tcpPort1009); });

  // START TCP SERVER 1010 - TEST IOT NETWORK
  server.tcpServer1010.listen(config.tcpPort1010, function () { console.log("\x1b[34m%s\x1b[0m","The TCP server is running on port " + config.tcpPort1010); });
