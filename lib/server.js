  ...
  socket.on("data", function (data) {
    ...
    if (buffer.length > 4) {
      ...
      if (MATCHKEY == '32004851' || MATCHKEY == '2345678901' ||MATCHKEY == "83709728" || MATCHKEY == "78563412" || MATCHKEY == "50512412" ) {
        ...
        // DATA PACKET ARRAY
        // ----------------
        // _logs.append('_ERROR',`[SERVER.JS] <1009> <${hourNow}:${minuteNow}> .${buffer.toString("UTF-8")}`,()=>{});
        var DataArr = buffer.toString("hex").toUpperCase().split("0D");
        // --------------------------------
        let DataArr1 = DataArr[0].split("2C");
        let DataArr2 = DataArr1[0].split("3A");
        ...
        if (DataArr.length < 4) {
          ...
          decoders.decode485Sensor(1009,DataArr,function(statusCode,payload) {
            ...
            if (payload)  {
              ...
              // console.log(payload);
              console.log(`[${"SERVER.JS".green}] PORT <${'1009'.yellow}> ::${statusCode}:: DTU ID=${payload.DTUID} ..RCV_BYTES=${RCV_BYTES} ..SENSOR ID=${payload.SENSORID} ..<${payload.DATAS[0]}|${payload.DATAS[1]}>`)
              // --------------------------------
