      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        ...
        decoders.decodeF8L10ST(1011,buffer,function(statusCode,payload) {              
          if (statusCode == 200) {
            ...
            let SNOWCITYIDs = [1633,1634,1635,1636]
            //  ------------------------------------
            payloadString = JSON.stringify(payload);
            let sensorObj = getSensorObj(payload.PortID,payload._DTUID,payload._MODE,payload._SENSORID,payload._DATAHEX);
            var jsonPayload = JSON.stringify(sensorObj);
              ...
              _data.read('shinko','settings',function(err,settingData) {
                ...
                switch (_TYPE) {
                  ...
                  case 'DEW PT.METER':
                    ...
                    console.log(`[SERVER.JS] DEW PT MTR HEX=${_HEX}...TEMP=${_HEX.slice(4,8)}=${_READINGTEMP}|RH=${_HEX.slice(0,4)}=${_READINGRH}|DEW=${_HEX.slice(8,12)}=${_READINGDEW}`)
                    break;
                  default:
                    break;
                }
            ...
            if (SNOWCITYIDs.includes(Number(payload._DTUID))) {
...
server.tcpServer1012= net.createServer(function (socket) {
  ...
  socket.on("data", function (data) {
    ...
    // SENDING BYTE TO 485 MODBUS (485 SENSORS)
    // ----------------------------------------
    _logs.append('_1012',`[SERVER.JS] ${hourNow}:${minuteNow} BUFFER=${buffer}`,()=>{})    
    if (buffer.length > 4) {
      // --------
      const MATCHKEY = buffer.substring(0, 8).toString("UTF-8").toUpperCase();
      let _CHECKAPKEY = buffer.substring(0, 2).toString("UTF-8").toUpperCase();
      ...
      if (_REGISTERKEYS.includes(MATCHKEY)) {
        // ---------
        let deviceID = hex_to_ascii(buffer.substring(10, 30));
        console.log(`[${"SERVER.JS".blue}] ${hourNow}:${minuteNow} <${'1012'.cyan}> DEVICE=<${deviceID.cyan}> KEY=<${MATCHKEY.blue}> REMOTE=<${String(socket.remotePort).blue}:${String(socket.remoteAddress)}>`);
        // ----------
        if (deviceID) {
          const nArray = server.socketArr.indexOf(socket);
          let gatewayData = {
        ...
        let DataArr1 = DataArr[0].split("2C");
        if (DataArr1.length<2) {
          _logs.append('_1012', `[SERVER.JS] ${hourNow}:${minuteNow} ERROR SPLICE '2C' NO VALUE <${DataArr}> ${DataArr1}..`,()=>{});
          return;
        }
        // -----------------------
        let _HEADER = DataArr1[1].substring(0, 2).toString("UTF-8").toUpperCase();
        ...
        if (DataArr.length < 4) {
          if (_HEADER === 'FA') {
            _logs.append('_1012',`[SERVER.JS] ${hourNow}:${minuteNow} DECODE F8L10ST ATMODE`,()=>{})
            decoders.decodeF8L10ST_ATMODE(1012,buffer,function(statusCode,payload) {
              if (statusCode == 200) {
                payloadString = JSON.stringify(payload);
                console.log(`..1012...DECODE F8L10ST (ATMODE)...${statusCode }`)
                // console.log(payload);
                // _logs.append('_NIPPONDEMO',`${payloadString}`,()=>{});
              }
            })
            ...
            decoders.decodeWISensorV2(1012,buffer,function (statusCode, payload) {
              if (statusCode != 200 )  {
                try
                {
                  const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
                  _logs.append('_1012', `[SERVER.JS] ${hourNow}:${minuteNow} DECODE WISENSOR V2 <${statusCode}> ${modelID}..`,()=>{});
                } catch (err) {
                  _logs.append('_1012', `[SERVER.JS] ${hourNow}:${minuteNow} DECODE WISENSOR V2 <${statusCode}> ${err}..`,()=>{});
                }
              }
            });
          } else {
            decoders.decode485Sensor(1008,DataArr,function(statusCode,payload) {
              if (statusCode != 200)  {
                try {
                  const {FUNCID,DTUID,SENSORID,NDATA,RCV_BYTES,DATAS,} = payload;
                  _logs.append('_1012', `[SERVER.JS] ${hourNow}:${minuteNow} DECODE 485 SENSOR <${statusCode}> ${DTUID}|${SENSORID}`,()=>{});
                } catch (err) {
                  _logs.append('_1012', `[SERVER.JS] ${hourNow}:${minuteNow} DECODE 485 SENSOR <${statusCode}> ${err}`,()=>{});
                }
              }
            });
          }
          ...
          decoders.decodeWISensorV1(1012,DataArr,function (statusCode, payload) {
            if (statusCode != 200)  {
              try
              {
                const {modelID,modelType,Temperature,Humidity,BATT,INTERVAL} = payload;
                _logs.append('_1012', `[SERVER.JS] ${hourNow}:${minuteNow} DECODE WISENSOR V1 <${statusCode}> ${modelID}..`,()=>{});
              } catch (err) {
                _logs.append('_1012', `[SERVER.JS] ${hourNow}:${minuteNow} DECODE WISENSOR V1 <${statusCode}> ${err}..`,()=>{});
              }
            }
          });
        }
        ...
        // AP MODE -----
        // -------------
        decoders.decodeF8L10ST(1012,buffer,function(statusCode,payload) {              
          if (statusCode == 200) {
            let SNOWCITYIDs = [1633,1634,1635,1636]
            //  ------------------------------------
            payloadString = JSON.stringify(payload);
        ...
        console.log(`[${'SERVER.JS'.yellow}] PORT ID <${'1012'.cyan}> MISS-MATCH KEY <${MATCHKEY.red}>`);
        _logs.append('_1012', `[SERVER.JS]  PORT ID <${'1012'}> MISS-MATCH KEY <${MATCHKEY}>`,()=>{});
      }
    } 
  });
  //  -----------------------------
  ...
  socket.once("close", function () {
    try {
      // ------
      console.log(`[${'SERVER.JS'.yellow}] PORT ID <${"1012".cyan}> ...${'GATEWAY DISCONNECTED'.red}...`);
      _logs.append('_ERROR', `[SERVER.JS]  PORT ID <1012> GATEWAY DISCONNECTED...${clientAddress}`,()=>{});
      // ------
      server.socketArr.pop(Socket);
    } catch (e) {}
    // ----------
  ...
  socket.setTimeout(1000*60*30, function () {
    console.log();
    console.log(".....[TIMER 1012] DISCONNECTING CLIENT ...".rainbow);
    socket.end(".....[TIMER 1012] DISCONNECTING CLIENT ...");
  });
});
// ---------------------------
...
server.init = function () {
  ...
  server.tcpServer1012.listen(config.tcpPort1012, function () {console.log(`[${'SERVER.JS'.yellow}]  ...TCP SERVER INIT On PORT ${String(config.tcpPort1012).cyan}`); });
