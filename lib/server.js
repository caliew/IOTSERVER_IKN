...
function startTCPServer(FileName,PortID,blnDEBUG) {
  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    //  ON DATA RECEIVED ....
    //  --------------------- 
    blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] LINE:268 <${_PORTID}> <${clientAddress}>`,()=>{});
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        ( true && blnDEBUG ) && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> REGKEY=${MATCHKEY}${ASCIIKEY}|APKEY=${_APKEY} ..BUFFER=${buffer.length}|${buffer}`,()=>{});
        ( true && blnDEBUG ) && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).rainbow}> REGKEY=${String(MATCHKEY).green}=${String(ASCIIKEY).red}|APKEY=${String(_APKEY).red} ..BUFFER=${String(buffer.length).yellow}|${truncateString(buffer,70)}`);
        // -----------------------------------
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          // ---------------------------------------------------
          let DEVICEID = hex_to_ascii(buffer.substring(10, 30));
          console.log(`[${"SERVER.JS".yellow}] ${GetTIMEStamp()} <${String(_PORTID).green}> DEVICE=${DEVICEID.green} [${String(_RAWFile).cyan}].. REGKEY=[${MATCHKEY.cyan}] APKEY=[${_APKEY.yellow}]`);
        ...
        } else if (ASCIIKEY == "+RCV" || ["80"].includes(_APKEY)) {  // TRANS MODE
          // ---------------
          // AT (TRANS) MODE
          // ---------------
          ...
          if (DataArr1.length<2) {
            _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} ERROR SPLICE '2C' NO VALUE <${DataArr}> ${DataArr1}..`,()=>{});
            return;
          }
          // -----------------------
          ...
          // --------------------------------------------------
          blnDEBUG && console.log(String(_PORTID).rainbow,'_HEADER',_HEADER);
          blnDEBUG && console.log(DataArr);
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              // F8L10ST - SELF BATTERY
              decoders.decodeF8L10ST_ATMODE(_PORTID,buffer,blnDEBUG,function(statusCode,payload) {
                // ------
                _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:330 ${String(_PORTID).cyan} ${String(FileName).green} -decodeF8L10ST_ATMODE- .${String(statusCode).blue}. HEADER[${String(_HEADER).red}]`);
                // ------
              ...
              decoders.decodeWISensorV2(_PORTID,buffer,function (statusCode, payload) {
                ...
                blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [decodeWISensorV2] HEADER=[${_HEADER}] <${statusCode}> [${modelID}]`,()=>{});
                // ------
                _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:351 ${String(_PORTID).cyan} ${String(FileName).green} -decodeWISensorV2- .${String(statusCode).blue}. HEADER[${String(_HEADER).red}] |${String(modelType??'-').cyan}|${String(modelID??'-').toUpperCase().magenta}|${String(Temperature??'-').red}|${String(Humidity??'-').cyan}|`);
                // ------
                if (statusCode == 407 )  {                  
                  _data.read(FileName,'settings',function(err,settingData) {
                    ...
                    //  -------------
                    blnDEBUG && console.log(`[${"SERVER.JS".white}] ${GetTIMEStamp()} <${String(_PORTID).green}> ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV2'.red}] <${String(statusCode).magenta}> [A|F=${String(_ALERT).yellow}|${String(_FLAG).cyan}] ..MESSAGE=${_MESSAGE}`);
                    if (_ALERT && _FLAG) {
              ...
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                ...
                false && _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:485 ${String(_PORTID).cyan} ${String(FileName).green} -decode485Sensor- .${String(statusCode).blue}. HEADER[${String(_HEADER).red}] |${String(DTUID??'-').magenta}|${String(SENSORID??'-').red}|`);
                // ------
                if (statusCode == 200)  {
                  blnDEBUG && console.log(`[SERVER.JS] ${GetTIMEStamp()} <${String(_PORTID).yellow}> [${"TRAN".red}] LINE:486 [${String(_RAWFile).cyan}] [${'decode485Sensor'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}]..${payload.DTUID}|${payload.SENSORID}|${payload.SENSORTYPE}`);
                  blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [decode485Sensor] HEADER=[${_HEADER}] <${statusCode}> ${SENSORTYPE} [${FUNCID}|${DTUID}|${SENSORID}]`,()=>{}); 
                } else {
                  _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()}  <${_PORTID}> [decode485Sensor] HEADER=[${_HEADER}] <${statusCode}> ..`,()=>{});
                }
              });
            }
            // ---------------
            ...
            decoders.decodeWISensorV1(1008,DataArr,function (statusCode, payload) {
              ...
              let _HUMD = Humidity ?? '';
              _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"TRAN".red}] LINE:503 ${String(_PORTID).cyan} ${String(FileName).green} -decodeWISensorV1- .${String(statusCode).blue}. HEADER[${String(_HEADER).red}] |${String(_MODELTYPE??'-').cyan}|${String(_MODELID??'-').toUpperCase().magenta}|${String(_TEMP??'-').red}|${String(_HUMD??'-').cyan}|`);
              // ------
              if (statusCode == 200)  {
                blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${GetTIMEStamp()} <${_PORTID}> [decodeWISensorV1] HEADER=[${_HEADER}] <${statusCode}> [${modelID}]`,()=>{});
              } else {
          ...
          blnDEBUG && console.log(`[${'SERVER.JS'.red}] ${GetTIMEStamp()} ...LINE 520... _APKEY=${String(_APKEY).green} CHECK [FA,FE,FD]...`)
          decoders.decodeF8L10ST(_PORTID,buffer,blnDEBUG,function(statusCode,payload) {
            ...
            _DEBUGINT && blnDEBUG && (statusCode != 407) && console.log(`[${"SERVER.JS".yellow}] [${"-AP-".red}] LINE:525 ${_DEBUGINT}/${blnDEBUG} ${String(_PORTID).cyan} ${String(FileName).green} -decodeF8L10ST- .${String(statusCode).blue}. ${String(_MODE).red}|${String(_TYPE).blue}|${String(_DTUID).magenta}|${String(_NAME).cyan}`);
            // -----
            if (statusCode == 407) {
              //  ------------------------------------
              payloadString = JSON.stringify(payload);
              if (_MODE == 'HEART-BEAT') {
                ...
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  // -------
                  _DEBUGINT && blnDEBUG && console.log(`[${"SERVER.JS".yellow}] [${"-AP-".red}] LINE:732 ${String(_PORTID).cyan} ${String(FileName).green} -decodeF8L10ST- .${String(statusCode).blue}. |${String(_MODE).red}|${String(_TYPE).blue}|${String(_DTUID).magenta}|${String(_NAME).cyan}||${_READINGCURRENT??'-'}A|${_READINGPRESSURE??'-'}Pa|${_READINGTEMP??'-'}C|${_READINGRH??'-'}%|${_READINGDEW??'-'}C|`);
                  // -------
                  var jsonPayload = JSON.stringify(sensorObj);
                  if (payload._SENSORID >= 0) _logs.append(_RAWFile,jsonPayload,()=>{});  
                });
  ...
  // ------------------------------------
  tcpServer.listen(PortID,function () {console.log(`[${'SERVER.JS'.yellow}]  INIT TCP SERVER PORT ${String(PortID).green} - DOMAIN <${FileName.cyan}>`);} );
}
// ---------------------------
...
server.init = function () {
  ...
  // ---------------------------------------
  blnSTATE && startTCPServer('TDK',config.tcpPort1008,false);          // TRANS
  blnSTATE && startTCPServer('IKN_OPROOM',config.tcpPort1009,false);   // API
  blnSTATE && startTCPServer('IKN_HOSPITAL',config.tcpPort1010,false); // TRANS
  blnSTATE && startTCPServer('SHINKO',config.tcpPort1011,false);           // API
  blnSTATE && startTCPServer('SNOWCITY',config.tcpPort1012,false);          // API
  blnSTATE &&startTCPServer('NIPPONGLASS',config.tcpPort1013,false);      // API
  blnSTATE && startTCPServer('MRE',config.tcpPort1020,false);              // TRANS
  true && startTCPServer('AEROSOFT',config.tcpPort1088,false);         // API
