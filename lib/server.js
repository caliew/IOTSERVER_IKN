          ...
          if (_HEADER === 'FA') {
            ...
            decoders.decodeF8L10ST_ATMODE(1009,buffer,function(statusCode,payload) {
              ...
              if (statusCode == 200) {
                // -------------------------
                payloadString = JSON.stringify(payload);
                console.log(`..1009...decodeF8L10ST_ATMODE...${statusCode }`)
                // console.log(payload);
              _logs.append('_NIPPONDEMO',`${payloadString}`,()=>{});
              }
            })
          ...
          if (statusCode == 200) {
            ...
            if (SHINKOIDs.includes(Number(payload._DTUID))) {
              ...
              _data.read('shinko','settings',function(err,settingData) {
                ...
                let _UNITSYSTEM = _FoundSensor.UNITSYSTEM ? Number(_FoundSensor.UNITSYSTEM) : null;
                ...
                switch (_TYPE) {
                  ...
                  case 'AIR PRESSURE':
                      ...
                      _FLAG && (_MESSAGE += ` PRESS=${_READINGPRESSURE}>${_PRESSMAX}Pa`);
                      _FLAGPRESS = _FLAGPRESS || _FLAG;
                      _FLAG = (_PRESSMIN == null) ? false : (_READINGPRESSURE < _PRESSMIN) ? true: false;
                      _FLAG && (_MESSAGE += ` PRESS=${_READINGPRESSURE}<${_PRESSMIN}Pa`);
