  ...
  const tcpServer = net.createServer(function(socket) {
    ...
    socket.on("data", function (data) {
      ...
      if (buffer.length > 4) {
        ...
        ( blnDEBUG && (_APKEY!="0D")) && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PORTID}> [-293-] APKEY=[${_APKEY}] REGKEY=${MATCHKEY} BUFFER=[${buffer.length}]${buffer}`,()=>{});
        ( blnDEBUG && (_APKEY!="0D")) && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).rainbow}> [-294-] APKEY=[${String(_APKEY).red}] REGKEY=[${String(MATCHKEY).green}] BUFFER=[${String(buffer.length).yellow}]${String(helpers.truncateString(buffer,35)).rainbow}`);
        // -----------------------------------
        if (_REGISTERKEYS?.[_PORTID] && _REGISTERKEYS?.[_PORTID].includes(MATCHKEY)) {
          // ---------------------------------------------------
          let DEVICEID = helpers.hex_to_ascii(buffer.substring(10, 30));
          console.log(`[${"SERVER.JS".yellow}] ${helpers.GetTIMEStamp()} <${String(_PORTID).magenta}> DEVICE=${DEVICEID.green} [${String(_RAWFile).cyan}].. REGKEY=[${MATCHKEY.cyan}] APKEY=[${_APKEY.yellow}]`);
          _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PORTID}> DEVICE=${DEVICEID} REGKEY=${MATCHKEY} APKEY=${_APKEY}`,()=>{});
          // -------------
          if (DEVICEID) {
          ...
          if (DataArr.length < 4) {
            if (_HEADER === 'FA') {
              ...
              // -----------------------
              false && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PORTID}> [${String(_CMODE)}] HEADER=${String(_HEADER)} DATAARR=[${String(DataArr.length)}] =decodeF8L10ST_ATMODE=`,()=>{});
              false && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] HEADER=${String(_HEADER).green} DATAARR=[${String(DataArr.length).yellow}] =decodeF8L10ST_ATMODE=`);
              // ---------------------
              const DTUIDMap = {
                11: 102,
                1: 2
              ...
              // -------------------------------------------------------------
              false && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PORTID}> [${String(_CMODE)}] HEADER=${String(_HEADER)} DATAARR=[${String(DataArr.length)}] =decodeWISensorV2=`,()=>{});
              false && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] HEADER=${String(_HEADER).green} DATAARR=[${String(DataArr.length).yellow}] =decodeWISensorV2=`);
              // ---------------------  
              true && !blnSKIPWISENSOR2 && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] HEADER=${String(_HEADER).yellow} =decodeWISensorV2= [${String(DataArr?.length).red}][${String(helpers.trimHexArray(DataArr)).magenta}]`);
              true && !blnSKIPWISENSOR2 && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID)}> [${_CMODE}] HEADER=${String(_HEADER)} =decodeWISensorV2= [${DataArr?.length}][${String(helpers.trimHexArray(DataArr2C))}]`,()=>{});
              ...
              decoders.decodeWISensorV2(_PORTID,blnDEBUG,buffer,function (statusCode, payload) {
                ...
                !blnSKIPWISENSOR2 && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] LINE:407 ${helpers.GetTIMEStamp()} <${_PORTID}> [${_CMODE}] =decodeWISensorV2= HEADER=[${_HEADER}] <${statusCode}> [${modelID}]`,()=>{});
                  ...
                  _data.read(FileName,'settings',function(err,settingData) {
                    ...
                    switch (_TYPE) {
                      ...
                      case 'AIR PRESSURE':
                          ...
                          // ------
                          true && console.log('LINE394 DECODEWISENSORV2',_READINGPRESSURE,_PRESSMIN,_PRESSMAX);
                      ...
                      let _FLAGTEMPRH = false;
                        ...
                        _FALSESIGNAL = _READINGTEMP > Number(filterLIMITS['TEMPERATURE_MAX']) || _READINGTEMP < Number(filterLIMITS['TEMPERATURE_MIN']) || _READINGRH < Number(filterLIMITS['HUMIDITY_MIN']);;
                        if (_FALSESIGNAL) return;
                        // ---------
                    ...
                    //  -------------
                    !blnSKIPWISENSOR2 && blnDEBUG && console.log(`[${"SERVER.JS".white}] LINE:532 ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${_CMODE.red}] ..[${String(_RAWFile).cyan}].. [${'decodeWISensorV2'.red}] <${String(statusCode).magenta}> [A|F=${String(_ALERT).yellow}|${String(_FLAG).cyan}] ..MESSAGE=${_MESSAGE}`);
                    if (_FLAG) {
              ...
              // ------------------
              false && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PORTID}> [${String(_CMODE)}] HEADER=${String(_HEADER)} DATAARR=[${String(DataArr.length)}] =decode485Sensor=`,()=>{});
              false && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] HEADER=${String(_HEADER).green} DATAARR=[${String(DataArr.length).yellow}] =decode485Sensor=`);
              // ---------------------  
              false && !blnSKIP485SENSOR && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${_CMODE.red}] HEADER=${String(_HEADER).yellow} =decode485Sensor= [${String(helpers.trimHexArray(DataArr)).magenta}]`);
              decoders.decode485Sensor(_PORTID,DataArr,function(statusCode,payload) {
                ...
                if (statusCode == 200)  {
                  ...
                  !blnSKIP485SENSOR && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] LINE:508 [${String(_RAWFile).cyan}] [${'decode485Sensor'.red}] <${String(statusCode).magenta}> HEADER=[${_HEADER.yellow}]..${payload.DTUID}|${payload.SENSORID}|${payload.SENSORTYPE}`);
                  !blnSKIP485SENSOR && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PORTID}> [${_CMODE}] =decode485Sensor= HEADER=[${_HEADER}] <${statusCode}> ${SENSORTYPE} [${FUNCID}|${DTUID}|${SENSORID}]`,()=>{}); 
                } else {
                  false && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()}  <${_PORTID}> -${_CMODE}- =decode485Sensor= HEADER=[${_HEADER}] <${statusCode}> ..`,()=>{});
                }
              });
            }
            // ---------------
            ...
            // --------
            false && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${_PORTID}> [${String(_CMODE)}] HEADER=${String(_HEADER)} DATAARR=[${String(DataArr.length)}] =decodeWISensorV1=`,()=>{});
            false && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${String(_CMODE).red}] HEADER=${String(_HEADER).green} DATAARR=[${String(DataArr.length).yellow}] =decodeWISensorV1=`);
            // ---------------------  
            false && !blnSKIPWISENSOR1 && blnDEBUG && console.log(`[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID).yellow}> [${_CMODE.red}] HEADER=${String(_HEADER).yellow} =decodeWISensorV1= [${String(DataArr?.length).red}][${String(helpers.trimHexArray(DataArr)).magenta}]`);
            false && !blnSKIPWISENSOR1 && blnDEBUG && _logs.append(_PORTIDFile, `[SERVER.JS] ${helpers.GetTIMEStamp()} <${String(_PORTID)}> [${_CMODE}] HEADER=${String(_HEADER)} =decodeWISensorV1= [${DataArr?.length}][${String(helpers.trimHexArray(DataArr))}]`,()=>{});
                ...
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  switch (_TYPE) {
                    ...
                    case 'AIR PRESSURE':
                        ...
                        _PRESSMIN = _UNITSYSTEM == 'BAR' ? _PRESSMIN : (_PRESSMIN*1.0E-5).toFixed(2);
                        // ------
                        true && console.log('LINE575 DECODEWISENSORV1',_READINGPRESSURE,_PRESSMIN,_PRESSMAX)
                        // ------
                        _FLAG = (_PRESSMAX == null) ? false : (_READINGPRESSURE > _PRESSMAX) ? true: false;
                        _FLAG && (_MESSAGE += ` PRESS=${_READINGPRESSURE}>${_PRESSMAX} ${_UNITSYSTEM}`);
                        _FLAGPRESS = _FLAGPRESS || _FLAG;
          ...
          _CMODE = 'APMOD'
          true && blnDEBUG && _logs.append(_PORTIDFile, `[${'SERVER.JS'}] ${helpers.GetTIMEStamp()} <${String(_PORTID)}> [${String(_CMODE)}] LINE:724 APKEY=${String(_APKEY)} CHECK [FA,FE,FD]... ENTER <decodeF8L10ST>`,()=>{});
          ...
          decoders.decodeF8L10ST_APIMODE(_PORTID,buffer,blnDEBUG,function(statusCode,payload) {
            ...
            blnDEBUG && console.log(`  >>[SERVER.JS] LINE:743 <${String(_PORTID).green}> [${String(_CMODE).red}] <${FileName}> STATUS CODE=[${String(statusCode).yellow}] MODE=[${_MODE}] APKEY=[${String(_APKEY).green}] MAC ID=[${String(_DTUID).blue}] FILENAME=[${String(FileName).red}] PAYLOAD=${String(_HEX).cyan}`)
            blnDEBUG && _logs.append(_PORTIDFile, ` LINE:744 <${String(_PORTID)}> [${String(_CMODE)}] STATUS CODE=[${statusCode}] MODE=[${_MODE}] APKEY=[${_APKEY}] MAC ID=[${_DTUID}] FILENAME=${String(FileName)} PAYLOAD=${String(_HEX)}`,()=>{});
            // -------------------------
            ...
            if (statusCode == 407) {
              if (_MODE == 'HEART-BEAT') {
                ...
                _data.read(FileName,'settings',function(err,settingData) {
                  ...
                  _ALERTObj['TIMESTAMP'] = new Date();
                  let _MESSAGE = `[${_NAME.toUpperCase()}]`;
                  blnDEBUG && console.log(`  >>[SERVER.JS] LINE:798 <${String(_PORTID).green}> [${String(FileName).yellow}] ID=[${String(_DTUID).cyan}] TYPE=${String(_TYPE).magenta} UNIT=<${_UNITSYSTEM}> ALERTFLAG=[${String(_ALERTFLAG).yellow}]`);
                  //  --------------
                  switch (_TYPE) {
                    case 'WATER LEVEL':
                    ...
                    case 'AC CURRENT':
                      ...
                      _ALERTObj['CURRENT'] = _READINGCURRENT;
                      blnDEBUG && console.log(`  >>[SERVER.JS] LINE:833 PORT ID=[${_PORTID}] [${_GROUP}][${_NAME}][${_TYPE}] <${_HEX}> [${_CTRATIO}] ..${_READINGCURRENT}=${_AMPMAX}|${_AMPMIN}`);
                      break;
                    case 'DIFF PRESS':
                  ...
                  (blnDEBUG  && _FLAG) && console.log(`[${"SERVER.JS".white}] ${helpers.GetTIMEStamp()} <${_PORTID.toString().green}> ..[${String(_RAWFile).cyan}].. [${'DECODEF8L10ST'.red}] <${String(statusCode).magenta}> [A|F=${String(_ALERT).yellow}|${String(_FLAG).cyan}] <${String(_TYPE).yellow}:${_DTUID}> ::${String(_MESSAGE).bgGreen}`);
                  // -- WRITE TO LOGS
                  var jsonPayload = JSON.stringify(sensorObj);
                  if (payload._SENSORID >= 0) _logs.append(_RAWFile,jsonPayload,()=>{});  
                  // -- CHECK ALERTS
          ...
          blnDEBUG && console.log(`[${'SERVER.JS'}] <${String(_PORTID).rainbow}> ${helpers.GetTIMEStamp()} ${String('LINE:981 !!!MISSING GATEWAY!!! ..REG KEY..').yellow} PORT=${String(_PORTID).green} REGKEY=${MATCHKEY.yellow} _APKEY=${String(_APKEY).blue}`);
          blnDEBUG && console.log(`[${'SERVER.JS'}] <${String(_PORTID).rainbow}> ${helpers.GetTIMEStamp()} ${String(buffer).rainbow}`);
    ...
    socket.once("end", function () {
      // --------------------------
      blnDEBUG && console.log(`[${'SERVER.JS'.yellow}] ${helpers.GetTIMEStamp()} <${String(_PORTID).green}>  ...${'GATEWAY SOCKET END'.red}...`);
    ...
    socket.once("close", function () {
      try {
        // ------
        blnDEBUG && console.log(`[${'SERVER.JS'.yellow}] ${helpers.GetTIMEStamp()} <${String(_PORTID).green}> ...${'GATEWAY SOCKET CLOSE'.red}...`);
    ...
    //  -------------------------
    socket.on("error", function (err) {
      blnDEBUG && console.log(`[${'SERVER.JS'.yellow}] ${helpers.GetTIMEStamp()} <${String(_PORTID).green}> ...${'GATEWAY SOCKET ERROR'}..${err.errno}|${err.code}..`);
...
function DISPATCH_ALERT(FileName,_PortALERTFILE,_FoundSensor,_ALERTObj,_ALERTFLAG,_ALERT) {
  ...
  _logs.read(_PortALERTFILE,10,null,null,false,function(err,alertsdata){
    ...
    let filteredData = [];
    deBUG && console.log(`  >>[SERVER.JS] LINE:1047 <${String('DISPATCH ALERT').green}> <${FileName.red}> .[${String(_FoundSensor.ALERTGROUP).yellow}]. -FLAG=${String(_ALERTFLAG).green}|${_ALERT}|${_ALERTObj.MESSAGE.red}-`)
    if (_ALERTFLAG > 0) {
      ...
      deBUG && console.log(`  >>[SERVER.JS] LINE:1054 <${FileName.red}> ..#ALERTS COUNT=${AlertCount}  FLAG=${_ALERTFLAG}`);
      if ( AlertCount <= _ALERTFLAG){
        false && console.log(`  >>[SERVER.JS] LINE:1056 COUNT [${AlertCount}] < FLAG [${_ALERTFLAG}]`)
        return;
      }
    }
    deBUG && console.log(`  >>[SERVER.JS] LINE:1060 <${FileName.red}> ${String(_ALERT).toUpperCase().blue} ALERT.GROUP=[${String(_FoundSensor.ALERTGROUP).yellow}] `)
    if (!_ALERT) return;
    switch (FileName) {
      case 'MRE':
...
server.init = function () {
  ...
  false && startTCPServer('CAMPBELL',config.tcpPort1016,false);         // AP
  false && startTCPServer('KAYAKU',config.tcpPort1015,false);           // AP
  true && startTCPServer('AEROSOFT',config.tcpPort1088,false);           // TRANS
